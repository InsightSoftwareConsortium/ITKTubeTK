...
public:
  ...
  void
  SetImageTop(typename ImageType::Pointer _top)
  {
    m_ImageTop = _top;
  }

  void
  SetImageMiddle(typename ImageType::Pointer _middle)
  {
    m_ImageMiddle = _middle;
  }

  void
  SetImageBottom(typename ImageType::Pointer _bottom)
  {
    m_ImageBottom = _bottom;
  }

  void
  SetImageMiddleTarget(typename ImageType::Pointer _targetMiddle)
  {
    m_ImageMiddleTarget = _targetMiddle;
  }

  void
  SetMetricMask(typename ImageType::Pointer _MetricMask)
  {
    m_MetricMask = _MetricMask;
  }

  void
  SetImageOutput(typename ImageType::Pointer _output)
  {
    m_ImageOutput = _output;
  }

  void
  SetScales(ParametersType & _scales)
  {
    m_Scales = _scales;
  }
  ...
  void
  GetDerivative(const ParametersType & params, DerivativeType & deriv) const override
  {
  ...
  MeasureType
  GetValue(const ParametersType & params) const override
  {
    ...
    ConstImageIteratorType iterBottomB(m_ImageBottom, m_ImageBottom->GetLargestPossibleRegion());
    ConstImageIteratorType iterMiddle(m_ImageMiddle, m_ImageMiddle->GetLargestPossibleRegion());
    ConstImageIteratorType iterTopB(m_ImageTop, m_ImageTop->GetLargestPossibleRegion());
    ConstImageIteratorType iterMiddleTarget(m_ImageMiddleTarget, m_ImageMiddleTarget->GetLargestPossibleRegion());
    ImageIteratorType      iterOutput(m_ImageOutput, m_ImageOutput->GetLargestPossibleRegion());
    ...
    ConstImageIteratorType iterMask(tmpMask, tmpMask->GetLargestPossibleRegion());
    while (!iterMiddle.IsAtEnd())
    {
      float tf = (params[0] * iterBottomB.Get() + iterMiddle.Get() + params[1] * iterTopB.Get()) + params[2];
    ...
    if (count255 > 0 && countNot > 0)
    {
      ...
      result = -std::fabs(mean255 - meanNot) / std::sqrt(stdDev255 * stdDevNot);
    }

    std::cout << ++m_CallsToGetValue << " : " << params[0] << ", " << params[1] << ", " << params[2] << ", ";
...
public:
  ...
  typedef itk::tube::SmoothingRecursiveGaussianImageFilter<ImageType, ImageType> BlurFilterType;
  ...
  GetValue(const ParametersType & params) const override
  {
    ...
    ConstImageIteratorType iterBottomB(imageBottomB, imageBottomB->GetLargestPossibleRegion());
    ConstImageIteratorType iterMiddle(m_ImageMiddle, m_ImageMiddle->GetLargestPossibleRegion());
    ConstImageIteratorType iterTopB(imageTopB, imageTopB->GetLargestPossibleRegion());
    ConstImageIteratorType iterMiddleTarget(m_ImageMiddleTarget, m_ImageMiddleTarget->GetLargestPossibleRegion());
    ImageIteratorType      iterOutput(m_ImageOutput, m_ImageOutput->GetLargestPossibleRegion());
...
DoIt(int argc, char * argv[])
{
  ...
  tube::CLIProgressReporter progressReporter("DeblendImages", CLPProcessInformation);
  ...
  timeCollector.Start("Read");
  {
    ...
    {
      tube::ErrorMessage("Reading bottom. Exception caught: " + std::string(err.GetDescription()));
    ...
    {
      tube::ErrorMessage("Reading middle. Exception caught: " + std::string(err.GetDescription()));
      timeCollector.Report();
      return EXIT_FAILURE;
    }
    ...
    {
      tube::ErrorMessage("Reading top. Exception caught: " + std::string(err.GetDescription()));
      timeCollector.Report();
      return EXIT_FAILURE;
    }
  ...
  if (1)
  {
    typedef itk::tube::BlendCostFunction<PixelType, VDimension> BlendCostFunctionType;
    typedef itk::OnePlusOneEvolutionaryOptimizer                InitialOptimizerType;
    typedef itk::FRPROptimizer                                  OptimizerType;

    typename BlendCostFunctionType::Pointer costFunc = BlendCostFunctionType::New();
    ...
    itk::Statistics::NormalVariateGenerator::Pointer normGen = itk::Statistics::NormalVariateGenerator::New();
    if (seed != 0)
    {
    ...
    std::cout << "Intermediate blendParams = " << blendParams << " Result = " << result << std::endl;
    ...
    std::cout << "Winning blendParams = " << blendParams << " Result = " << result << std::endl;
  }
  ...
  if (1)
  {
    typedef itk::tube::BlendScaleCostFunction<PixelType, VDimension> BlendScaleCostFunctionType;
    typedef itk::OnePlusOneEvolutionaryOptimizer                     InitialOptimizerType;
    typedef itk::FRPROptimizer                                       OptimizerType;

    typename BlendScaleCostFunctionType::Pointer costFunc = BlendScaleCostFunctionType::New();
    costFunc->SetImageBottom(imageBottom);
    costFunc->SetImageMiddle(imageMiddle);
    costFunc->SetImageTop(imageTop);
    costFunc->SetImageMiddleTarget(imageMiddleTarget);
    ...
    std::cout << "Intermediate blendScaleParams = " << blendScaleParams << " Result = " << result << std::endl;
    ...
    std::cout << "Winning blendScaleParams = " << blendScaleParams << " Result = " << result << std::endl;
  }
