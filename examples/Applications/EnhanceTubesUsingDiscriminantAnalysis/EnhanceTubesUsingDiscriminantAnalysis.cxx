...
void
WriteImageInSequence(const typename TImage::Pointer & img, const std::string & base, const std::string & ext, int num)
{
...
template <class TImage>
void
WriteImage(const typename TImage::Pointer & img, const std::string & str)
{
  typedef itk::ImageFileWriter<TImage> ImageWriterType;
...
DoIt(int argc, char * argv[])
{
  ...
  typedef itk::tube::RidgeSeedFilterIO<InputImageType, LabelmapType> RidgeSeedFilterIOType;

  typedef itk::tube::RidgeSeedFilter<InputImageType, LabelmapType> RidgeSeedFilterType;

  typedef typename RidgeSeedFilterType::ProbabilityImageType OutputImageType;
  typedef itk::ImageFileWriter<OutputImageType>              OutputImageWriterType;

  typedef itk::Image<float, TDimension> RidgeSeedImageType;

  typename RidgeSeedFilterType::Pointer tubeFilter = RidgeSeedFilterType::New();
  ...
  if (loadDiscriminantInfo.empty())
  {
    tubeFilter->GetPDFSegmenter()->SetProbabilityImageSmoothingStandardDeviation(tubeScales[0]);
  }
  ...
  if (outputSeedScaleImage.size() > 0)
  {
    ...
    WriteImage<RidgeSeedImageType>(tubeFilter->GetOutputSeedScales(), outputSeedScaleImage);
  ...
  if (saveFeatureImages.size() > 0)
  {
    ...
    {
      WriteImageInSequence<RidgeSeedImageType>(tubeFilter->GetBasisImage(i), saveFeatureImages, ".f%02d.mha", i);
    }
  ...
  typename OutputImageWriterType::Pointer outputWriter = OutputImageWriterType::New();
  outputWriter->SetFileName(outputVolume.c_str());
  outputWriter->SetUseCompression(true);
  outputWriter->SetInput(tubeFilter->GetClassLikelihoodRatioImage(0));
  outputWriter->Update();
