...
template <class T>
bool
check_vnl_vector_equality(const vnl_vector<T> & v, const vnl_vector<T> & g, double tol = 1.0e-6)
{
  if (v.size() != g.size())
  {
    return false;
...
template <class T>
bool
check_vnl_matrix_equality(const vnl_matrix<T> & V, const vnl_matrix<T> & G, double tol = 1.0e-6)
{
...
void
CreateEmptyImage(typename TImage::Pointer & outImage, typename TImage::SizeType targetSize)
{
...
template <class TImage>
bool
CheckCompatibility(typename TImage::Pointer imageA, typename TImage::Pointer imageB)
{
  // Spacing tolerance is imageA's 1st coord. spacing * 1e-6
  double spacingTol = imageA->GetSpacing()[0] * 1.0e-6;

  if (!check_vnl_vector_equality(imageA->GetOrigin().GetVnlVector(), imageB->GetOrigin().GetVnlVector()))
  {
    tube::ErrorMessage("Origin mismatch between input images!");
    return false;
  }
  if (!check_vnl_vector_equality(imageA->GetSpacing().GetVnlVector(), imageB->GetSpacing().GetVnlVector(), spacingTol))
  {
  ...
  if (!check_vnl_matrix_equality(imageA->GetDirection().GetVnlMatrix().as_ref(),
...
DoIt(int argc, char * argv[])
{
  ...
  if (!IsDiscrete(argInImageFileName) || !IsDiscrete(argInLabelFileName))
  {
  ...
  typename InputReaderType::Pointer inImageReader = InputReaderType::New();
  ...
  typename InputReaderType::Pointer inLabelImageReader = InputReaderType::New();
  ...
  typename itk::ImageRegionIteratorWithIndex<InputImageType> inImageIt(inImage, inImage->GetLargestPossibleRegion());
  ...
  tube::FmtDebugMessage("%d distinct CVT cells!", static_cast<int>(cvtCellIdentifiers.size()));

  TPixel maxCVTIndex = *std::max_element(cvtCellIdentifiers.begin(), cvtCellIdentifiers.end());
  TPixel minCVTIndex = *std::min_element(cvtCellIdentifiers.begin(), cvtCellIdentifiers.end());
  ...
  tube::FmtDebugMessage("Maximum CVT identifier = %d!", static_cast<int>(maxCVTIndex));
  ...
  typename itk::ImageRegionIteratorWithIndex<InputImageType> inLabelImageIt(inLabelImage,
                                                                            inLabelImage->GetLargestPossibleRegion());
  ...
  tube::FmtDebugMessage("Identified %d unique labels ( max. = %d )!", labelSet.size(), static_cast<int>(maxLabel));
  ...
  {
    tube::FmtDebugMessage("%.4d mapsto %.4d", (*u).first, (*u).second);
    ++u;
  }
  ...
  CreateEmptyImage<OutputImageType>(outImage, inImage->GetLargestPossibleRegion().GetSize());


  // Count newly created IDs for CVT cells, other than segmentation labels
  unsigned int omittedRegionCounter = maxLabel + 1;
  tube::FmtDebugMessage("OmittedRegionCounter init to %d", omittedRegionCounter);
  ...
  {
    tube::FmtErrorMessage("Could not open %s for writing!", argOutMappingFileName.c_str());
    return EXIT_FAILURE;
  }
  ...
  while (mapIt != cvtToIndex.end())
  {
    ...
    const std::vector<IndexType> & indexVector = (*mapIt).second;

    tube::FmtDebugMessage(
      "Cell %d: Index vector has size = %d!", static_cast<int>(cell), static_cast<int>(indexVector.size()));
    ...
    unsigned int dominantLabel = distance(cellHist.begin(), std::max_element(cellHist.begin(), cellHist.end()));

    // Map the artificial label to the original one
    TPixel originalLabel = labelRemapInv[dominantLabel];
    tube::FmtDebugMessage(
      "Dominant label in CVT cell %d = %d ( %d )", cell, dominantLabel, labelRemapInv[dominantLabel]);
  ...
  tube::FmtDebugMessage("Introduced %d new labels!", omittedRegionCounter - maxLabel - 1);
