...
DoIt(int argc, char * argv[])
{
  ...
  tube::CLIProgressReporter progressReporter("SegmentUsingQuantileThreshold", CLPProcessInformation);
  ...
  if (thresholdQuantile >= 0 && thresholdQuantile <= 1.0)
  {
    ...
    typedef boost::accumulators::
      accumulator_set<PixelType, boost::accumulators::stats<boost::accumulators::tag::p_square_quantile>>
     ...
     */
    QuantileAccumulatorType acc(boost::accumulators::quantile_probability = thresholdQuantile);
    ...
    if (!maskVolume.empty())
    {
      ImageIteratorType maskIt(maskImage, maskImage->GetLargestPossibleRegion());
      while (!imIt.IsAtEnd())
      {
