...
DoIt(int argc, char * argv[])
{
  ...
  using RegistrationType = typename itk::tube::SpatialObjectToImageRegistrationHelper<TDimension, ImageType>;
  ...
  typename SpatialObjectReaderType::Pointer soReader = SpatialObjectReaderType::New();
  ...
  if (metric == "ImageIntensityMetric")
  {
    ...
    reger->SetRigidMetricMethodEnum(RegistrationType ::OptimizedRegistrationMethodType ::IMAGE_INTENSITY_METRIC);
    reger->SetAffineMetricMethodEnum(RegistrationType ::OptimizedRegistrationMethodType ::IMAGE_INTENSITY_METRIC);
  }

  using ImageReader = typename itk::ImageFileReader<itk::Image<unsigned char, TDimension>>;
  ...
  if (movingSpatialObjectMask != "")
  {
    ...
    {
      std::cerr << "Exception caught while loading moving image mask." << std::endl;
      std::cerr << exception << std::endl;
      return EXIT_FAILURE;
    }
  ...
  if (resampledSpatialObject.size() > 1)
  {
    ...
    try
    {
      resultSpatialObject = reger->ResampleSpatialObject(NULL, NULL, resampledPortion);
    }
    catch (itk::ExceptionObject & exception)
    {
    ...
    try
    {
      ...
      typename SOWriterType::Pointer soWriter = SOWriterType::New();
