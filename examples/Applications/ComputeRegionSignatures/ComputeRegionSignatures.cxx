...
template <typename T>
bool
CheckVNLVectorEquality(const vnl_vector<T> & v, const vnl_vector<T> & g, double tol = 1.0e-6)
{
  if (v.size() != g.size())
  {
    return false;
...
template <typename T>
bool
CheckVNLMatrixEquality(const vnl_matrix<T> & V, const vnl_matrix<T> & G, double tol = 1.0e-6)
{
...
bool
CheckCompatibility(typename ImageT::Pointer imageA, typename ImageT::Pointer imageB)
{
  // Spacing tolerance is imageA's 1st coord. spacing * 1e-6
  const double spacingTol = imageA->GetSpacing()[0] * 1.0e-6;

  if (!CheckVNLVectorEquality(imageA->GetOrigin().GetVnlVector(), imageB->GetOrigin().GetVnlVector()))
  {
    tube::ErrorMessage("Origin mismatch between input images");
    return false;
  }
  if (!CheckVNLVectorEquality(imageA->GetSpacing().GetVnlVector(), imageB->GetSpacing().GetVnlVector(), spacingTol))
  {
  ...
  if (!CheckVNLMatrixEquality(imageA->GetDirection().GetVnlMatrix().as_ref(),
...
void
CreateEmptyImage(typename ImageT::Pointer & outImage, typename ImageT::SizeType targetSize)
{
 ...
 */
bool
IsDiscrete(const std::string & fileName)
{
  ...
  if (pixelType == itk::ImageIOBase::FLOAT || pixelType == itk::ImageIOBase::DOUBLE)
  {
    return false;
  }
  return true;
...
void
CreateRenumberingMap(const std::set<TPixel> & in, std::map<TPixel, unsigned int> & map)
{
  unsigned int                              cnt = 0;
...
DoIt(int argc, char ** argv)
{
  ...
  typedef std::vector<typename InputImageType::IndexType> InputIndexVectorType;
  ...
  if (!IsDiscrete(argSegImageFileName) || !IsDiscrete(argCVTImageFileName))
  {
  ...
  if (cvtCenterFile.is_open())
  {
    ...
    tube::FmtInfoMessage("Reading %d CVT centers ...", nCenters);

    unsigned int centerCounter = 0;
    while (cvtCenterFile >> c0 >> c1 >> c2)
    {
      itkAssertOrThrowMacro(VImageDimension == 3, "No support for 2-D CVT center file");
    ...
    cvtCenterFile.close();
    itkAssertOrThrowMacro(centerCounter = nCenters, "#CVT centers mismatch");
  }
  else
  {
    tube::FmtErrorMessage("Could not read %s", argCVTCenterFileName.c_str());
    return EXIT_FAILURE;
  }
  ...
  {
    tube::FmtErrorMessage("%s and %s are incompatible", argSegImageFileName.c_str(), argCVTImageFileName.c_str());
    return EXIT_FAILURE;
  }


  // Determine the number of unqique segmentation regions
  typename itk::ImageRegionIteratorWithIndex<InputImageType> segImageIt(segImage, segImage->GetLargestPossibleRegion());
  ...
  tube::FmtInfoMessage("Found %d distinct segmentation IDs", segmentationRegions.size());
  ...
  typename std::set<TPixel>::const_iterator regionIt = segmentationRegions.begin();
  ...
  itkAssertOrThrowMacro(segmentationRegions.size() == segmentationRegionCounter,
  ...
  ImageIteratorType cvtImageIt(cvtImage, cvtImage->GetLargestPossibleRegion());
  ...
  itkAssertOrThrowMacro(fwdCVTMapper.size() == cvtCellSet.size(), "Size mismatch after renumbering CVT cells");
  ...
  itkAssertOrThrowMacro((largestKey + 1) == static_cast<TPixel>(cvtToIndexVector.size()),
  ...
  for (unsigned int e = 0; e < argExcludeRegions.size(); ++e)
  {
    ...
    itkAssertOrThrowMacro(mappedRegionID < segmentationRegions.size(), "Excluded region ID out of range");
  ...
  vnl_matrix<TPixel> signatureMatrix(numberOfCVTCells, numberOfSegmentedRegions);
  ...
  while (mapIt != segToIndexVector.end())
  {
    ...
    {
      tube::FmtInfoMessage("Exclude segmentation region %d ( Orig: %d )", id, invSegMapper[id]);
    ...
    {
      tube::FmtInfoMessage(
        "Found %s and %s - Loading ...", fullPrtFilePath.string().c_str(), fullMapFilePath.string().c_str());
    ...
    {
      tube::FmtInfoMessage("Recomputing distance map for ( original ) seg. %d", invSegMapper[id]);

      typename OutputImageType::Pointer outImage = OutputImageType::New();

      CreateEmptyImage<OutputImageType>(outImage, segImage->GetLargestPossibleRegion().GetSize());
      ...
      typedef typename itk::DanielssonDistanceMapImageFilter<InputImageType, OutputImageType> DistanceMapType;
    ...
    switch (argSignatureSelector)
    {
      ...
      case SIGNATURE_CVT_MIN:
      {
        ...
        typename IdToIndexVectorType::iterator cvtToIndexVectorIt = cvtToIndexVector.begin();
        ...
        while (cvtToIndexVectorIt != cvtToIndexVector.end())
        {
          ...
          const std::vector<typename InputImageType::IndexType> & voxelIndices = (*cvtToIndexVectorIt).second;
  ...
  if (outSignatureFile.is_open())
  {
    for (unsigned int r = 0; r < signatureMatrix.rows(); ++r)
    {
      ...
      outSignatureFile << signatureMatrix.get(r, signatureMatrix.columns() - 1) << std::endl;
    }
  }
  else
  {
    tube::FmtErrorMessage("Could not open %s for writing", argDistanceSignatureFileName.c_str());
    return EXIT_FAILURE;
  }
