...
void
WriteBasis(const typename TImage::Pointer & img, std::string base, std::string ext, int num)
{
...
DoIt(int argc, char * argv[])
{
  ...
  using BasisFeatureVectorGeneratorType = itk::tube::BasisFeatureVectorGenerator<InputImageType, MaskImageType>;

  typename NJetFeatureVectorGeneratorType::Pointer fvGenerator = NJetFeatureVectorGeneratorType::New();
  ...
  basisGenerator->SetInputFeatureVectorGenerator(static_cast<FeatureVectorGeneratorType *>(fvGenerator.GetPointer()));
  ...
  else
  {
    ...
    {
      basisGenerator->SetNumberOfLDABasisToUseAsFeatures(objectIdList.size() - 1);
    }
    else
    {
  ...
  if (!saveBasisInfo.empty())
  {
    ...
    itk::tube::MetaNJetLDA basisWriter(fvGenerator->GetZeroScales(),
  ...
  if (!saveFeatureImages.empty())
  {
    ...
    {
      WriteBasis<BasisImageType>(fvGenerator->GetFeatureImage(i), saveFeatureImages, ".f%02d.mha", i);
    }
  }
