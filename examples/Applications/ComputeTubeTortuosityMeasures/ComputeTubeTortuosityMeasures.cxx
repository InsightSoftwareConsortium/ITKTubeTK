...
DoIt(int argc, char * argv[])
{
  ...
  typedef itk::tube::TortuositySpatialObjectFilter<TubeType> TortuosityFilterType;
  ...
  if (basicMetrics)
  {
    metricFlag = metricFlag | TortuosityFilterType::AVERAGE_RADIUS_METRIC | TortuosityFilterType::CHORD_LENGTH_METRIC |
                 TortuosityFilterType::PATH_LENGTH_METRIC;
  }

  if (oldMetrics)
  {
    metricFlag = metricFlag | TortuosityFilterType::DISTANCE_METRIC | TortuosityFilterType::INFLECTION_COUNT_METRIC |
                 TortuosityFilterType::INFLECTION_POINTS_METRIC | TortuosityFilterType::SUM_OF_ANGLES_METRIC;
  }

  if (curvatureMetrics)
  {
    metricFlag = metricFlag | TortuosityFilterType::INFLECTION_COUNT_1_METRIC |
                 TortuosityFilterType::INFLECTION_COUNT_2_METRIC | TortuosityFilterType::PERCENTILE_95_METRIC |
                 TortuosityFilterType::TOTAL_CURVATURE_METRIC | TortuosityFilterType::TOTAL_SQUARED_CURVATURE_METRIC |
                 TortuosityFilterType::CURVATURE_SCALAR_METRIC | TortuosityFilterType::CURVATURE_VECTOR_METRIC;
  }
  ...
  MetricFlagToNameMap[TortuosityFilterType::AVERAGE_RADIUS_METRIC] = "AverageRadiusMetric";
  MetricFlagToNameMap[TortuosityFilterType::CHORD_LENGTH_METRIC] = "ChordLengthMetric";
  MetricFlagToNameMap[TortuosityFilterType::DISTANCE_METRIC] = "DistanceMetric";
  MetricFlagToNameMap[TortuosityFilterType::INFLECTION_COUNT_METRIC] = "InflectionCountMetric";
  MetricFlagToNameMap[TortuosityFilterType::INFLECTION_COUNT_1_METRIC] = "InflectionCount1Metric";
  MetricFlagToNameMap[TortuosityFilterType::INFLECTION_COUNT_2_METRIC] = "InflectionCount2Metric";
  MetricFlagToNameMap[TortuosityFilterType::PATH_LENGTH_METRIC] = "PathLengthMetric";
  MetricFlagToNameMap[TortuosityFilterType::PERCENTILE_95_METRIC] = "Percentile95Metric";
  MetricFlagToNameMap[TortuosityFilterType::SUM_OF_ANGLES_METRIC] = "SumOfAnglesMetric";
  MetricFlagToNameMap[TortuosityFilterType::TOTAL_CURVATURE_METRIC] = "TotalCurvatureMetric";
  MetricFlagToNameMap[TortuosityFilterType::TOTAL_SQUARED_CURVATURE_METRIC] = "TotalSquaredCurvatureMetric";

  // Run tortuosity filter
  timeCollector.Start("Computing tortuosity measures");

  char                childName[] = "Tube";
  TubeListPointerType tubeList = pTubeGroup->GetChildren(pTubeGroup->GetMaximumDepth(), childName);

  vtkSmartPointer<vtkIntArray> tubeIdArray = vtkSmartPointer<vtkIntArray>::New();
  tubeIdArray->Initialize();
  tubeIdArray->SetName("TubeIDs");
  tubeIdArray->SetNumberOfValues(tubeList->size());

  vtkSmartPointer<vtkIntArray> numPointsArray = vtkSmartPointer<vtkIntArray>::New();
  ...
  for (int compareFlag = 0x01; compareFlag <= static_cast<int>(TortuosityFilterType::BITMASK_ALL_METRICS);
       compareFlag = compareFlag << 1)
  {
    // If metric is asked to print and is printable
    if ((metricFlag & compareFlag & TortuosityFilterType::BITMASK_VESSEL_WISE_METRICS) > 0)
    {
      vtkSmartPointer<vtkDoubleArray> metricArray = vtkSmartPointer<vtkDoubleArray>::New();
  ...
  if (curvatureMetrics)
  {
    vtkSmartPointer<vtkDoubleArray> tau4Array = vtkSmartPointer<vtkDoubleArray>::New();
  ...
  if ((metricFlag & TortuosityFilterType::CURVATURE_HISTOGRAM_METRICS) > 0)
  {
    ...
    for (int i = 0; i < numberOfHistogramBins; i++)
    {
      ...
      vtkSmartPointer<vtkIntArray> histArray = vtkSmartPointer<vtkIntArray>::New();
  ...
  int tubeIndex = 0;
  for (typename TubeGroupType::ChildrenListType::iterator itTubes = tubeList->begin(); itTubes != tubeList->end();
       ++itTubes)
  {
    ...
    typename TortuosityFilterType::Pointer tortuosityFilter = TortuosityFilterType::New();
    ...
    for (unsigned int i = 0; i < metricArrayVec.size(); i++)
    {
      ...
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetAverageRadiusMetric());
      }
      if (metricName == "ChordLengthMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetChordLengthMetric());
      }
      if (metricName == "DistanceMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetDistanceMetric());
      }
      if (metricName == "InflectionCountMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetInflectionCountMetric());
      }
      if (metricName == "InflectionCount1Metric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetInflectionCount1Metric());
      }
      if (metricName == "InflectionCount2Metric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetInflectionCount2Metric());
      }
      if (metricName == "PathLengthMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetPathLengthMetric());
      }
      if (metricName == "Percentile95Metric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetPercentile95Metric());
      }
      if (metricName == "SumOfAnglesMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetSumOfAnglesMetric());
      }
      if (metricName == "Tau4Metric")
      {
        metricArrayVec[i]->SetValue(
          tubeIndex, tortuosityFilter->GetTotalCurvatureMetric() / tortuosityFilter->GetPathLengthMetric());
      }
      if (metricName == "TotalCurvatureMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetTotalCurvatureMetric());
      }
      if (metricName == "TotalSquaredCurvatureMetric")
      {
        metricArrayVec[i]->SetValue(tubeIndex, tortuosityFilter->GetTotalSquaredCurvatureMetric());
      }
    }
    ...
    if ((metricFlag & TortuosityFilterType::CURVATURE_HISTOGRAM_METRICS) > 0)
    {
      ...
      for (int i = 0; i < numberOfHistogramBins; i++)
      {
        histogramArrays[i]->SetValue(tubeIndex, tortuosityFilter->GetCurvatureHistogramMetric(i));
      }
    }
