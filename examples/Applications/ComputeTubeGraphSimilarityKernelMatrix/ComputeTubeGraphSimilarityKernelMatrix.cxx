 ...
 */
void
readGraphList(const std::string &        fileName,
              ...
              std::vector<int> &         labels)
{
  ...
  try
  {
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type & v, pt.get_child("graphList"))
    {
      ...
      {
        tube::FmtErrorMessage("Graph file %s not found!", fullGraphFileName.c_str());
        throw std::exception();
      }
  ...
  {
    tube::FmtErrorMessage("Error reading JSON graph file %s!", fileName.c_str());
    throw std::exception();
  }
}
 ...
 */
void
writeKernel(const std::string & baseFileName, const vnl_matrix<double> & K)
{
  ...
  if (!ofs)
  {
    tube::FmtErrorMessage("Could not open kernel matrix %s for writing!", outFileName.c_str());
    throw std::exception();
  }
  ofs.write((const char *)K.data_block(), K.size() * sizeof(double));
  if (ofs.bad())
  {
    tube::FmtErrorMessage("Could not write kernel matrix %s!", outFileName.c_str());
    throw std::exception();
  }
  ofs.close();
  if (ofs.fail())
  {
    tube::FmtErrorMessage("Could not close kernel matrix file %s!", outFileName.c_str());
    throw std::exception();
  }
}
 ...
 */
void
writeKernelPlainText(const std::string & baseFileName, const vnl_matrix<double> & K)
{
  ...
  if (!ofs)
  {
    tube::FmtErrorMessage("Could not open kernel matrix %s!", outFileName.c_str());
    throw std::exception();
  }
 ...
 */
void
writeKernelLIBSVM(const std::string & baseFileName, const vnl_matrix<double> & K, const std::vector<int> & labels)
{
 ...
 */
tube::GraphKernel::GraphType
loadGraph(std::string                                graphFile,
          ...
          const std::string &                        globalLabelFile = std::string())
{
  ...
  if (!globalLabelFile.empty())
  {
    ...
    tube::FmtInfoMessage("Trying to use global label file %s", labelFile);
  }
  ...
  else
  {
    ...
    tube::FmtInfoMessage("Trying to use graph-specific label file %s", labelFile);
  }
  // In case it does not exist, reset to 0
  if (!boost::filesystem::exists(labelFile))
  {
    tube::FmtInfoMessage("Label file %s not existent - fallback to default!", labelFile);
    labelFile = 0;
  }
  // Load graph and return
  return tube::GraphKernel::GraphFromAdjFile(graphFile.c_str(), labelFile, defNodeLabel);
}
...
main(int argc, char * argv[])
{
  ...
  try
  {
    ...
    if (argGraphKernelType == GK_WLKernel)
    {
      for (int i = 0; i < N; ++i)
      {
        tube::FmtInfoMessage("Adding data from graph %s", listA[i].c_str());

        tube::GraphKernel::GraphType f = loadGraph(listA[i], defLabelType, argGlobalLabelFileName);
        tube::WLSubtreeKernel::UpdateLabelCompression(f, labelMap, labelCount, argSubtreeHeight);
      }
    }
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j < M; ++j)
      {
        tube::GraphKernel::GraphType g = loadGraph(listB[j], defLabelType, argGlobalLabelFileName);

        tube::FmtInfoMessage("Running kernel on graphs ( %d,%d )", i, j);
        ...
        switch (argGraphKernelType)
        {
          ...
          case GK_WLKernel:
          {
            gk = new tube::WLSubtreeKernel(f, g, labelMap, labelCount, argSubtreeHeight);
