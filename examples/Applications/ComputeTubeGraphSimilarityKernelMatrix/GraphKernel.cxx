...
std::string
GraphKernel::LabelVectorToString(const std::vector<int> & labVec)
{
  std::stringstream res;
  std::copy(labVec.begin(), labVec.end(), std::ostream_iterator<int>(res, ""));
  return res.str();
}


std::string
GraphKernel::BuildPrefixFromVertexID(int v)
{
  std::stringstream res;
  res << v << ",";
  return res.str();
...
void
GraphKernel::CountingSort(std::vector<int> & vec)
{
...
std::string
GraphKernel::BuildNeighborStr(const GraphType & G, int v)
{
  // Algorithm:
  //
  ...
  return BuildPrefixFromVertexID(G[vertex(v, G)].type) + LabelVectorToString(nbVec);
}


bool
GraphKernel::IsValidDefaultNodeLabeling(int desiredType)
{
  switch (desiredType)
  {
    case LABEL_BY_NUM:
...
GraphKernel::GraphFromAdjFile(const char * graphFile, const char * labelFile, DefaultNodeLabelingType defNodeLabel)
{
  std::ifstream reader;
  ...
  tube::FmtInfoMessage("Label file given - reading %s", labelFile);
...
GraphKernel::GraphFromJSONFile(const char * graphFile)
{
  try
  {
    ...
    // Gets #vertices
    int nVertices = boost::lexical_cast<int>(pt.get<std::string>("nVertices"));


    // Parses linkage information
    int                           cnt = 0;
    std::vector<std::vector<int>> linkInfo(nVertices);
    BOOST_FOREACH (boost::property_tree::ptree::value_type & v, pt.get_child("adjM"))
    {
      boost::property_tree::ptree adjInfoTree = (boost::property_tree::ptree)v.second;

      BOOST_FOREACH (boost::property_tree::ptree::value_type & w, adjInfoTree.get_child(""))
      {
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type & v, pt.get_child("dist"))
    {
      boost::property_tree::ptree distInfoTree = (boost::property_tree::ptree)v.second;

      BOOST_FOREACH (boost::property_tree::ptree::value_type & w, distInfoTree.get_child(""))
      {
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type & v, pt.get_child("type"))
    {
      int type = boost::lexical_cast<int>(v.second.data());
      typeInfo.push_back(type);
    }

    assert(linkInfo.size() == distInfo.size() && static_cast<int>(linkInfo.size()) <= nVertices &&
           static_cast<int>(distInfo.size()) <= nVertices && static_cast<int>(typeInfo.size()) <= nVertices);
    ...
    for (unsigned int i = 0; i < linkInfo.size(); ++i)
    {
      ...
      {
        tube::FmtDebugMessage("Adding edge ( %d,%d ) with weight %.5f", linkInfo[i][0], linkInfo[i][j], distInfo[i][j]);
  ...
  {
    tube::FmtErrorMessage("Error reading JSON graph file %s ( Msg: %s )", graphFile, e.what());
    throw;
  }
}
