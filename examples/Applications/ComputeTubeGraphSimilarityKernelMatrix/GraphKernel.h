...
public:
  ...
  typedef boost::property_map<GraphType, boost::vertex_index_t>::type IndexMapType;

  /** Access vertex neighborhoods */
  typedef boost::graph_traits<GraphType>::adjacency_iterator AdjacencyIteratorType;

  using VertexNeighborType = std::pair<AdjacencyIteratorType, AdjacencyIteratorType>;

  /** Access to shortest-path information */
  using DistancePropertyType = boost::exterior_vertex_property<GraphType, double>;
  ...
  static bool
  IsValidDefaultNodeLabeling(int desiredType);

  /** Read graph from adj file */
  static GraphType
  GraphFromAdjFile(const char * graphFile, const char * labelFile, DefaultNodeLabelingType defNodeLabel);
  /** Read graph from JSON file */
  static GraphType
  GraphFromJSONFile(const char * graphFile);
...
protected:
  ...
  static std::string
  BuildNeighborStr(const GraphType & G, int v);

  /** Builds and returns a prefix "<i>," string, see [1] for details */
  static std::string
  BuildPrefixFromVertexID(int v);

  /** Format vector of int as string */
  static std::string
  LabelVectorToString(const std::vector<int> &);

  /** In-place counting sort on int vector */
  static void
  CountingSort(std::vector<int> & vec);
