...
DoIt(int argc, char * argv[])
{
  ...
  tube::CLIProgressReporter progressReporter("ComputeTubeFlyThroughImage", CLPProcessInformation);
  ...
  typedef tube::ComputeTubeFlyThroughImage<TPixel, VDimension> ComputeFlyThroughImageFilterType;

  timeCollector.Start("Computing tube fly through images");

  typename ComputeFlyThroughImageFilterType::Pointer pFlyThroughImageFilter = ComputeFlyThroughImageFilterType::New();
  ...
  {
    tube::ErrorMessage("Error writing tube fly through image: " + std::string(err.GetDescription()));
    timeCollector.Report();
    return EXIT_FAILURE;
  }
  ...
  {
    tube::ErrorMessage("Error writing tube mask fly through image: " + std::string(err.GetDescription()));
    timeCollector.Report();
    return EXIT_FAILURE;
  }
