...
/*! constructor */
QtGlSliceView::
QtGlSliceView( QWidget* p, Qt::WFlags f )
...
const QtGlSliceView::ImageType::Pointer &
QtGlSliceView::
...
void
QtGlSliceView::
size( int itkNotUsed( w ), int itkNotUsed( h ) )
{
  this->update();
  this->paintGL();
}
...
paintGL( void )
{
  ...
  if( cViewValue )
    ...
    if( ( ImagePixelType )1.1==1.1 )
      ...
      sprintf( s, "( %0.1f,  %0.1f,  %0.1f ) = %0.3f",
        cClickSelect[0], cClickSelect[1], cClickSelect[2],
        ( float )cClickSelectV );
      }
    else
      {
  ...
  if( cViewCrosshairs
    ...
    if( cFlipX[cWinOrientation] )
      {
      xx = ( int )( cW - ( cClickSelect[cWinOrder[0]] - cWinMinX )
        * scale0 - originX );
      }
    else
      {
      xx = ( int )( ( cClickSelect[cWinOrder[0]] - cWinMinX )
        * scale0 + originX );
      }
    int yy;
    if( cFlipY[cWinOrientation] )
      {
      yy = ( int )( cH - ( cClickSelect[cWinOrder[1]] - cWinMinY )
        * scale1 - originY );
      }
    else
      {
      yy = ( int )( ( cClickSelect[cWinOrder[1]] - cWinMinY )
        * scale1 + originY );
      }
    glBegin( GL_LINES );
    glVertex2d( 0, yy );
...
{
  double zoomBase = cW/( cDimSize[cWinOrder[0]] *
    ( fabs( cSpacing[cWinOrder[0]] )/fabs( cSpacing[0] ) ) );
  if( zoomBase > cH/( cDimSize[cWinOrder[1]] *
      ( fabs( cSpacing[cWinOrder[1]] )/fabs( cSpacing[0] ) ) ) )
    {
    zoomBase = cH/( cDimSize[cWinOrder[1]] *
      ( fabs( cSpacing[cWinOrder[1]] )/fabs( cSpacing[0] ) ) );
    }
  ...
  if( cClickMode == CM_SELECT || cClickMode == CM_BOX )
    ...
    float p[3];
    p[cWinOrder[0]] = cWinMinX + ( ( 1-cFlipX[cWinOrientation] )
      * ( _event->x()-originX )
      + ( cFlipX[cWinOrientation] )
        * ( this->width()-_event->x()-originX ) ) / scale0;
    ...
    p[cWinOrder[1]] = cWinMinY + ( cFlipY[cWinOrientation]
      * ( _event->y() - originY ) + ( 1-cFlipY[cWinOrientation] )
      * ( this->height()-_event->y()-originY ) ) / scale1;
...
void
QtGlSliceView::
sliceNum( unsigned int newSliceNum )
{
...
ShiftUp( void )
{
  if( cFlipY[ cWinOrientation ] )
    {
    cWinCenter[ cWinOrder[1] ] += 0.1 * cWinSizeY;
    }
  else
    {
    cWinCenter[ cWinOrder[1] ] -= 0.1 * cWinSizeY;
...
ShiftRight( void )
{
  if( cFlipX[ cWinOrientation ] )
    {
    cWinCenter[ cWinOrder[0] ] -= 0.1 * cWinSizeX;
    }
  else
    {
    cWinCenter[ cWinOrder[0] ] += 0.1 * cWinSizeX;
...
SetOrientation( int orientation )
{
  ...
  cWinOrientation = 2-orientation;
  emit MaxNumberOfSlicesChanged( cDimSize[cWinOrientation] );
...
{
  cImageMode = (ImageModeType)viewMode;
