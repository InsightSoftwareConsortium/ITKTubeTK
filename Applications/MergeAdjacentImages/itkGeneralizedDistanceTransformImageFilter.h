...
* radis, for example. If r is a radius map and f(x) = -r(x)^2, then the <=0
...
*
* TEMPLATE PARAMETERS
* \param TFunctionImage is mandatory. It is the first input to the filter and
* - in the wording of the alternative description above - effectively describes
* the height of all parabolas. When you use the filter as an alternative to
* DanielssonMapImageFilter, you need to provide an image that is 0 at foreground
* pixels and infinite (i.e. GetMaximalSquaredDistance()) at background pixels.
*
* \param TDistanceImage - is the type of the image that should hold the output.
* This type defaults to TFunctionImage, but you might need a larger data type
* in order to hold the squared distance to your foreground. For example, in a
* radiologic dataset with pixel type short, you would need a TDistanceImage with
* pixel type int to hold distances larger that 255.
*
* \param TLabelImage If voronoi maps are generated, which is disabled by
* default, you can specify the type of the label image as well. You can use any
* pixel type you like. This will be the type of label input and voronoi map output.
*
* \param MaximalSquaredDistance This is initialized by default to to half of the
* maximum value that TDistanceImage can hold in order to prevent overflows. You
* can get the value by calling GetMaximalSquaredDistance(). You can also set
* a smaller value, i.e. to reinitialize the near band of a level set. The
* computation will be a little bit faster and you might also be able to use a
* smaller pixel type for TDistanceImage.
*
* \param CacheLineSize The length of one or two L1 cache lines on your platform.
* On most current hardware, one line of L1 cache can hold either 64 or 128
* bytes and you have probably 1024 lines per processor. If you set a value below
* your platforms cache line size, the performance wil degrade considerably. Try
* 1 to see how the algorithm performs when the cache is not taken into account.
* If you select the exact cache line size of your hardware and all data types
* are allocated cache line aligned, then GeneralizedDistanceTransformImageFilter
* will utilize each cache line to its fullest. Choosing twice the cache line
* size will be more performant for non cache aligned data. Even larger values
* will probably degrade performance again, because the number of cache lines
...
public:
  ...
  /** Get the largest possible squared distance. Can be changed only at compile
   * time by giving a different template parameter. Using a smaller value will
   * save some runtime, but it is more costly to make it changeable at runtime.
   *
   ...
   * "label" is loosly defined and can be anything you like, including a vector
   * that contains a voxel position from which you can derive a vector distance
   * map. */
...
private:
  ...
  // DETAILS
  //
  // The algorithm builds the lower envelope of parabolas for each scanline and
  // samples them in order to compute the distance map. The following functions
  // and types help with this.

  /** The index positions on which a parabola apex can be. */
  typedef typename DistanceImageType::IndexValueType AbscissaIndexType;

  /** A parabola p(x) = (x-is)^2+y is defined by the apex abscissa index i and
   * the apex height y. The label is additional information for generating a
   ...
   * \param divisionTable Should hold 1/(i * s^2) for i=q.i-p.i and spacing s
   ...
   * \param pl The label that should fill the voronoi region dominated by this
   *           parabola
   ...
   *
   * \param envelope The envelope. If it is empty, nothing will be written to
   *                 the output buffer and the function will return false.
   *                 Otherwise, each parabola needs to be dominant for at least
   *                 one index.
