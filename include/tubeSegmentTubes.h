...
=========================================================================*/
#ifndef tubeSegmentTubes_h
#define tubeSegmentTubes_h
...
class SegmentTubes : public itk::ProcessObject
{
public:
  ...
  void
  SetDataMinMaxLimits(double minLimit, double maxLimit)
  {
  ...
  tubeWrapSetConstReferenceMacro(ExtractBoundMinInIndexSpace, IndexType, Filter);
  tubeWrapGetMacro(ExtractBoundMinInIndexSpace, IndexType, Filter);

  tubeWrapSetConstReferenceMacro(ExtractBoundMaxInIndexSpace, IndexType, Filter);
  ...
  double
  RidgenessInObjectSpace(const PointType & x)
  {
    m_Ridgeness = m_RidgeFilter->Ridgeness(x, m_Intensity, m_Roundness, m_Curvature, m_Levelness);
    return m_Ridgeness;
  };
  ...
  PointType
  FindLocalRidgeInObjectSpace(const PointType & x)
  {
    PointType localX = x;
    m_RidgeFilter->LocalRidge(localX, m_Verbose);
    return localX;
  };

  TubeType *
  ExtractRidgeInObjectSpace(const PointType & x, int tubeId)
  {
