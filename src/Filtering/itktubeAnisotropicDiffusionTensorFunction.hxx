...
AnisotropicDiffusionTensorFunction<TImageType>::AnisotropicDiffusionTensorFunction(void)
{
...
template <class TImageType>
void
AnisotropicDiffusionTensorFunction<TImageType>::PrintSelf(std::ostream & os, Indent indent) const
{
...
AnisotropicDiffusionTensorFunction<TImageType>::ComputeUpdate(const NeighborhoodType & neighborhood,
                                                              void *                   globalData,
                                                              const FloatOffsetType &  offset)
{
  ...
  SpacingType                     spacing;
  spacing = m_Spacing;
  return this->ComputeUpdate(neighborhood, tensorNeighborhood, spacing, globalData, offset);
}

template <class TImageType>
typename AnisotropicDiffusionTensorFunction<TImageType>::PixelType
AnisotropicDiffusionTensorFunction<TImageType>::ComputeUpdate(
  ...
  const FloatOffsetType &                 itkNotUsed(offset))
{
  ...
  this->ComputeIntensityFirstAndSecondOrderPartialDerivatives(neighborhood, gd->m_dx, gd->m_dxy, spacing);

  // Compute diffusion tensor matrix first order partial derivatives
  this->ComputeDiffusionTensorFirstOrderPartialDerivatives(tensorNeighborhood, gd->m_DT_dxy, spacing);
...
template <class TImageType>
void
AnisotropicDiffusionTensorFunction<TImageType>::ComputeDiffusionTensorFirstOrderPartialDerivatives(
  const DiffusionTensorNeighborhoodType & tensorNeighborhood,
  ...
  const SpacingType &                     spacing) const
{
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    DiffusionTensorType positionA_Tensor_value = tensorNeighborhood.GetPixel(m_positionA[i]);
    DiffusionTensorType positionB_Tensor_value = tensorNeighborhood.GetPixel(m_positionB[i]);

    for (unsigned int j = 0; j < ImageDimension; j++)
    {
      firstOrderResult(i, j) = 0.5 * (positionA_Tensor_value(i, j) - positionB_Tensor_value(i, j));
...
AnisotropicDiffusionTensorFunction<TImageType>::ComputeDiffusionTensorFirstOrderPartialDerivatives(
  ...
  const SpacingType &                     spacing) const
{
  this->ComputeDiffusionTensorFirstOrderPartialDerivatives(tensorNeighborhood, firstOrderResult.Value(), spacing);
}

template <class TImageType>
void
AnisotropicDiffusionTensorFunction<TImageType>::ComputeIntensityFirstAndSecondOrderPartialDerivatives(
  const NeighborhoodType & neighborhood,
  ...
  const SpacingType &      spacing) const
{
  ...
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    const ScalarValueType it_positionA = neighborhood.GetPixel(m_positionA[i]);
    const ScalarValueType it_positionB = neighborhood.GetPixel(m_positionB[i]);

    // First order partial derivatives
    firstOrderResult[i] = 0.5 * (it_positionA - it_positionB);

    // Second order partial derivatives
    secondOrderResult[i][i] = (it_positionA + it_positionB - 2.0 * center_value);

    for (unsigned int j = i + 1; j < ImageDimension; j++)
    {
      secondOrderResult[i][j] = secondOrderResult[j][i] // Guaranteed symmetric
        = 0.25 * (neighborhood.GetPixel(m_positionAa[i][j]) - neighborhood.GetPixel(m_positionBa[i][j]) -
                  neighborhood.GetPixel(m_positionCa[i][j]) + neighborhood.GetPixel(m_positionDa[i][j]));
    }
  }
  ...
  const SpacingType &               spacing) const
{
  this->ComputeIntensityFirstAndSecondOrderPartialDerivatives(
    neighborhood, firstOrderResult.Value(), secondOrderResult.Value(), spacing);
}

template <class TImageType>
typename AnisotropicDiffusionTensorFunction<TImageType>::PixelType
AnisotropicDiffusionTensorFunction<TImageType>::ComputeFinalUpdateTerm(
  const DiffusionTensorNeighborhoodType & tensorNeighborhood,
  const GlobalDataStruct *                gd) const
{
  ...
  ScalarValueType pdWrtDiffusion1 =
    gd->m_DT_dxy[0][0] * gd->m_dx[0] + gd->m_DT_dxy[0][1] * gd->m_dx[1] + gd->m_DT_dxy[0][2] * gd->m_dx[2];

  ScalarValueType pdWrtDiffusion2 =
    gd->m_DT_dxy[1][0] * gd->m_dx[0] + gd->m_DT_dxy[1][1] * gd->m_dx[1] + gd->m_DT_dxy[1][2] * gd->m_dx[2];

  ScalarValueType pdWrtDiffusion3 =
    gd->m_DT_dxy[2][0] * gd->m_dx[0] + gd->m_DT_dxy[2][1] * gd->m_dx[1] + gd->m_DT_dxy[2][2] * gd->m_dx[2];

  ScalarValueType pdWrtImageIntensity1 = center_Tensor_value(0, 0) * gd->m_dxy[0][0] +
                                         center_Tensor_value(0, 1) * gd->m_dxy[0][1] +
                                         center_Tensor_value(0, 2) * gd->m_dxy[0][2];

  ScalarValueType pdWrtImageIntensity2 = center_Tensor_value(1, 0) * gd->m_dxy[1][0] +
                                         center_Tensor_value(1, 1) * gd->m_dxy[1][1] +
                                         center_Tensor_value(1, 2) * gd->m_dxy[1][2];

  ScalarValueType pdWrtImageIntensity3 = center_Tensor_value(2, 0) * gd->m_dxy[2][0] +
                                         center_Tensor_value(2, 1) * gd->m_dxy[2][1] +
                                         center_Tensor_value(2, 2) * gd->m_dxy[2][2];

  ScalarValueType total = pdWrtDiffusion1 + pdWrtDiffusion2 + pdWrtDiffusion3 + pdWrtImageIntensity1 +
                          pdWrtImageIntensity2 + pdWrtImageIntensity3;
...
void
AnisotropicDiffusionTensorFunction<TImageType>::CheckTimeStepStability(
  ...
  bool                                        useImageSpacing)
{
  ...
  // TODO plus 1?
  double ratio = minSpacing / std::pow(2.0, static_cast<double>(ImageDimension) + 1);

  if (m_TimeStep > ratio)
  {
    itkWarningMacro(<< std::endl
                    << "Anisotropic diffusion unstable time step:" << m_TimeStep << std::endl
