...
AnisotropicHybridDiffusionImageFilter<TInputImage, TOutputImage>::AnisotropicHybridDiffusionImageFilter(void)
{
...
template <class TInputImage, class TOutputImage>
void
AnisotropicHybridDiffusionImageFilter<TInputImage, TOutputImage>::UpdateDiffusionTensorImage(void)
{
  ...
  */
  ...
  using EigenVectorAnalysisFilterType = SymmetricEigenVectorAnalysisImageFilter<SymmetricSecondRankTensorImageType,
                                                                                EigenValueImageType,
  ...
  eigenVectorAnalysisFilter->OrderEigenValuesBy(EigenValueOrderEnum::OrderByValue);
  ...
  using EigenAnalysisFilterType =
    itk::SymmetricEigenAnalysisImageFilter<SymmetricSecondRankTensorImageType, EigenValueImageType>;
  ...
  eigenAnalysisFilter->OrderEigenValuesBy(EigenValueOrderEnum::OrderByValue);
  ...
  */
  ...
  typename GradientMagnitudeOutputImageType::Pointer            gradientMagnitudeOutputImage =
  ...
  while (!it.IsAtEnd())
  {
    ...
    {
      if (std::fabs(eigenValue[i]) != smallest && std::fabs(eigenValue[i]) != largest)
      {
    ...
    else
    {
      ...
      double expVal = std::exp((-1.0 * m_ThresholdParameterC) / (std::pow(ratio, 4.0)));
    ...
    if ((std::fabs(eigenValue[middleEigenValueIndex]) < zeroValueTolerance) ||
        (std::fabs(eigenValue[smallestEigenValueIndex]) < zeroValueTolerance))
    {
      LambdaCED3 = 1.0;
    }
    else
    {
      double kappa =
        std::pow(((float)(eigenValue[middleEigenValueIndex]) / (m_Alpha + eigenValue[smallestEigenValueIndex])), 4.0);

      double contrastParameterLambdaCEDSquare = m_ContrastParameterLambdaCED * m_ContrastParameterLambdaCED;

      double expVal = std::exp((-1.0 * (std::log(2.0) * contrastParameterLambdaCEDSquare) / kappa));
    ...
    double numerator = eigenValue[middleEigenValueIndex] *
                       ((m_ContrastParameterLambdaHybrid * m_ContrastParameterLambdaHybrid) * (xi - std::fabs(xi)) -
                        2.0 * eigenValue[smallestEigenValueIndex]);


    double denominator = 2.0 * std::pow(m_ContrastParameterLambdaHybrid, 4.0);
    ...
    EigenVectorMatrixType eigenVectorMatrix = eigenVectorImageIterator.Get();
...
template <class TInputImage, class TOutputImage>
void
AnisotropicHybridDiffusionImageFilter<TInputImage, TOutputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "Threshold C parameter " << m_ThresholdParameterC << std::endl;
