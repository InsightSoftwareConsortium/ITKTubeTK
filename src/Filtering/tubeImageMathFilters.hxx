...
ImageMathFilters<VDimension>::ImageMathFilters()
{
  m_Input = nullptr;
}
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ApplyIntensityWindowing(float valMin, float valMax, float outMin, float outMax)
{
  itk::ImageRegionIterator<ImageType> it2(m_Input, m_Input->GetLargestPossibleRegion());
  it2.GoToBegin();
  while (!it2.IsAtEnd())
  {
    double tf = it2.Get();
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ApplyIntensityMultiplicativeBiasCorrection(ImageType * inMeanFieldImage)
{
  itk::ImageRegionIterator<ImageType> it2(inMeanFieldImage, inMeanFieldImage->GetLargestPossibleRegion());
  int                                 count = 0;
  double                              mean = 0;
  it2.GoToBegin();
  while (!it2.IsAtEnd())
  ...
  mean /= count;
  itk::ImageRegionIterator<ImageType> it3(m_Input, m_Input->GetLargestPossibleRegion());
  it3.GoToBegin();
  it2.GoToBegin();
  while (!it3.IsAtEnd())
  {
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ResampleImage(ImageType * ref)
{
  bool doResample = false;
  for (unsigned int i = 0; i < VDimension; i++)
  {
    if (m_Input->GetLargestPossibleRegion().GetSize()[i] != ref->GetLargestPossibleRegion().GetSize()[i] ||
        m_Input->GetLargestPossibleRegion().GetIndex()[i] != ref->GetLargestPossibleRegion().GetIndex()[i] ||
        m_Input->GetSpacing()[i] != ref->GetSpacing()[i] || m_Input->GetOrigin()[i] != ref->GetOrigin()[i])
    {
      doResample = true;
      break;
    }
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::AddUniformNoise(float valMin, float valMax, float noiseMin, float noiseMax, int seed)
{
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::AddGaussianNoise(float valMin, float valMax, float noiseMean, float noiseStdDev, int seed)
{
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::AddImages(ImageType * input2, float weight1, float weight2)
{
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> it2(input2, input2->GetLargestPossibleRegion());
  it1.GoToBegin();
  it2.GoToBegin();
  while (!it1.IsAtEnd())
  {
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::MultiplyImages(ImageType * input2)
{
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> it2(input2, input2->GetLargestPossibleRegion());
  it1.GoToBegin();
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::MirrorAndPadImage(int numPadVoxels)
{
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::NormalizeImage(int normType)
{
  if (normType == 0)
  {
  ...
  else
  {
    ...
    itk::ImageRegionIteratorWithIndex<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
    it1.GoToBegin();
    ...
    while (loop++ < 4)
    {
      ...
      std::cout << "  Mean = " << meanV << " : StdDev = " << stdDevV << std::endl;
      ...
      std::cout << "  binfwhmmin = " << binFWHMMin << std::endl;
      binFWHMMin += (fwhm - bin[(int)binFWHMMin]) / (bin[(int)binFWHMMin + 1] - bin[(int)binFWHMMin]);
      std::cout << "  tweak: binfwhmmin = " << binFWHMMin << std::endl;
      ...
      std::cout << "  binfwhmmax = " << binFWHMMax << std::endl;
      binFWHMMax -= (fwhm - bin[(int)binFWHMMax]) / (bin[(int)binFWHMMax - 1] - bin[(int)binFWHMMax]);
      std::cout << "  tweak: binfwhmmax = " << binFWHMMax << std::endl;
      ...
      double minV = ((binFWHMMin + 0.5) / (nBins - 1.0)) * (binMax - binMin) + binMin;
      double maxV = ((binFWHMMax + 0.5) / (nBins - 1.0)) * (binMax - binMin) + binMin;
    ...
    std::cout << "FINAL: binMin = " << binMin << " : binMax = " << binMax << std::endl;
    std::cout << "  Mean = " << meanV << " : StdDev = " << stdDevV << std::endl;
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::FuseImages(ImageType * input2, float offset2)
{
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> it2(input2, input2->GetLargestPossibleRegion());
  it1.GoToBegin();
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::MedianImage(int filterSize)
{
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ThresholdImage(float threshLow, float threshHigh, float valTrue, float valFalse)
{
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  it1.GoToBegin();
  while (!it1.IsAtEnd())
...
template <unsigned int VDimension>
double
ImageMathFilters<VDimension>::ComputeImageStatisticsWithinMaskRange(ImageType * mask,
                                                                    float       maskThreshLow,
                                                                    float       maskThreshHigh,
                                                                    int         mode)
{
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> it2(mask, mask->GetLargestPossibleRegion());
  it1.GoToBegin();
  it2.GoToBegin();
  double       sum = 0;
  double       sumS = 0;
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::AbsoluteImage(void)
{
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  it1.GoToBegin();
  while (!it1.IsAtEnd())
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ReplaceValuesOutsideMaskRange(ImageType * mask,
                                                            float       maskThreshLow,
                                                            float       maskThreshHigh,
                                                            float       valFalse)
{
  ...
  itk::ImageRegionIterator<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> it2(imf2.GetOutput(), imf2.GetOutput()->GetLargestPossibleRegion());
  it1.GoToBegin();
  it2.GoToBegin();
  while (!it1.IsAtEnd())
  {
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::MorphImage(int mode, int radius, float foregroundValue, float backgroundValue)
{
  typedef itk::BinaryBallStructuringElement<PixelType, VDimension> BallType;
  BallType                                                         ball;
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ReplaceValueWithinMaskRange(ImageType * mask,
                                                          float       maskThreshLow,
                                                          float       maskThreshHigh,
                                                          float       imageVal,
                                                          float       newImageVal)
{
  itk::ImageRegionIterator<ImageType> itIm(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> itMask(mask, mask->GetLargestPossibleRegion());
  while (!itIm.IsAtEnd())
  {
    if (itMask.Get() >= maskThreshLow && itMask.Get() <= maskThreshHigh && itIm.Get() == imageVal)
    {
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::BlurImage(float sigma)
{
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::BlurOrderImage(float sigma, int order, int direction)
{
  typename itk::RecursiveGaussianImageFilter<ImageType>::Pointer filter;
  filter = itk::RecursiveGaussianImageFilter<ImageType>::New();
  filter->SetInput(m_Input);
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::CopyImageInformation(ImageType * sourceImage)
{
...
template <unsigned int VDimension>
std::vector<double>
ImageMathFilters<VDimension>::ComputeImageHistogram(unsigned int nBins, float & binMin, float & binSize)
{
  itk::ImageRegionIteratorWithIndex<ImageType> it1(m_Input, m_Input->GetLargestPossibleRegion());
  it1.GoToBegin();
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::CorrectIntensitySliceBySliceUsingHistogramMatching(unsigned int numberOfBins,
                                                                                 unsigned int numberOfMatchPoints)
{
  typedef itk::Image<PixelType, 2>                                    ImageType2D;
  typedef itk::HistogramMatchingImageFilter<ImageType2D, ImageType2D> HistogramMatchFilterType;
  ...
  im2DIn->Allocate();
  itk::ImageRegionIterator<ImageType>   it3D(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType>   it3DSliceStart(m_Input, m_Input->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType2D> it2DRef(im2DRef, im2DRef->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType2D> it2DIn(im2DIn, im2DIn->GetLargestPossibleRegion());
  unsigned int                          x;
  unsigned int                          y;
  unsigned int                          z;
  it3D.GoToBegin();
  ...
  while (z < zMax)
  {
    ...
    matchFilter->Update();
    itk::ImageRegionIterator<ImageType2D> it2DOut(matchFilter->GetOutput(), im2DIn->GetLargestPossibleRegion());
    it2DRef.GoToBegin();
    it2DOut.GoToBegin();
    it3D = it3DSliceStart;
    for (y = 0; y < m_Input->GetLargestPossibleRegion().GetSize()[1]; y++)
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::CorrectIntensityUsingHistogramMatching(unsigned int numberOfBins,
                                                                     unsigned int numberOfMatchPoints,
                                                                     ImageType *  ref)
{
  typedef itk::HistogramMatchingImageFilter<ImageType, ImageType> HistogramMatchFilterType;
  typename HistogramMatchFilterType::Pointer                      matchFilter;
  matchFilter = HistogramMatchFilterType::New();
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::Resize(double factor)
{
  ...
  if (factor != 0)
  {
    for (unsigned int i = 0; i < VDimension; i++)
    {
      size[i] = (long unsigned int)(m_Input->GetLargestPossibleRegion().GetSize()[i] / factor);
  ...
  else
  {
    ...
    size[0] = (long unsigned int)(m_Input->GetLargestPossibleRegion().GetSize()[0] / factor);
    factor = meanSpacing / spacing[1];
    size[1] = (long unsigned int)(m_Input->GetLargestPossibleRegion().GetSize()[1] / factor);
    ...
    if (VDimension == 3)
    {
      ...
      size[VDimension - 1] =
        (long unsigned int)(m_Input->GetLargestPossibleRegion().GetSize()[VDimension - 1] / factor);
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::Resize(ImageType * ref)
{
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::ExtractSlice(unsigned int dimension, unsigned int slice)
{
  typedef itk::ExtractImageFilter<ImageType, ImageType> ExtractSliceFilterType;

  typename ExtractSliceFilterType::Pointer filter = ExtractSliceFilterType::New();
  ...
  typename ImageType::RegionType desiredRegion(extractIndex, extractSize);
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::EnhanceVessels(double scaleMin, double scaleMax, double numScales)
{
  double logScaleStep = (std::log(scaleMax) - std::log(scaleMin)) / (numScales - 1);
...
template <unsigned int VDimension>
void
ImageMathFilters<VDimension>::SegmentUsingConnectedThreshold(float threshLow,
                                                             float threshHigh,
                                                             float labelValue,
                                                             float x,
                                                             float y,
...
ImageMathFilters<VDimension>::ComputeVoronoiTessellation(unsigned int numberOfCentroids,
