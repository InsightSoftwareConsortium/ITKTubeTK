...
RidgeFFTFilter<TInputImage>::RidgeFFTFilter()
{
  m_Intensity = NULL;
  m_Ridgeness = NULL;
  m_Curvature = NULL;
  ...
  m_DerivativeFilter = FFTGaussianDerivativeIFFTFilter<InputImageType, OutputImageType>::New();
}


template <typename TInputImage>
void
RidgeFFTFilter<TInputImage>::GenerateData()
{
  ...
  if (!m_UseIntensityOnly)
  {
    ...
    ImageRegionIterator<OutputImageType> iterRidge(m_Ridgeness, m_Ridgeness->GetLargestPossibleRegion());
    ImageRegionIterator<OutputImageType> iterRound(m_Roundness, m_Roundness->GetLargestPossibleRegion());
    ImageRegionIterator<OutputImageType> iterCurve(m_Curvature, m_Curvature->GetLargestPossibleRegion());
    ImageRegionIterator<OutputImageType> iterLevel(m_Levelness, m_Levelness->GetLargestPossibleRegion());

    std::vector<ImageRegionIterator<OutputImageType>> iterDx(ImageDimension);
    ...
    {
      iterDx[i] = ImageRegionIterator<OutputImageType>(dx[i], dx[i]->GetLargestPossibleRegion());
      for (unsigned int j = i; j < ImageDimension; ++j)
      {
        iterDdx[count] = ImageRegionIterator<OutputImageType>(ddx[count], ddx[count]->GetLargestPossibleRegion());
        ++count;
      }
    }
    ...
    while (!iterRidge.IsAtEnd())
    {
      ...
      ::tube::ComputeRidgeness(H, D, prevTangent, ridgeness, roundness, curvature, levelness, HEVect, HEVal);
...
template <typename TInputImage>
void
RidgeFFTFilter<TInputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
