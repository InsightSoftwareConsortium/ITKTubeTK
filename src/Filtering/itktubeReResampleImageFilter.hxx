...
#ifndef itktubeReResampleImageFilter_hxx
#define itktubeReResampleImageFilter_hxx
...
ReResampleImageFilter<TPixel, VDimension>::Update(void)
{
  ...
  {
    using SincInterpType = typename itk::WindowedSincInterpolateImageFunction<ImageType, VDimension>;
    interp = SincInterpType::New();
  }
  else if (m_Interpolator == "BSpline")
  {
    using BSplineInterpType = typename itk::BSplineInterpolateImageFunction<ImageType, double>;
    interp = BSplineInterpType::New();
  }
  else if (m_Interpolator == "NearestNeighbor")
  {
    using NearestNeighborInterpType = typename itk::NearestNeighborInterpolateImageFunction<ImageType, double>;
    interp = NearestNeighborInterpType::New();
  }
  else // default = if( interpolator == "Linear" )
  {
    using LinearInterpType = typename itk::LinearInterpolateImageFunction<ImageType, double>;
