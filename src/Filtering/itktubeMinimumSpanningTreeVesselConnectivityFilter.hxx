...
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::MinimumSpanningTreeVesselConnectivityFilter(void)
{
...
template <unsigned int ObjectDimension>
bool
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::GraphEdgeType ::operator>(const GraphEdgeType & rhs) const
{
...
template <unsigned int ObjectDimension>
bool
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::ConnectionPointType ::operator>(
  const ConnectionPointType & rhs) const
{
  if (dist != rhs.dist)
  {
...
template <unsigned int ObjectDimension>
bool
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::TubePQElementType ::operator<(
  const TubePQElementType & rhs) const
{
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::SetRootTubeIdList(const TubeIdListType & rootTubeIdList)
{
  m_RootTubeIdList = rootTubeIdList;
}

template <unsigned int ObjectDimension>
const typename MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::TubeIdListType &
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::GetRootTubeIdList(void) const
{
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::BuildTubeGraph(void)
{
  ...
  char                tubeName[] = "Tube";
  TubeListPointerType pTubeList = inputTubeGroup->GetChildren(inputTubeGroup->GetMaximumDepth(), tubeName);

  m_TubeIdToObjectMap.clear();

  for (typename TubeGroupType::ChildrenListType::iterator itTubes = pTubeList->begin(); itTubes != pTubeList->end();
       ++itTubes)
  {
    m_TubeIdToObjectMap[(*itTubes)->GetId()] = dynamic_cast<TubeType *>(itTubes->GetPointer());
  }
  ...
  for (typename TubeGroupType::ChildrenListType::iterator itSourceTubes = pTubeList->begin();
       itSourceTubes != pTubeList->end();
       ++itSourceTubes)
  {
    TubePointerType   pCurSourceTube = dynamic_cast<TubeType *>(itSourceTubes->GetPointer());
    ...
    for (typename TubePointListType::const_iterator itSourcePoints = sourcePointList.begin();
         itSourcePoints != sourcePointList.end();
         ++itSourcePoints)
    {
      TubePointType      ptSource = *itSourcePoints;
      PositionVectorType ptSourcePos = ptSource.GetPositionInObjectSpace().GetVectorFromOrigin();

      for (typename TubeGroupType::ChildrenListType::iterator itTargetTubes = pTubeList->begin();
           itTargetTubes != pTubeList->end();
           ++itTargetTubes)
      {
        TubePointerType curTargetTube = dynamic_cast<TubeType *>(itTargetTubes->GetPointer());
        ...
        std::priority_queue<ConnectionPointType, std::vector<ConnectionPointType>, std::greater<ConnectionPointType>>
                            minpqConnPoint;
        ...
        for (unsigned int i = 0; i < 2; i++)
        {
          ...
          PositionVectorType ptCurPos = ptCur.GetPositionInObjectSpace().GetVectorFromOrigin();
          ...
          if (curDist > m_MaxTubeDistanceToRadiusRatio * ptSource.GetRadiusInObjectSpace())
          {
            continue;
          }
          ...
          if (curPtId == 0)
          {
            ptNextPos = targetPointList[curPtId + 1].GetPositionInObjectSpace().GetVectorFromOrigin();
          }
          else
          {
            ptNextPos = targetPointList[curPtId - 1].GetPositionInObjectSpace().GetVectorFromOrigin();
        ...
        if (m_TubeGraph[curSourceTubeId].find(curTargetTubeId) != m_TubeGraph[curSourceTubeId].end())
        {
  ...
  // print graph
  tubeDebugMacro(<< "\nNumber of tubes = " << m_TubeIdToObjectMap.size());

  for (typename TubeAdjacencyListGraphType::const_iterator itV = m_TubeGraph.begin(); itV != m_TubeGraph.end(); ++itV)
  {
    tubeDebugMacro(<< "Id = " << itV->first << ", numPoints = " << m_TubeIdToObjectMap[itV->first]->GetNumberOfPoints()
                   << ", Out-degree = " << m_TubeGraph[itV->first].size());

    for (typename GraphEdgeListType::const_iterator itE = itV->second.begin(); itE != itV->second.end(); ++itE)
    {
      GraphEdgeType e = itE->second;
      tubeDebugMacro(<< "  Id = " << e.targetTubeId << ", sourcePointId = " << e.sourceTubePointId
                     << ", targetPointId = " << e.targetTubePointId << ", weight = " << std::setprecision(3) << e.weight
                     << ", distanceToRadiusRatio = " << std::setprecision(3) << e.distToRadRatio
                     << ", continuityAngleError = " << std::setprecision(3) << e.continuityAngleError);
    }
  }
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::VisitTube(TubePointerType pTube)
{
  ...
  for (typename GraphEdgeListType::iterator itRootNeighbors = m_TubeGraph[tubeId].begin();
       itRootNeighbors != m_TubeGraph[tubeId].end();
       ++itRootNeighbors)
  {
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::RunMinimumSpanningTree(TubeIdType rootTubeId)
{
  ...
  {
    bool isActualRoot =
      std::find(m_RootTubeIdList.begin(), m_RootTubeIdList.end(), rootTubeId) != m_RootTubeIdList.end();
    if (!isActualRoot)
    {
      return;
    }
  ...
  itkDebugStatement(unsigned numChildren = 0;);
  ...
  while (!m_minpqGraphEdge.empty())
  {
    ...
    if (m_SetTubesReversed.find(eTop.sourceTubeId) != m_SetTubesReversed.end())
    {
    ...
    // print some info
    tubeDebugMacro(<< "  sourceTubeId = " << eTop.sourceTubeId << ", targetTubeId = " << eTop.targetTubeId
                   << ", reversed points = " << (eTop.targetTubePointId > 0) << std::setprecision(3)
                   << ", weight = " << eTop.weight << std::setprecision(3)
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::ComputeTubeConnectivity(void)
{
  const TubeGroupType * inputTubeGroup = this->GetInput();
  TubeGroupType *       outputTubeGroup = this->GetOutput();
  ...
  if (!m_RootTubeIdList.empty())
  {
    ...
    for (typename TubeIdListType::const_iterator itRootTubeId = m_RootTubeIdList.begin();
         itRootTubeId != m_RootTubeIdList.end();
         ++itRootTubeId)
    {
      // throw exception if current root tube id is not present
      if (m_TubeIdToObjectMap.find(*itRootTubeId) == m_TubeIdToObjectMap.end())
      {
  ...
  tubeDebugMacro(<< "Number of output connected components = " << m_numOutputConnectedComponents);
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::AddRemainingTubes(void)
{
  const TubeGroupType * inputTubeGroup = this->GetInput();
  TubeGroupType *       outputTubeGroup = this->GetOutput();
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::GenerateData(void)
{
  BuildTubeGraph();
...
template <unsigned int ObjectDimension>
void
MinimumSpanningTreeVesselConnectivityFilter<ObjectDimension>::PrintSelf(std::ostream & os, Indent indent) const
{
  this->Superclass::PrintSelf(os, indent);

  // print parameters
  tubeStandardOutputMacro(<< "\nMaxTubeDistanceToRadiusRatio = " << m_MaxTubeDistanceToRadiusRatio);
  tubeStandardOutputMacro(<< "\nMaxContinuityAngleError = " << m_MaxContinuityAngleError);
  tubeStandardOutputMacro(<< "\nRemove Orphan Tubes = " << m_RemoveOrphanTubes);
}
