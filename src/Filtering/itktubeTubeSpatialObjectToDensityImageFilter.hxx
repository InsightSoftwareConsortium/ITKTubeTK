...
/** Constructor */
template <class TDensityImageType, class TRadiusImageType, class TTangentImageType>
TubeSpatialObjectToDensityImageFilter<TDensityImageType, TRadiusImageType, TTangentImageType>::
...
template <class TDensityImageType, class TRadiusImageType, class TTangentImageType>
void
TubeSpatialObjectToDensityImageFilter<TDensityImageType, TRadiusImageType, TTangentImageType>::SetSpacing(SpacingType s)
{
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    m_Spacing[i] = s[i];
...
template <class TDensityImageType, class TRadiusImageType, class TTangentImageType>
void
TubeSpatialObjectToDensityImageFilter<TDensityImageType, TRadiusImageType, TTangentImageType>::Update(void)
{
  if (m_Size[0] == 0)
  {
  ...
  try
  {
    ...
    typedef TubeSpatialObjectToImageFilter<ImageDimension, DensityImageType, RadiusImageType, TangentImageType>
                                 FilterType;
    ...
    typename DanielssonFilterType::Pointer danFilter = DanielssonFilterType::New();
    ...
    VectorIteratorType it_vector(vectorImage, vectorImage->GetLargestPossibleRegion());
    RadiusIteratorType it_radius(m_RadiusMapImage, m_RadiusMapImage->GetLargestPossibleRegion());
    ...
    TangentIteratorType it_tangent(m_TangentMapImage, m_TangentMapImage->GetLargestPossibleRegion());
    ...
    typename InverseIntensityImageFilter<DensityImageType>::Pointer InverseFilter;
  ...
  {
    std::cerr << "\n Error caught in TubeSpatialObjectToDensityImageFilter Class" << std::endl;
