...
public:
  ...
  void
  SetInput(ImageType * input)
  {
    typename ImageType::Pointer tmpImage = ImageType::New();
    tmpImage = input;
    typename itk::ImageDuplicator<ImageType>::Pointer dupFilter = itk::ImageDuplicator<ImageType>::New();
  ...
  void
  ApplyIntensityWindowing(float inValMin, float inValMax, float outMin, float outMax);

  /** Intensity multiplicative correct using inMeanField. */
  void
  ApplyIntensityMultiplicativeBiasCorrection(ImageType * inMeanFieldImage);
  ...
  void
  AddUniformNoise(float valMin, float valMax, float noiseMean, float noiseRange, int seed);

  /** Adds Gaussian noise to all pixels within inVal range */
  void
  AddGaussianNoise(float valMin, float valMax, float noiseMean, float noiseStdDev, int seed);

  /** I( x ) = weight1*I( x ) + weight2*inFile2( x ) */
  void
  AddImages(ImageType * input2, float weight1, float weight2);
  ...
  void
  MedianImage(int size);

  /** If I( x ) in [tLow,tHigh] then I( x )=vTrue else I( x )=vFalse. */
  void
  ThresholdImage(float threshLow, float threshHigh, float valTrue, float valFalse);

  /** Return image value within masked region ( mode: 0=mean, 1=stdDev ) */
  double
  ComputeImageStatisticsWithinMaskRange(ImageType * mask, float threshLow, float threshHigh, int mode);
  ...
  void
  ReplaceValuesOutsideMaskRange(ImageType * mask, float maskThreshLow, float maskThreshHigh, float valFalse);

  /** Mathematical morphology using a sphere. Mode: 0=erode, 1=dilate. */
  void
  MorphImage(int mode, int radius, float foregroundValue, float backgroundValue);

  /** Replace values within the image, with a mask. */
  void
  ReplaceValueWithinMaskRange(ImageType * mask,
                              float       maskThreshLow,
  ...
  void
  CorrectIntensitySliceBySliceUsingHistogramMatching(unsigned int numberOfBins, unsigned int numberOfMatchPoints);

  /** Match intensity to another volume using HistogramMatchingFilter. */
  void
  CorrectIntensityUsingHistogramMatching(unsigned int numberOfBins, unsigned int numberOfMatchPoints, ImageType * ref);
  ...
  void
  SegmentUsingConnectedThreshold(float threshLow, float threshHigh, float labelValue, float x, float y, float z);
  ...
  GetVoronoiTessellationAdjacencyMatrix()
  {
