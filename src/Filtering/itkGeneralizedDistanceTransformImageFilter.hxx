...
void
updateDivisionTable(std::vector<TSpacingType> & table, TSpacingType spacing, size_t size)
{
...
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::SetCreateVoronoiMap(bool b)
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::CreateVoronoiMapOn()
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::CreateVoronoiMapOff()
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
double
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::GetMaximalSquaredDistance() const
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::SetInput1(
  const FunctionImageType * functionImage)
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::SetInput2(
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
typename GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::DistanceImageType *
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::GetDistance(void)
{
  return dynamic_cast<DistanceImageType *>(this->ProcessObject::GetOutput(0));
}

template <class TFunctionImage, class TDistanceImage, class TLabelImage>
typename GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::LabelImageType *
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::GetVoronoiMap()
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::
  GeneralizedDistanceTransformImageFilter()
  : m_MaximalSquaredDistance(std::numeric_limits<typename TDistanceImage::PixelType>::max() / 2)
  , m_CacheLineSize(128)
{
  ...
  assert(std::numeric_limits<SpacingType>::max() >= std::numeric_limits<DistancePixelType>::max());
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::PrintSelf(std::ostream & os,
                                                                                                Indent indent) const
{
  Superclass::PrintSelf(os, indent);
  os << indent << "Generalized Distance Transform: " << std::endl;
  os << indent << "MaximalSquaredDistance: " << m_MaximalSquaredDistance << std::endl;
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::EnlargeOutputRequestedRegion(
  DataObject *)
{
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::PrepareData()
{
  // Copy the function image into the distance image
  FunctionImageConstPointer functionImage = dynamic_cast<FunctionImageType *>(ProcessObject::GetInput(0));
  ...
  ImageRegionConstIterator<FunctionImageType> functionIt(functionImage, distance->GetRequestedRegion());
  ImageRegionIterator<DistanceImageType>      distanceIt(distance, distance->GetRequestedRegion());
  ...
  while (!distanceIt.IsAtEnd())
  {
    ...
    distanceIt.Value() = static_cast<typename DistanceImageType::PixelType>(functionIt.Get());
  ...
  if (this->m_CreateVoronoiMap)
  {
    ...
    LabelImagePointer labelImage = dynamic_cast<LabelImageType *>(ProcessObject::GetInput(1));
    ...
    ImageRegionConstIterator<LabelImageType> labelIt(labelImage, distance->GetRequestedRegion());
    ImageRegionIterator<LabelImageType>      voronoiIt(voronoiMap, distance->GetRequestedRegion());
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::GenerateData()
{
  this->PrepareData();
  ...
  if (this->m_CreateVoronoiMap)
  {
    LabelImagePointer voronoiMap = dynamic_cast<LabelImageType *>(this->ProcessObject::GetOutput(1));
  ...
  maxSize = (DistSizeValueType)(std::ceil((double)maxSize / m_CacheLineSize) * m_CacheLineSize);
  ...
  while (!distanceIt.IsAtEnd())
  {
    ...
    LabelPixelType *    rawLabel = this->m_CreateVoronoiMap ? &voronoiMapIt.Value() : 0;
    for (size_t i = 0; i < size[1]; ++i)
    {
      ...
      if (this->m_UseImageSpacing)
      {
        ...
        {
          addParabola(envelope,
                      size[0],
                      j,
                      *(rawDistance + offset),
                      this->m_CreateVoronoiMap ? *(rawLabel + offset) : typename LabelImageType::PixelType(),
                      divisionTable);
        }
        offset -= size[0];
      }
  ...
  const DistSizeValueType pixelsInCacheLine = std::max(
    (DistSizeValueType)1, (DistSizeValueType)(m_CacheLineSize / sizeof(typename DistanceImageType::PixelType)));
  ...
  // for (int d = 1; d < FunctionImageType::ImageDimension; ++d)
  {
    ...
    const int strips = static_cast<int>(std::ceil(double(size[0]) / pixelsInCacheLine));
    ...
    while (!distanceIt.IsAtEnd())
    {
      ...
      for (int i = 0; i < strips; ++i)
      {
        ...
        const DistSizeValueType parallelLines = std::min(pixelsInCacheLine, size[0] - i * pixelsInCacheLine);
        ...
        for (size_t j = 0, offset = currentStripOffset; j < size[d]; ++j, offset += strideD - parallelLines)
        {
          if (this->m_UseImageSpacing)
          {
        ...
        {
          for (size_t line = 0, offset = 0; line < parallelLines; ++line, offset += maxSize)
          {
            lineNeedsCopy[line] = sampleValues(envelopeD[line], 0, size[d], &valuesBuffer[offset], sqrsD);
        ...
        else
        {
          for (size_t line = 0, offset = 0; line < parallelLines; ++line, offset += maxSize)
          {
            lineNeedsCopy[line] = sampleValues(envelopeD[line], 0, size[d], &valuesBuffer[offset]);
            stripNeedsCopy &= lineNeedsCopy[line];
          }
        }
        ...
        if (!stripNeedsCopy)
          continue;
        for (size_t j = 0, outoffset = currentStripOffset; j < size[d]; ++j, outoffset += strideD - parallelLines)
        {
          for (size_t line = 0, inoffset = j; line < parallelLines; ++line,
        ...
        if (this->m_CreateVoronoiMap)
        {
          ...
          for (size_t line = 0, offset = 0; line < parallelLines; ++line, offset += maxSize)
          {
            if (lineNeedsCopy[line])
            {
              sampleVoronoi(envelopeD[line], 0, size[d], &voronoiBuffer[offset]);
            }
          }

          for (size_t j = 0, outoffset = currentStripOffset; j < size[d]; ++j, outoffset += strideD - parallelLines)
          {
            for (size_t line = 0, inoffset = j; line < parallelLines; ++line, inoffset += maxSize, ++outoffset)
            {
              if (lineNeedsCopy[line])
              {
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
typename GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::AbscissaIndexType
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::intersection(
  const Parabola &                 p,
  const Parabola &                 q,
  ...
  const AbscissaIndexType &        to)
{
  ...
  const AbscissaIndexType i(
    (static_cast<SpacingType>(q.i + p.i) + divisionTable[q.i - p.i] * static_cast<SpacingType>(q.y - p.y)) * 0.5 + 1);
  if (i < from)
  {
    return from;
  }
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
void
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::addParabola(
  Parabolas &                      envelope,
  const AbscissaIndexType &        to,
  const AbscissaIndexType &        pi,
  const DistancePixelType &        py,
  ...
  const std::vector<SpacingType> & divisionTable)
{
  ...
  while (!envelope.empty() && (i = intersection(envelope.back(), p, divisionTable, envelope.back().dominantFrom, to)) <=
                                envelope.back().dominantFrom)
  {
  ...
  const LabelPixelType &    pl)
{
  ...
  while (!envelope.empty() &&
         (i = intersection(envelope.back(), p, envelope.back().dominantFrom, to)) <= envelope.back().dominantFrom)
  {
    envelope.pop_back();
  }
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
bool
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::sampleValues(
  ...
  const SpacingType &       sqrs)
{
  ...
  while (parabolaIndex != envelope.size() - 1 && envelope[parabolaIndex + 1].dominantFrom <= from)
  {
    ++parabolaIndex;
  }
  ...
  assert(pow(SpacingType(10), std::numeric_limits<SpacingType>::digits10) >= steps);
  ...
  for (AbscissaIndexType i = from; i != from + steps; ++parabolaIndex) // i is increased within the loop
  {
    ...
    if (parabolaIndex < envelope.size() - 1 && envelope[parabolaIndex + 1].dominantFrom <= from + steps)
    {
  ...
  DistancePixelType *       buffer)
{
  ...
  for (AbscissaIndexType i = from; i != from + steps; ++parabolaIndex)
  {
    const Parabola &  p = envelope[parabolaIndex];
    AbscissaIndexType stepsInRegion =
      (parabolaIndex < envelope.size() - 1 && envelope[parabolaIndex + 1].dominantFrom <= from + steps)
...
template <class TFunctionImage, class TDistanceImage, class TLabelImage>
bool
GeneralizedDistanceTransformImageFilter<TFunctionImage, TDistanceImage, TLabelImage>::sampleVoronoi(
  const Parabolas &         envelope,
  const AbscissaIndexType & from,
  const long &              steps,
  LabelPixelType *          buffer)
{
  ...
  while (i != from + steps)
  {
    ...
    ++parabolaIndex;
    while (parabolaIndex < envelope.size() - 1 && envelope[parabolaIndex + 1].l == envelope[parabolaIndex].l)
    {
      ++parabolaIndex;
    }
  }
