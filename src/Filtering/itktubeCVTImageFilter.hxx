...
CVTImageFilter<TInputImage, TOutputImage>::CVTImageFilter(void)
{
  m_Seed = -1;
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::SetCentroids(const PointArrayType & centroids)
{
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::GenerateInputRequestedRegion(void)
{
  Superclass::GenerateInputRequestedRegion();
  if (this->GetInput())
  {
    typename InputImageType::Pointer inpt = const_cast<TInputImage *>(this->GetInput());
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::EnlargeOutputRequestedRegion(DataObject * output)
{
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::GenerateData(void)
{
  ...
  {
    this->ComputeSample(&m_Centroids, m_NumberOfCentroids, m_InitialSamplingMethod);
  }
  ...
  if (this->GetDebug())
  {
    ...
    {
      std::cout << "Initial Centroid [" << j << "] = " << m_Centroids[j] << std::endl;
    }
  }
  ...
  while (iteration < m_NumberOfIterations)
  {
    ...
    if (this->GetDebug())
    {
      std::cout << "Iteration = " << iteration << " : E = " << iterationEnergy
  ...
  for (int j = 0; j < (int)m_NumberOfCentroids; j++)
  {
    ...
    if (this->GetDebug())
    {
      std::cout << " Final Centroid [" << j << "] = " << m_Centroids[j] << std::endl;
    }
  }

  typedef DanielssonDistanceMapImageFilter<OutputImageType, OutputImageType> DDFilterType;
...
template <class TInputImage, class TOutputImage>
double
CVTImageFilter<TInputImage, TOutputImage>::ComputeIteration(double & energyDiff)
{
  int i;
  int j;
  int j2;
  ...
  while (have < (int)m_NumberOfSamples)
  {
    ...
    for (j = 0; j < get; j++)
    {
      ...
      for (i = 0; i < (int)ImageDimension; i++)
      {
        ...
        dist = (m_Centroids[j2][i] - batch[j][i]) * (m_Centroids[j2][i] - batch[j][i]);
      }
  ...
  for (j = 0; j < (int)m_NumberOfCentroids; j++)
  {
    ...
    for (i = 0; i < (int)ImageDimension; i++)
    {
      term += (centroids2[j][i] - m_Centroids[j][i]) * (centroids2[j][i] - m_Centroids[j][i]);
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::ComputeSample(PointArrayType *   sample,
                                                         ...
                                                         SamplingMethodEnum samplingMethod)
{
  if (sampleSize < 1)
  {
    if (this->GetDebug())
    {
      std::cout << "ComputeSample: Fatal error! SampleSize < 1." << std::endl;
    }
  ...
  switch (samplingMethod)
  {
    case CVT_GRID:
    {
      ...
      for (unsigned int j = len; j < sampleSize; j++)
      {
        for (unsigned int i = 0; i < ImageDimension; i++)
        {
          iIndx[i] = (int)(m_RandomGenerator->GetUniformVariate(0, 1) * m_InputImageSize[i] - 1);
        }
        (*sample).push_back(iIndx);
      }
      break;
    ...
    case CVT_RANDOM:
    {
      for (unsigned int j = 0; j < sampleSize; j++)
      {
        ...
        while (u >= p1)
        {
          for (unsigned int i = 0; i < ImageDimension; i++)
          {
            indx[i] = (int)(m_RandomGenerator->GetUniformVariate(0, 1) * m_InputImageSize[i] - 1);
            iIndx[i] = (int)(indx[i]);
          }
    ...
    case CVT_USER:
    {
      if (this->GetDebug())
      {
        std::cout << "Sampling method CVT_USER not supported for resampling" << std::endl;
      }
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::ComputeClosest(const PointArrayType & sample,
                                                          ...
                                                          unsigned int *         nearest)
{
  ...
  for (unsigned int js = 0; js < numberOfSamples; js++)
  {
    ...
    for (unsigned int jc = 0; jc < numberOfCentroids; jc++)
    {
      ...
      for (unsigned int i = 0; i < ImageDimension; i++)
      {
        dist += (sample[js][i] - centroids[jc][i]) * (sample[js][i] - centroids[jc][i]);
      }
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::ComputeAdjacencyMatrix(void)
{
  m_AdjacencyMatrix.SetSize(m_NumberOfCentroids, m_NumberOfCentroids);
  m_AdjacencyMatrix.Fill(0);

  typename OutputImageType::SizeType size = m_OutputImage->GetLargestPossibleRegion().GetSize();
  ...
  while (!done)
  {
    ...
    while (!done && indx[i] >= (int)size[i])
    {
      ...
      else
      {
        if (i == (int)(ImageDimension)-1)
        {
          std::cout << "Computing adjacency of slice : " << indx[ImageDimension - 1] << std::endl;
        }
        indx[i]++;
      }
    }
...
template <class TInputImage, class TOutputImage>
void
CVTImageFilter<TInputImage, TOutputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  std::cout << "InitialSamplingMethod = " << m_InitialSamplingMethod << std::endl;
  ...
  std::cout << "NumberOfIterationsPerBatch = " << m_NumberOfIterationsPerBatch << std::endl;
  std::cout << "NumberOfSamplesPerBatch = " << m_NumberOfSamplesPerBatch << std::endl;
}
