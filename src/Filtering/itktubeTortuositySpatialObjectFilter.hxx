...
#ifndef itktubeTortuositySpatialObjectFilter_hxx
#define itktubeTortuositySpatialObjectFilter_hxx
...
TortuositySpatialObjectFilter<TTubeSpatialObject>::TortuositySpatialObjectFilter(void)
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetAverageRadiusMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetChordLengthMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetDistanceMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetInflectionCountMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetInflectionCount1Metric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetInflectionCount2Metric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetPathLengthMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetPercentile95Metric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetSumOfAnglesMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetSumOfTorsionMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetTotalCurvatureMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetTotalSquaredCurvatureMetric() const
{
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetCurvatureScalarMetric(unsigned int i) const
{
  ...
  {
    std::cerr << "GetCurvatureScalarMetric( int ): Index " << i << " out of bounds" << std::endl;
  }
  else
  {
...
template <class TTubeSpatialObject>
double
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetInflectionPointValue(unsigned int i) const
{
  ...
  {
    std::cerr << "GetInflectionPointValue( int ): Index " << i << " out of bounds" << std::endl;
  }
  else
  {
...
template <class TTubeSpatialObject>
int
TortuositySpatialObjectFilter<TTubeSpatialObject>::GetCurvatureHistogramMetric(unsigned int bin) const
{
  ...
  {
    std::cerr << "GetHistogramMetric( int ): Index " << bin << " out of bounds" << std::endl;
    return 0;
  }
  else
  {
...
template <class TTubeSpatialObject>
void
TortuositySpatialObjectFilter<TTubeSpatialObject>::GenerateData(void)
{
  // Get I/O
  ...
  {
    SOVectorType currentPoint = processedInput->GetPoint(index)->GetPositionInObjectSpace().GetVectorFromOrigin();
    ...
    {
      nextPoint = processedInput->GetPoint(index + 1)->GetPositionInObjectSpace().GetVectorFromOrigin();
    }
    bool         previousPointAvailable = (index > 0);
    SOVectorType previousPoint(0.0);
    if (previousPointAvailable)
    {
      previousPoint = processedInput->GetPoint(index - 1)->GetPositionInObjectSpace().GetVectorFromOrigin();
    }
    ...
    {
      nPlus2Point = processedInput->GetPoint(index + 2)->GetPositionInObjectSpace().GetVectorFromOrigin();
    }
    ...
    if ((soam || sot) && previousPointAvailable && nextPointAvailable && nPlus2PointAvailable)
    {
      ...
      if (SafeNormalize(normT1) > this->m_EpsilonForZero && SafeNormalize(normT2) > this->m_EpsilonForZero)
      {
      ...
      if (SafeNormalize(t1t2Cross) > this->m_EpsilonForZero && SafeNormalize(t2t3Cross) > this->m_EpsilonForZero)
      {
  ...
  if (soam || sot)
  {
    ...
    {
      itkExceptionMacro(<< "Cannot compute angle metrics, path length ( =" << pathLength << " ) <= 0.0");
    }
  }
  ...
  if (p95m || chm || ic1m || ic2m)
  {
    ...
    using HistogramType = itk::Statistics::Histogram<double, itk::Statistics::DenseFrequencyContainer2>;
    ...
    if (ic1m || ic2m)
    {
      ...
      {
        if (ic1m && (this->m_CurvatureScalar[i - 1] - threshold1) * (this->m_CurvatureScalar[i] - threshold1) < 0)
        {
          ++inflectionCount1;
        }
        if (ic2m && (this->m_CurvatureScalar[i - 1] - threshold2) * (this->m_CurvatureScalar[i] - threshold2) < 0)
        {
          ++inflectionCount2;
        }
      }
    ...
    if (chm)
    {
      ...
      SampleToHistogramFilterType::Pointer sampleToHistogramFilter2 = SampleToHistogramFilterType::New();
      ...
      else
      {
        ...
        {
          this->m_CurvatureHistogramMetrics.push_back(histogramForFeatures->GetFrequency(i));
        }
      }
    }
  }
