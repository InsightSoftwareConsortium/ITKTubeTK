...
CropImageFilter<TInputImage, TOutputImage>::CropImageFilter(void)
{
  // itkCropImageFilter
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetMin(typename ImageType::IndexType roiMin)
{
  m_ROIMin = roiMin;
  m_UseROIMin = true;
}
...
CropImageFilter<TInputImage, TOutputImage>::GetMin(void)
{
  return m_ROIMin;
}
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetMax(typename ImageType::IndexType roiMax)
{
  m_ROIMax = roiMax;
  m_UseROIMax = true;
}
...
template <typename TInputImage, typename TOutputImage>
typename TInputImage::IndexType
CropImageFilter<TInputImage, TOutputImage>::GetMax(void)
{
  return m_ROIMax;
}
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetSize(typename ImageType::SizeType roiSize)
{
  m_ROISize = roiSize;
  m_UseROISize = true;
}
...
CropImageFilter<TInputImage, TOutputImage>::GetSize(void)
{
  return m_ROISize;
}
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetCenter(typename ImageType::IndexType roiCenter)
{
...
template <typename TInputImage, typename TOutputImage>
typename TInputImage::IndexType
CropImageFilter<TInputImage, TOutputImage>::GetCenter(void)
{
  return m_ROICenter;
}
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetBoundary(typename ImageType::IndexType roiBoundary)
{
...
template <typename TInputImage, typename TOutputImage>
typename TInputImage::IndexType
CropImageFilter<TInputImage, TOutputImage>::GetBoundary(void)
{
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetMatchVolume(const ImageType * matchVolume)
{
  const typename ImageType::RegionType  matchRegion = matchVolume->GetLargestPossibleRegion();
  const typename ImageType::IndexType   matchIndex = matchRegion.GetIndex();
  const typename ImageType::SizeType    matchSize = matchRegion.GetSize();
  const typename ImageType::PointType   matchOrigin = matchVolume->GetOrigin();
  const typename ImageType::SpacingType matchSpacing = matchVolume->GetSpacing();

  const TInputImage *                   image = this->GetInput();
  const typename ImageType::RegionType  imgRegion = image->GetLargestPossibleRegion();
  const typename ImageType::IndexType   imgIndex = imgRegion.GetIndex();
  ...
  if (imgOrigin != matchOrigin || imgSpacing != matchSpacing)
  {
    ...
    {
      minI[i] = itk::Math::rnd(
        ((matchOrigin[i] + matchIndex[i] * matchSpacing[i]) - (imgOrigin[i] + imgIndex[i] * imgSpacing[i])) /
        imgSpacing[i]);
      sizeI[i] = itk::Math::rnd((matchSize[i] * matchSpacing[i]) / imgSpacing[i]);
    }
  }
  else
  {
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetMatchMask(const ImageType * maskImage)
{
  typename ImageType::IndexType minI;
  typename ImageType::IndexType maxI;

  itk::ImageRegionConstIterator<ImageType> it(maskImage, maskImage->GetLargestPossibleRegion());
...
template <typename TInputImage, typename TOutputImage>
void
CropImageFilter<TInputImage, TOutputImage>::SetSplitInput(typename ImageType::IndexType splitIndex,
                                                          typename ImageType::IndexType roiIndex)
{
  typename ImageType::SizeType inputImageSize = this->GetInput()->GetLargestPossibleRegion().GetSize();
