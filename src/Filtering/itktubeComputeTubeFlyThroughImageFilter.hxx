...
#ifndef itktubeComputeTubeFlyThroughImageFilter_hxx
#define itktubeComputeTubeFlyThroughImageFilter_hxx
...
ComputeTubeFlyThroughImageFilter<TPixel, Dimension>::ComputeTubeFlyThroughImageFilter(void)
{
  m_OutputMask = NULL;
}
...
template <class TPixel, unsigned int Dimension>
void
ComputeTubeFlyThroughImageFilter<TPixel, Dimension>::PrintSelf(std::ostream & os, Indent indent) const
{
...
template <class TPixel, unsigned int Dimension>
void
ComputeTubeFlyThroughImageFilter<TPixel, Dimension>::GenerateData(void)
{
  ...
  TubeListType * tubeList = inputTubeGroup->GetChildren(inputTubeGroup->GetMaximumDepth(), tubeName);
  ...
  for (unsigned int i = 0; i < Dimension - 1; i++)
  {
    size[i] = 2 * (typename OutputImageType::SizeValueType)(0.5 + (maxTubeRadius / outputSpacing[i])) + 1;
  }
  ...
  using OutputImageIteratorType = ImageRegionIteratorWithIndex<OutputImageType>;
  ...
  for (itPts = tubePointList.begin(); itPts != tubePointList.end(); itPts++, ptInd++)
  {
    ...
    typename TubeType::PointType curTubePosition = itPts->GetPositionInWorldSpace();
    ...
    for (itOutSlice.GoToBegin(), itMask.GoToBegin(); !itOutSlice.IsAtEnd(); ++itOutSlice, ++itMask)
    {
