...
TubeSpatialObjectToTubeGraphFilter<TPixel, Dimension>::TubeSpatialObjectToTubeGraphFilter(void)
{
  m_CVTImage = NULL;
}
...
template <class TPixel, unsigned int Dimension>
vnl_matrix<double>
TubeSpatialObjectToTubeGraphFilter<TPixel, Dimension>::GetAdjacencyMatrix(void)
{
...
template <class TPixel, unsigned int Dimension>
vnl_vector<int>
TubeSpatialObjectToTubeGraphFilter<TPixel, Dimension>::GetRootNodes(void)
{
  return m_RootNodes;
}
...
template <class TPixel, unsigned int Dimension>
vnl_vector<double>
TubeSpatialObjectToTubeGraphFilter<TPixel, Dimension>::GetBranchNodes(void)
{
...
template <class TPixel, unsigned int Dimension>
void
TubeSpatialObjectToTubeGraphFilter<TPixel, Dimension>::Update(void)
{
  ...
  typename TubeSpatialObjectType::ChildrenListType * tubeList =
    m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), tubeName);
  typename TubeSpatialObjectType::ChildrenListType::const_iterator tubeIt = tubeList->begin();
  ...
  {
    typename TubeSpatialObjectType::Pointer tube = dynamic_cast<TubeSpatialObjectType *>((*tubeIt).GetPointer());
    ...
    for (int p = 1; p < numberOfPoints; p++)
    {
      ...
      else
      {
        ...
        if (graph->GetPoints().size() > 3 && graph->GetPoints().at(len - 1)->m_GraphNode == tNode &&
            graph->GetPoints().at(len - 2)->m_GraphNode == cNode)
        {
          itkDebugMacro(<< " ");
          itkDebugMacro(<< "Oscillation detected"
                        << " : tube = " << cNode << " : seq = " << graph->GetPoints().at(len - 3)->m_GraphNode << " "
                        << graph->GetPoints().at(len - 2)->m_GraphNode << " "
                        << graph->GetPoints().at(len - 1)->m_GraphNode << " " << cNode << " " << tNode);
        ...
        else
        {
          ...
          m_AdjacencyMatrix[cNode - 1][tNode - 1] = m_AdjacencyMatrix[cNode - 1][tNode - 1] + 1;
...
template <class TPixel, unsigned int Dimension>
void
TubeSpatialObjectToTubeGraphFilter<TPixel, Dimension>::PrintSelf(std::ostream & os, Indent indent) const
{
