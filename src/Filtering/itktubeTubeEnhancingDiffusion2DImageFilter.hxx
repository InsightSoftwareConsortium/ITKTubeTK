...
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::TubeEnhancingDiffusion2DImageFilter(void)
  : m_TimeStep(0.2)
  , m_Iterations(200)
...
template <class TPixel, unsigned int TDimension>
void
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::PrintSelf(std::ostream & os, Indent indent) const
{
...
template <class TPixel, unsigned int TDimension>
void
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::VED2DSingleIteration(typename PrecisionImageType::Pointer ci)
{
  bool rec(false);
  if ((m_CurrentIteration == 1) || (m_RecalculateTubeness == 0) || (m_CurrentIteration % m_RecalculateTubeness == 0))
  {
    rec = true;
    if (m_Verbose)
    {
  ...
  typedef typename NeighborhoodAlgorithm::ImageBoundaryFacesCalculator<PrecisionImageType> FT;
  ...
  for (fitci = fci.begin(), fitxx = fxx.begin(), fitxy = fxy.begin(), fityy = fyy.begin(); fitci != fci.end();
       ++fitci, ++fitxx, ++fitxy, ++fityy)
  {
    ...
    for (itci.GoToBegin(), dit.GoToBegin(), itxx.GoToBegin(), itxy.GoToBegin(), ityy.GoToBegin(); !itci.IsAtEnd();
...
template <class TPixel, unsigned int TDimension>
void
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::MaxTubeResponse(const typename PrecisionImageType::Pointer im)
{
  ...
  for (unsigned int i = 0; i < m_Scales.size(); ++i)
  {
    ...
    for (itxx.GoToBegin(), itxy.GoToBegin(), ityy.GoToBegin(), vit.GoToBegin(), hit.GoToBegin(); !vit.IsAtEnd();
...
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::TubenessFunction2D(const Precision l1, const Precision l2)
{
  if (l2 < 0)
  {
    return 0;
...
template <class TPixel, unsigned int TDimension>
void
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::DiffusionTensor(void)
{
  ImageRegionIterator<PrecisionImageType> itxx(m_Dxx, m_Dxx->GetLargestPossibleRegion());
  ImageRegionIterator<PrecisionImageType> itxy(m_Dxy, m_Dxy->GetLargestPossibleRegion());
  ImageRegionIterator<PrecisionImageType> ityy(m_Dyy, m_Dyy->GetLargestPossibleRegion());

  for (itxx.GoToBegin(), itxy.GoToBegin(), ityy.GoToBegin(); !itxx.IsAtEnd(); ++itxx, ++itxy, ++ityy)
  {
    vnl_matrix<Precision> H(2, 2);
    H(0, 0) = itxx.Value();
...
template <class TPixel, unsigned int TDimension>
void
TubeEnhancingDiffusion2DImageFilter<TPixel, TDimension>::GenerateData(void)
{
  if (m_Verbose)
  {
    std::cout << std::endl << "begin vesselenhancingdiffusion2Dimagefilter ... " << std::endl;
  }
  ...
  const Precision htmax = 0.5 / (1.0 / (ispacing[0] * ispacing[0]) + 1.0 / (ispacing[1] * ispacing[1]));
