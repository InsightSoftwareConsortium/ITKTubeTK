...
#ifndef itktubeTubeSpatialObjectToImageFilter_hxx
#define itktubeTubeSpatialObjectToImageFilter_hxx
...
/** Constructor */
template <unsigned int ObjectDimension, class TOutputImage, class TRadiusImage, class TTangentImage>
TubeSpatialObjectToImageFilter<ObjectDimension, TOutputImage, TRadiusImage, TTangentImage>::
...
template <unsigned int ObjectDimension, class TOutputImage, class TRadiusImage, class TTangentImage>
typename TubeSpatialObjectToImageFilter<ObjectDimension, TOutputImage, TRadiusImage, TTangentImage>::RadiusImagePointer
TubeSpatialObjectToImageFilter<ObjectDimension, TOutputImage, TRadiusImage, TTangentImage>::GetRadiusImage(void)
{
...
template <unsigned int ObjectDimension, class TOutputImage, class TRadiusImage, class TTangentImage>
typename TubeSpatialObjectToImageFilter<ObjectDimension, TOutputImage, TRadiusImage, TTangentImage>::TangentImagePointer
TubeSpatialObjectToImageFilter<ObjectDimension, TOutputImage, TRadiusImage, TTangentImage>::GetTangentImage(void)
{
...
template <unsigned int ObjectDimension, class TOutputImage, class TRadiusImage, class TTangentImage>
void
TubeSpatialObjectToImageFilter<ObjectDimension, TOutputImage, TRadiusImage, TTangentImage>::GenerateData(void)
{
  ...
  if (this->m_Size[0] == 0)
  {
    std::cout << "WARNING: itktubeTubeSpacialObjectToImageFilter: Size not set." << std::endl;
    std::cout << "   Reverting to an incorrect method to compute region." << std::endl;
    ...
    for (unsigned int i = 0; i < ObjectDimension; i++)
    {
      ...
      size[i] = (long unsigned int)(physicalSize[i] / this->m_Spacing[i]) + buffer;
    }
    region.SetSize(size);
  }
  else
...
#  endif
  ...
  char               tubeName[] = "Tube";
  ChildrenListType * tubeList = InputTube->GetChildren(this->m_ChildrenDepth, tubeName);
  ...
  while (TubeIterator != tubeList->end())
  {
    ...
    for (unsigned int k = 0; k < tube->GetNumberOfPoints(); k++)
    {
      ...
      const TubePointType * tubePoint = static_cast<const TubePointType *>(tube->GetPoint(k));
      bool                  isInside =
        OutputImage->TransformPhysicalPointToContinuousIndex(tubePoint->GetPositionInWorldSpace(), pointI);
      if (isInside)
      {
        for (unsigned int i = 0; i < ObjectDimension; i++)
        {
              ...
              for (double y = -radiusI[1]; y <= radiusI[1]; y += 0.5)
              {
                ...
                // test  inside the sphere
                {
                  ...
                  if (OutputImage->GetLargestPossibleRegion().IsInside(index2))
                  {
                    if (m_Cumulative)
                    {
                      OutputImage->SetPixel(index2, (PixelType)(OutputImage->GetPixel(index2) + val));
                    }
                    else
                    {
