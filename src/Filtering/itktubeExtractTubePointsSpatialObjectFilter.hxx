...
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::ExtractTubePointsSpatialObjectFilter(void)
{
...
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::MakeOutput(
  ProcessObject::DataObjectPointerArraySizeType itkNotUsed(idx))
{
  ...
  this->ProcessObject::SetNthOutput(0, m_PointsContainerDecorator.GetPointer());
...
template <class TTubeSpatialObject>
void
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::SetInput(const GroupSpatialObjectType * group)
{
  // ProcessObject is not const-correct so a const_cast is required here.
  this->ProcessObject::SetNthInput(0, const_cast<GroupSpatialObjectType *>(group));
}


template <class TTubeSpatialObject>
const typename ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::GroupSpatialObjectType *
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::GetInput(void) const
{
...
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::GetPointsContainerOutput(void)
{
  return this->m_PointsContainerDecorator.GetPointer();
}
...
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::GetPointsContainerOutput(void) const
{
  return this->m_PointsContainerDecorator.GetPointer();
}
...
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::GetPointsContainer(void) const
{
...
template <class TTubeSpatialObject>
void
ExtractTubePointsSpatialObjectFilter<TTubeSpatialObject>::GenerateData(void)
{
  ...
  ChildrenListType * childrenList = inputGroup->GetChildren(inputGroup->GetMaximumDepth(), childName);

  for (typename ChildrenListType::const_iterator childrenIt = childrenList->begin(); childrenIt != childrenList->end();
       ++childrenIt)
  {
    typename TubeSpatialObjectType::Pointer tube;
    tube = dynamic_cast<TubeSpatialObjectType *>(childrenIt->GetPointer());
    if (tube.IsNotNull())
    {
      tube->RemoveDuplicatePointsInObjectSpace();
      tube->ComputeTangentsAndNormals();
      const typename TubeSpatialObjectType::TubePointListType pointsForThisTube = tube->GetPoints();
      unsigned int                                            count = m_PointsContainer->Size();
      m_PointsContainer->Reserve(count + pointsForThisTube.size());
