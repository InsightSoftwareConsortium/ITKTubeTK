...
TubeMathFilters<DimensionT, ImagePixelT>::TubeMathFilters()
{
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetInputTubeGroup(TubeGroupType * inputTubeGroup)
{
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetInputTube(TubeType * inputTube)
{
...
TubeMathFilters<DimensionT, ImagePixelT>::GetOutputTubeGroup(void)
{
...
TubeMathFilters<DimensionT, ImagePixelT>::GetOutputTube(void)
{
  return m_InputTube;
}
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetCurrentTubeId(int currentTubeId)
{
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetUseAllTubes()
{
  m_CurrentTubeId = -1;
}
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetPointValues(std::string propertyId, double val, double blend)
{
  typename TubeType::ChildrenListType::iterator tubeIterator;
  typename TubeType::ChildrenListPointer        inputTubeList =
    m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), "Tube");
  for (tubeIterator = inputTubeList->begin(); tubeIterator != inputTubeList->end(); tubeIterator++)
  {
    ...
    if (m_CurrentTubeId == -1 || inputTube->GetId() == m_CurrentTubeId)
    {
      ...
      {
        TubePointType * currentPoint = static_cast<TubePointType *>(inputTube->GetPoint(pointNum));
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetPointValuesFromImage(
  ...
  for (tubeIterator = inputTubeList->begin(); tubeIterator != inputTubeList->end(); tubeIterator++)
  {
    ...
    if (m_CurrentTubeId == -1 || inputTube->GetId() == m_CurrentTubeId)
    {
      ...
      for (unsigned int pointNum = 0; pointNum < pointListSize; ++pointNum)
      {
        ...
        double                        val = 0;
        if (inputImage->TransformPhysicalPointToIndex(pointWorld, imageIndex))
        {
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetPointValuesFromImageMean(
  const itk::Image<ImagePixelT, DimensionT> * inputImage,
  std::string                                 propertyId)
{
  typename TubeType::ChildrenListType::iterator tubeIterator;
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::ComputeTubeRegions(const ImageType * referenceImage)
{
  this->RenumberTubes();
  this->RenumberPoints();

  typedef itk::tube::TubeSpatialObjectToImageFilter<DimensionT, FloatImageType> TubeToImageFilterType;
  typename TubeToImageFilterType::Pointer tubeToImageFilter = TubeToImageFilterType::New();
  ...
  typedef itk::DanielssonDistanceMapImageFilter<FloatImageType, FloatImageType> DanielssonFilterType;
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetPointValuesFromTubeRegions(
  const itk::Image<ImagePixelT, DimensionT> * inputImage,
  ...
  double                                      maxRFactor)
{
  ...
  FloatImageIteratorType itDistImage(m_TubeDistanceImage, m_TubeDistanceImage->GetLargestPossibleRegion());
  itDistImage.GoToBegin();

  VectorImageIteratorType itDirImage(m_TubeDirectionImage, m_TubeDirectionImage->GetLargestPossibleRegion());
  itDirImage.GoToBegin();

  ImageConstIteratorType itInputImage(inputImage, inputImage->GetLargestPossibleRegion());
  ...
  TubeListPointerType inputTubeList = m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), "Tube");
  typename TubeType::ChildrenListType::iterator tubeIterator = inputTubeList->begin();
  while (tubeIterator != inputTubeList->end())
  {
    typename TubeType::Pointer           curTube = dynamic_cast<TubeType *>(tubeIterator->GetPointer());
    typename TubePointListType::iterator itTubePoints = curTube->GetPoints().begin();
    while (itTubePoints != curTube->GetPoints().end())
    {
      TubePointType *                    currentPoint = static_cast<TubePointType *>(&(*itTubePoints));
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SetPointValuesFromTubeRadius(
  const itk::Image<ImagePixelT, DimensionT> * inputImage,
  const std::string &                         propertyId,
  double                                      minRFactor,
  double                                      maxRFactor)
{
  ...
  typename TubeGroupType::ChildrenListType::iterator tubeIterator = inputTubeList->begin();
  ...
  while (tubeIterator != inputTubeList->end())
  {
    ...
    {
      TubePointType * tubePoint = dynamic_cast<TubePointType *>(tube->GetPoint(k));
      bool isInside = inputImage->TransformPhysicalPointToContinuousIndex(tubePoint->GetPositionInWorldSpace(), pointI);
      if (isInside)
      {
        double       val = 0;
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::FillGapToParent(double stepSize)
{
  TubeListPointerType inputTubeList = m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), "Tube");

  for (typename TubeGroupType::ChildrenListType::iterator itCurTube = inputTubeList->begin();
       itCurTube != inputTubeList->end();
       ++itCurTube)
  {
    typename TubeType::Pointer curTube = dynamic_cast<TubeType *>(itCurTube->GetPointer());
    if (m_CurrentTubeId == -1 || curTube->GetId() == m_CurrentTubeId)
    {
      ...
      if (curTube->GetRoot() == false && curParentTubeId != curTube->GetId())
      {
        ...
        for (typename TubeGroupType::ChildrenListType::iterator itTubes = inputTubeList->begin();
             itTubes != inputTubeList->end();
             ++itTubes)
        {
          typename TubeType::Pointer tube = dynamic_cast<TubeType *>(itTubes->GetPointer());
          if (tube->GetId() == curParentTubeId)
          {
            double minDistance = itk::NumericTraits<double>::max();
            int    flag = -1;
            for (unsigned int index = 0; index < tube->GetNumberOfPoints(); ++index)
            {
              TubePointType * tubePoint = dynamic_cast<TubePointType *>(tube->GetPoint(index));
              PositionType    tubePointPosition = tubePoint->GetPositionInObjectSpace();
              double distance = tubePointPosition.EuclideanDistanceTo(curTube->GetPoint(0)->GetPositionInObjectSpace());
              ...
              distance = tubePointPosition.EuclideanDistanceTo(
                curTube->GetPoint(curTube->GetNumberOfPoints() - 1)->GetPositionInObjectSpace());
              if (minDistance > distance)
              {
                minDistance = distance;
                parentNearestPoint = tubePoint;
            ...
            if (flag == 1)
            {
              TubePointType * childTubeStartPoint = dynamic_cast<TubePointType *>(curTube->GetPoint(0));
              InterpolatePath(parentNearestPoint, childTubeStartPoint, stepSize, newTubePoints);
              TubePointListType targetTubePoints = curTube->GetPoints();
              curTube->Clear();
              for (unsigned int index = 0; index < newTubePoints.size(); ++index)
              {
            ...
            if (flag == 2)
            {
              TubePointType * childTubeEndPoint =
                dynamic_cast<TubePointType *>(curTube->GetPoint(curTube->GetNumberOfPoints() - 1));
              InterpolatePath(parentNearestPoint, childTubeEndPoint, stepSize, newTubePoints);
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::InterpolatePath(typename TubeType::TubePointType *     parentNearestTubePoint,
                                                          ...
                                                          typename TubeType::TubePointListType & newTubePoints)
{
  ...
  {
    double tf =
      (parentNearestTubePoint->GetPositionInObjectSpace()[i] - childEndTubePoint->GetPositionInObjectSpace()[i]);
    midPoint[i] = tf / 2;
  ...
  {
    InterpolatePath(&midTubePoint, childEndTubePoint, stepSize, newTubePoints);
  }
  newTubePoints.push_back(midTubePoint);
  if (stepSize > 0 && midProjLen > stepSize)
  {
    InterpolatePath(parentNearestTubePoint, &midTubePoint, stepSize, newTubePoints);
  }
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SmoothTube(double h, SmoothTubeFunctionEnum smoothFunction)
{
  if (h <= 0)
  {
    return;
    ...
    if (m_CurrentTubeId == -1 || curTube->GetId() == m_CurrentTubeId)
    {
      ...
      for (pointItr = beginItr; pointItr != endItr; ++pointItr)
      {
        ...
        while (pos < wSize && tmpPointItr != endItr)
        {
          ...
          unsigned int                            d = 0;
          std::map<std::string, double>::iterator iter = tmpPointItr->GetTagScalarDictionary().begin();
          while (d < dictSize)
          {
        ...
        if (wTotal > 0)
        {
          ...
          unsigned int                            d = 0;
          std::map<std::string, double>::iterator iter = pointItr->GetTagScalarDictionary().begin();
          while (d < dictSize)
          {
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SmoothTubeProperty(const std::string &    propertyId,
                                                             double                 h,
    ...
    if (m_CurrentTubeId == -1 || curTube->GetId() == m_CurrentTubeId)
    {
      ...
      for (pointItr = beginItr; pointItr != endItr; ++pointItr)
      {
        ...
        while (pos < wSize && tmpPointItr != endItr)
        {
          ...
          {
            avgPropertyValue += w[pos] * tmpPointItr->GetTagScalarValue(propertyId);
          }
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::SubsampleTube(int N)
{
...
template <unsigned int DimensionT, class ImagePixelT>
double
TubeMathFilters<DimensionT, ImagePixelT>::ComputeTubeLength(void)
{
  TubeListPointerType inputTubeList = m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), "Tube");
       ...
       ++itCurTube)
  {
    ...
    if (m_CurrentTubeId == -1 || curTube->GetId() == m_CurrentTubeId)
    {
      ...
      typename TubePointListType::const_iterator itTubePoints = curTube->GetPoints().begin();

      PositionVectorType ptPrevPosVec = itTubePoints->GetPositionInObjectSpace().GetVectorFromOrigin();

      ++itTubePoints;

      while (itTubePoints != curTube->GetPoints().end())
      {
        PositionVectorType ptCurPosVec = itTubePoints->GetPositionInObjectSpace().GetVectorFromOrigin();
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::RenumberTubes(void)
{
  TubeListPointerType inputTubeList = m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), "Tube");
...
template <unsigned int DimensionT, class ImagePixelT>
void
TubeMathFilters<DimensionT, ImagePixelT>::RenumberPoints(void)
{
  TubeListPointerType inputTubeList = m_InputTubeGroup->GetChildren(m_InputTubeGroup->GetMaximumDepth(), "Tube");
