...
#ifndef itktubeAnisotropicDiffusionTensorImageFilter_hxx
#define itktubeAnisotropicDiffusionTensorImageFilter_hxx
...
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::AnisotropicDiffusionTensorImageFilter(void)
{
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::InitializeIteration(void)
{
  itkDebugMacro(<< "InitializeIteration() called .");

  AnisotropicDiffusionTensorFunction<UpdateBufferType> * f =
    dynamic_cast<AnisotropicDiffusionTensorFunction<UpdateBufferType> *>(this->GetDifferenceFunction().GetPointer());

  if (!f)
  {
    throw ExceptionObject(
      __FILE__, __LINE__, "Anisotropic diffusion Vessel Enhancement function is not set.", ITK_LOCATION);
  }
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::CopyInputToOutput(void)
{
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::AllocateUpdateBuffer(void)
{
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::AllocateDiffusionTensorImage(void)
{
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::ApplyUpdate(const TimeStepType & dt)
{
...
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::ApplyUpdateThreaderCallback(void * arg)
{
  DenseFDThreadStruct * str;
  ...
  total = str->Filter->SplitRequestedRegion(threadId, threadCount, splitRegionDiffusionTensorImage);
  if (threadId < total)
  {
    str->Filter->ThreadedApplyUpdate(str->TimeStep, splitRegion, splitRegionDiffusionTensorImage, threadId);
  }
...
template <class TInputImage, class TOutputImage>
typename AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::TimeStepType
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::CalculateChange(void)
{
...
template <class TInputImage, class TOutputImage>
ITK_THREAD_RETURN_FUNCTION_CALL_CONVENTION
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::CalculateChangeThreaderCallback(void * arg)
{
  DenseFDThreadStruct * str;
  ThreadIdType          total, threadId, threadCount;
  ...
  total = str->Filter->SplitRequestedRegion(threadId, threadCount, splitDiffusionimageRegion);
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::ThreadedApplyUpdate(
  TimeStepType             dt,
...
template <class TInputImage, class TOutputImage>
typename AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::TimeStepType
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::ThreadedCalculateChange(
  const ThreadRegionType &                     regionToProcess,
  ...
  ThreadIdType)
{
  ...
  typename FiniteDifferenceFunctionType::SpacingType spacing = output->GetSpacing();
  ...
  using DiffusionTensorFaceCalculatorType =
    NeighborhoodAlgorithm::ImageBoundaryFacesCalculator<DiffusionTensorImageType>;

  typedef typename DiffusionTensorFaceCalculatorType::FaceListType DiffusionTensorFaceListType;
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::GenerateData(void)
{
...
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::GetDiffusionTensorImage(void)
{
...
template <class TInputImage, class TOutputImage>
void
AnisotropicDiffusionTensorImageFilter<TInputImage, TOutputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
