...
PDFSegmenterParzenIO<TImage, TLabelMap>::PDFSegmenterParzenIO(void)
{
  Clear();
}

template <class TImage, class TLabelMap>
PDFSegmenterParzenIO<TImage, TLabelMap>::PDFSegmenterParzenIO(const char * _headerName)
{
  Clear();
...
template <class TImage, class TLabelMap>
PDFSegmenterParzenIO<TImage, TLabelMap>::PDFSegmenterParzenIO(const typename PDFSegmenterType::Pointer & _filter)
{
  Clear();
...
template <class TImage, class TLabelMap>
PDFSegmenterParzenIO<TImage, TLabelMap>::~PDFSegmenterParzenIO()
{}

template <class TImage, class TLabelMap>
void
PDFSegmenterParzenIO<TImage, TLabelMap>::PrintInfo() const
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzenIO<TImage, TLabelMap>::CopyInfo(const PDFSegmenterIOType & _filterIO)
{
  Clear();
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzenIO<TImage, TLabelMap>::Clear(void)
{
  m_PDFSegmenter = NULL;
}
...
template <class TImage, class TLabelMap>
bool
PDFSegmenterParzenIO<TImage, TLabelMap>::InitializeEssential(const typename PDFSegmenterType::Pointer & _filter)
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzenIO<TImage, TLabelMap>::SetPDFSegmenter(const typename PDFSegmenterType::Pointer & _filter)
{
  m_PDFSegmenter = _filter;
}
...
PDFSegmenterParzenIO<TImage, TLabelMap>::GetPDFSegmenter(void) const
{
...
template <class TImage, class TLabelMap>
bool
PDFSegmenterParzenIO<TImage, TLabelMap>::CanRead(const char * _headerName) const
{
...
template <class TImage, class TLabelMap>
bool
PDFSegmenterParzenIO<TImage, TLabelMap>::Read(const char * _headerName)
{
  ...
  mF = new MET_FieldRecordType;
  MET_InitReadField(mF, "BinsPerFeature", MET_INT_ARRAY, true, nFeaturesRec);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitReadField(mF, "ObjectPDFWeight", MET_FLOAT_ARRAY, true, nObjectsRec);
  metaFields.push_back(mF);
  ...
  tmpReadStream.open(_headerName, std::ios::binary | std::ios::in);
  ...
  {
    std::cerr << "PDFSegmenterParzenIO: Read: MET_Read Failed" << std::endl;
  ...
  if (static_cast<unsigned int>(mF->value[0]) != m_PDFSegmenter->GetNumberOfFeatures())
  {
  ...
  m_PDFSegmenter->SetHoleFillIterations(static_cast<int>(mF->value[0]));

  mF = MET_GetFieldRecord("ProbabilityImageSmoothingStandardDeviation", &metaFields);
  m_PDFSegmenter->SetProbabilityImageSmoothingStandardDeviation(static_cast<double>(mF->value[0]));

  mF = MET_GetFieldRecord("HistogramSmoothingStandardDeviation", &metaFields);
  m_PDFSegmenter->SetHistogramSmoothingStandardDeviation(static_cast<double>(mF->value[0]));

  mF = MET_GetFieldRecord("OutlierRejectPortion", &metaFields);
  m_PDFSegmenter->SetOutlierRejectPortion(static_cast<double>(mF->value[0]));
  ...
  for (unsigned int i = 0; i < nObjects; ++i)
  {
    ...
    for (unsigned int j = 0; j < numFeatures; ++j)
    {
      ...
      if (std::fabs(spacing[j] - m_PDFSegmenter->GetBinSize()[j]) > 0.005 * spacing[j])
      {
        std::cout << "ERROR: Spacing mismatch" << std::endl;
        std::cout << "   Spacing[" << j << "] = " << spacing[j] << std::endl;
        std::cout << "   PDFSpacing[" << j << "] = " << m_PDFSegmenter->GetBinSize()[j] << std::endl;
        for (unsigned int f = 0; f < metaFields.size(); ++f)
        {
          delete metaFields[f];
        }
      ...
      if (std::fabs(origin[j] - m_PDFSegmenter->GetBinMin()[j]) > 0.005 * spacing[j])
      {
        std::cout << "ERROR: Min mismatch" << std::endl;
        std::cout << "   Origin[" << j << "] = " << origin[j] << std::endl;
        std::cout << "   PDFMin[" << j << "] = " << m_PDFSegmenter->GetBinMin()[j] << std::endl;
        std::cout << "      spacing[" << j << "] = " << spacing[j] << std::endl;
        for (unsigned int f = 0; f < metaFields.size(); ++f)
        {
          delete metaFields[f];
    ...
    for (unsigned int j = numFeatures; j < PARZEN_MAX_NUMBER_OF_FEATURES; ++j)
    {
      spacing[j] = 1;
      origin[j] = 0;
      size[j] = 1;
    ...
    img->GetPixelContainer()->SetImportPointer(pdfClassReader.ExportPDF(), nPixels, true);
...
template <class TImage, class TLabelMap>
bool
PDFSegmenterParzenIO<TImage, TLabelMap>::Write(const char * _headerName)
{
  if (m_PDFSegmenter.IsNull())
  {
    return false;
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<int>(mF, "BinsPerFeature", MET_INT_ARRAY, numFeatures, tmpI);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<float>(mF, "BinMin", MET_FLOAT_ARRAY, numFeatures, tmpF);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<float>(mF, "BinSize", MET_FLOAT_ARRAY, numFeatures, tmpF);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<float>(mF, "ObjectPDFWeight", MET_FLOAT_ARRAY, nObjects, tmpF);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField(mF, "ErodeDilateRadius", MET_INT, m_PDFSegmenter->GetErodeDilateRadius());
  metaFields.push_back(mF);

  mF = new MET_FieldRecordType;
  MET_InitWriteField(mF, "HoleFillIterations", MET_INT, m_PDFSegmenter->GetHoleFillIterations());
  metaFields.push_back(mF);

  mF = new MET_FieldRecordType;
  MET_InitWriteField(mF,
                     "ProbabilityImageSmoothingStandardDeviation",
                     MET_FLOAT,
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField(
    mF, "HistogramSmoothingStandardDeviation", MET_FLOAT, m_PDFSegmenter->GetHistogramSmoothingStandardDeviation());
  metaFields.push_back(mF);

  mF = new MET_FieldRecordType;
  MET_InitWriteField(mF, "OutlierRejectPortion", MET_FLOAT, m_PDFSegmenter->GetOutlierRejectPortion());
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<const char>(mF, "ReclassifyObjectLabels", MET_STRING, strlen(tmpC), tmpC);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<const char>(mF, "ReclassifyNotObjectLabels", MET_STRING, strlen(tmpC), tmpC);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<const char>(mF, "ForceClassification", MET_STRING, strlen(tmpC), tmpC);
  metaFields.push_back(mF);
  ...
  mF = new MET_FieldRecordType;
  MET_InitWriteField<const char>(mF, "ObjectPDFFile", MET_STRING, tmpString.size(), tmpString.c_str());
  metaFields.push_back(mF);

  std::ofstream writeStream;

  writeStream.open(fullFileName.c_str(), std::ios::binary | std::ios::out);
  ...
  for (unsigned int i = 0; i < nObjects; ++i)
  {
    MetaClassPDF pdfClassWriter(m_PDFSegmenter->GetNumberOfFeatures(),
                                ...
                                m_PDFSegmenter->GetClassPDFImage(i)->GetPixelContainer()->GetBufferPointer());

    std::vector<int> tmpObjectId;
    for (unsigned int j = 0; j < nObjects; ++j)
    {
      tmpObjectId.push_back(static_cast<int>(m_PDFSegmenter->GetObjectId()[j]));
    }
    pdfClassWriter.SetObjectId(tmpObjectId);
    pdfClassWriter.SetObjectPDFWeight(m_PDFSegmenter->GetObjectPDFWeight());
    pdfClassWriter.SetVoidId(m_PDFSegmenter->GetVoidId());
    pdfClassWriter.SetErodeDilateRadius(m_PDFSegmenter->GetErodeDilateRadius());
    pdfClassWriter.SetHoleFillIterations(m_PDFSegmenter->GetHoleFillIterations());
    pdfClassWriter.SetHistogramSmoothingStandardDeviation(m_PDFSegmenter->GetHistogramSmoothingStandardDeviation());
    pdfClassWriter.SetProbabilityImageSmoothingStandardDeviation(
      m_PDFSegmenter->GetProbabilityImageSmoothingStandardDeviation());
    pdfClassWriter.SetOutlierRejectPortion(m_PDFSegmenter->GetOutlierRejectPortion());
    pdfClassWriter.SetReclassifyObjectLabels(m_PDFSegmenter->GetReclassifyObjectLabels());
    pdfClassWriter.SetReclassifyNotObjectLabels(m_PDFSegmenter->GetReclassifyNotObjectLabels());
    pdfClassWriter.SetForceClassification(m_PDFSegmenter->GetForceClassification());
