...
template <unsigned int TDimension>
TubeXIO<TDimension>::~TubeXIO(void)
{}

//
template <unsigned int TDimension>
void
TubeXIO<TDimension>::PrintSelf(std::ostream & os, Indent indent) const
{
...
template <unsigned int TDimension>
bool
TubeXIO<TDimension>::Read(const std::string & _fileName)
{
  ...
  std::ifstream tmpReadStream(_fileName.c_str(), std::ios::binary | std::ios::in);
  ...
  if (mF->defined)
  {
    ...
    {
      std::cerr << "Tube: Read failed: object is " << nDims << " dimensional and was expecting " << TDimension
                << " dimensional." << std::endl;
      for (unsigned int i = 0; i < fields.size(); ++i)
      {
        delete fields[i];
      }
  ...
  for (int obj = 0; obj < nObjects; ++obj)
  {
    ...
    tube->GetProperty().SetColor(tubeColor[0], tubeColor[1], tubeColor[2]);
...
template <unsigned int TDimension>
bool
TubeXIO<TDimension>::Write(const std::string & _fileName)
{
  std::ofstream tmpWriteStream(_fileName.c_str(), std::ios::binary | std::ios::out);
  ...
  typename TubeType::ChildrenListType * tubeList = m_TubeGroup->GetChildren(99999, soType);
  tmpWriteStream << std::endl;
  ...
  while (tIt != tEnd)
  {
    ...
    if (tube->GetProperty().GetTagStringValue("Artery", artery) && artery == "True")
    {
    ...
    tmpWriteStream << "Color: " << tube->GetProperty().GetRed() << " " << tube->GetProperty().GetGreen() << " "
...
template <unsigned int TDimension>
void
TubeXIO<TDimension>::SetTubeGroup(TubeGroupType * _tubes)
{
  m_TubeGroup = _tubes;
}
...
TubeXIO<TDimension>::GetTubeGroup(void)
{
  return m_TubeGroup;
}
