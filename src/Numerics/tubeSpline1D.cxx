...
void
Spline1D ::Use(ValueFunctionType::Pointer funcVal, Optimizer1D::Pointer optimizer1D)
{
  m_Defined = true;
...
double
Spline1D ::Value(double x)
{
  if (!m_Defined || (m_Clip && (x < (double)m_XMin || x > (double)m_XMax)))
  {
    std::cout << "clipping: " << m_XMin << " <= " << x << " <= " << m_XMax << std::endl;
    return 0;
  }
...
double
Spline1D ::ValueJet(double x, double * d, double * d2)
{
  if (!m_Defined || (m_Clip && (x < (double)m_XMin || x > (double)m_XMax)))
  {
    return 0;
...
bool
Spline1D ::Extreme(double * extX, double * extVal)
{
...
void
Spline1D ::PrintSelf(std::ostream & os, Indent indent) const
{
