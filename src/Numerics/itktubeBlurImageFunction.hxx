...
#ifndef itktubeBlurImageFunction_hxx
#define itktubeBlurImageFunction_hxx
...
BlurImageFunction<TInputImage>::BlurImageFunction(void)
{
  this->m_Image = NULL;
...
template <class TInputImage>
void
BlurImageFunction<TInputImage>::SetInputImage(const InputImageType * ptr)
{
  ...
  m_ImageIndexMin = this->GetInputImage()->GetLargestPossibleRegion().GetIndex();
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    m_ImageIndexMax[i] = m_ImageIndexMin[i] + this->GetInputImage()->GetLargestPossibleRegion().GetSize()[i] - 1;
  }
...
template <class TInputImage>
void
BlurImageFunction<TInputImage>::SetUseRelativeSpacing(bool useRelativeSpacing)
{
...
template <class TInputImage>
void
BlurImageFunction<TInputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "calculate Blurring value at point:" << std::endl;
  os << indent << "UseRelativeSpacing = " << m_UseRelativeSpacing << std::endl;
  os << indent << "Spacing = " << m_Spacing << std::endl;
  os << indent << "OriginalSpacing = " << m_OriginalSpacing << std::endl;
  os << indent << "Scale = " << m_Scale << std::endl;
  os << indent << "Extent = " << m_Extent << std::endl;

  os << indent << "KernelWeights.size = " << m_KernelWeights.size() << std::endl;
...
template <class TInputImage>
void
BlurImageFunction<TInputImage>::RecomputeKernel(void)
{
  if (this->GetDebug())
  {
...
template <class TInputImage>
void
BlurImageFunction<TInputImage>::SetScale(double scale)
{
  if (m_Scale != scale)
  {
    m_Scale = scale;
...
template <class TInputImage>
void
BlurImageFunction<TInputImage>::SetExtent(double extent)
{
  if (m_Extent != extent)
  {
    m_Extent = extent;
...
template <class TInputImage>
double
BlurImageFunction<TInputImage>::Evaluate(const PointType & point) const
{
  if (this->GetDebug())
  {
  ...
  else
  {
    ...
    if (!isInside)
    {
      std::cout << "ERROR: BlurImageFunction: Point is outside of image" << std::endl;
      return 0;
    }
  }
...
template <class TInputImage>
double
BlurImageFunction<TInputImage>::EvaluateAtIndex(const IndexType & point) const
{
  if (this->GetDebug())
  {
  ...
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    if (point[i] + m_KernelMin[i] < m_ImageIndexMin[i] || point[i] + m_KernelMax[i] > m_ImageIndexMax[i])
    {
      boundary = true;
      break;
    }
  ...
  if (!boundary)
  {
    itk::ImageRegionConstIterator<InputImageType> imIt(this->m_Image, this->m_Image->GetLargestPossibleRegion());
    imIt.GoToBegin();
  ...
  else
  {
    ...
    while (it != itEnd)
    {
      ...
      {
        kernelX[i] = point[i] + (*itX)[i];
        if (kernelX[i] < m_ImageIndexMin[i] || kernelX[i] > m_ImageIndexMax[i])
        {
          valid = false;
          break;
        }
...
template <class TInputImage>
double
BlurImageFunction<TInputImage>::EvaluateAtContinuousIndex(const ContinuousIndexType & point) const
{
  if (this->GetDebug())
  {
    std::cout << "BlurImageFunction::EvaluateAtContinuousIndex" << std::endl;
    std::cout << "  Point = " << point << std::endl;
  }
  ...
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    if ((int)(point[i]) + m_KernelMin[i] < m_ImageIndexMin[i] || (int)(point[i]) + m_KernelMax[i] > m_ImageIndexMax[i])
    {
      boundary = true;
      break;
    }
  ...
  else
  {
    ...
    for (unsigned int i = 0; i < ImageDimension; i++)
    {
      minX[i] = std::max((int)((int)(point[i]) + m_KernelMin[i]), (int)(m_ImageIndexMin[i]));
      maxX[i] = std::min((int)((int)(point[i]) + m_KernelMax[i]), (int)(m_ImageIndexMax[i]));
    }
