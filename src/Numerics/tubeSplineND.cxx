...
public:
  ...
  const OutputType &
  Value(const InputType & input) override
  {
...
public:
  ...
  typedef UserFunction<vnl_vector<double>, vnl_vector<double>> Superclass;
  typedef Self *                                               Pointer;
  typedef const Self *                                         ConstPointer;
...
SplineND ::SplineND(unsigned int               dimension,
...
void
SplineND ::Use(unsigned int               dimension,
               ValueFunctionType::Pointer funcVal,
               Spline1D::Pointer          spline1D,
               Optimizer1D::Pointer       optimizer1D)
{
...
void
SplineND ::SetXMin(IntVectorType xMin)
{
...
void
SplineND ::SetXMax(IntVectorType xMax)
{
  VectorType t(m_Dimension);
...
void
SplineND ::m_GetData(const VectorType & x)
{
  bool eql = true;
  if (!m_NewData)
  {
  ...
  if (!eql || m_NewData)
  {
    if (m_NewData)
    {
      ...
      itk::ImageRegionIterator<ImageType> it(m_Data, m_Data->GetLargestPossibleRegion());
      it.GoToBegin();
    ...
    else
    {
      ...
      itk::ImageRegionIterator<ImageType> it(m_DataWS, m_DataWS->GetLargestPossibleRegion());
      it.GoToBegin();
      ...
      itk::ImageRegionIterator<ImageType> itDest(m_Data, m_Data->GetLargestPossibleRegion());
      it.GoToBegin();
      itDest.GoToBegin();
      while (!it.IsAtEnd())
      {
...
double
SplineND ::Value(const VectorType & x)
{
  this->m_GetData(x);

  itk::ImageRegionIterator<ImageType> itData(m_Data, m_Data->GetLargestPossibleRegion());

  itk::ImageRegionIterator<ImageType> itDataWS(m_DataWS, m_DataWS->GetLargestPossibleRegion());
  ...
  itk::ImageRegionIterator<ImageType> itDataWSColumn(m_DataWS, m_DataWS->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ImageType> itDataWSDest(m_DataWS, m_DataWS->GetLargestPossibleRegion());
  ...
  for (int i = (int)m_Dimension - 1; i >= 0; i--)
  {
    ...
    for (unsigned int j = 0; j < k; j++)
    {
      ...
      itDataWSDest.Set(
        m_Spline1D->DataValue(m_Data1D, ((x((int)m_Dimension - i - 1) - (int)x((int)m_Dimension - i - 1)))));
      ++itDataWSDest;
    }
  }
...
double
SplineND ::ValueD(const VectorType & x, IntVectorType & dx)
{
  this->m_GetData(x);
...
SplineND::VectorType &
SplineND ::ValueD(const VectorType & x)
{
...
SplineND::MatrixType &
SplineND ::Hessian(const VectorType & x)
{
...
double
SplineND ::ValueJet(const VectorType & x, VectorType & d, MatrixType & h)
{
  IntVectorType dx(m_Dimension, 0);
...
double
SplineND ::ValueVDD2(const VectorType & x, VectorType & d, VectorType & d2)
{
  this->m_GetData(x);
  ...
  while (itWSX != m_DataWSX->End())
  {
    ...
    itk::ImageRegionIterator<ImageType> itImageWSX(itWSX->Value(), itWSX->Value()->GetLargestPossibleRegion());
    itk::ImageRegionIterator<ImageType> itImageWSXX(itWSXX->Value(), itWSXX->Value()->GetLargestPossibleRegion());
    itData.GoToBegin();
  ...
  for (int i = (int)m_Dimension - 1; i >= 0; i--)
  {
    ...
    itk::ImageRegionIterator<ImageType> itDataWSX(m_DataWS, m_DataWS->GetLargestPossibleRegion());
    itDataWSX.GoToBegin();
    ...
    for (unsigned int j = 0; j < k; j++)
    {
      ...
      for (unsigned int l = 0; l < m_Dimension; l++)
      {
        if ((int)m_Dimension - i != (int)l)
        {
          ...
          itk::ImageRegionIterator<ImageType> itImageWSX2(itWSX2->Value(), itWSX2->Value()->GetLargestPossibleRegion());
          itk::ImageRegionIterator<ImageType> itImageWSXX2(itWSXX2->Value(),
...
bool
SplineND ::Extreme(VectorType & extX, double * extVal)
{
...
bool
SplineND ::Extreme(VectorType & extX, double * extVal, unsigned int n, MatrixType & dirs)
{
...
bool
SplineND ::Extreme(VectorType & extX, double * extVal, VectorType & dir)
{
...
bool
SplineND ::ExtremeConjGrad(VectorType & extX, double * extVal)
{
  Hessian(extX);
...
void
SplineND ::PrintSelf(std::ostream & os, Indent indent) const
{
