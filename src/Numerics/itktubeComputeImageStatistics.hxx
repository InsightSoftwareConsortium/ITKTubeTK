...
ComputeImageStatistics<TPixel, VDimension>::ComputeImageStatistics()
{
  m_InputMask = NULL;
...
template <class TPixel, unsigned int VDimension>
void
ComputeImageStatistics<TPixel, VDimension>::SetQuantiles(const std::vector<float> _arg)
{
...
template <class TPixel, unsigned int VDimension>
void
ComputeImageStatistics<TPixel, VDimension>::GenerateData(void)
{
  // Sanity checks
  if (!this->GetInput())
  {
  ...
  itk::ImageRegionIterator<MaskType>     maskIter(m_InputMask, m_InputMask->GetLargestPossibleRegion());
  itk::ImageRegionIterator<ConnCompType> connCompIter(curConnComp, curConnComp->GetLargestPossibleRegion());
  ...
  itk::ImageRegionConstIterator<VolumeType> volumeIter(this->GetInput(), this->GetInput()->GetLargestPossibleRegion());
  id = 0;
  ...
  for (unsigned int i = 0; i < m_NumberOfComponents; ++i)
  {
    ...
    {
      m_CompStdDev[i] =
        std::sqrt((m_CompStdDev[i] - ((m_CompMean[i] * m_CompMean[i]) / m_CompCount[i])) / (m_CompCount[i] - 1));
  ...
  while (!volumeIter.IsAtEnd())
  {
    ...
    id = mapIter->second;
    int bin =
      static_cast<int>(((volumeIter.Get() - m_CompMin[id]) / (m_CompMax[id] - m_CompMin[id])) * maxNumBins + 0.5);
    if (bin < 0)
    {
      bin = 0;
    }
  ...
  m_QuantileValue.set_size(m_NumberOfComponents, numberOfQuantiles);
  for (unsigned int quantileNumber = 0; quantileNumber < numberOfQuantiles; ++quantileNumber)
  {
    double quant = m_Quantiles[quantileNumber];
    for (unsigned int comp = 0; comp < m_NumberOfComponents; ++comp)
    {
      unsigned int targetCount = static_cast<unsigned int>(quant * m_CompCount[comp]);
      unsigned int bin = 0;
      ...
      double binPortion = static_cast<double>(targetCount - binCount) / compHisto[comp][bin];
      double binSize = (m_CompMax[comp] - m_CompMin[comp]) / maxNumBins;
      m_QuantileValue[comp][quantileNumber] = (bin + binPortion) * binSize + m_CompMin[comp];
    }
  }
...
ComputeImageStatistics<TPixel, VDimension>::WriteCSVStatistics(std::string csvStatisticsFile) const
{
  ...
  {
    writeStream.open(csvStatisticsFile.c_str(), std::ios::binary | std::ios::out);
...
template <class TPixel, unsigned int VDimension>
void
ComputeImageStatistics<TPixel, VDimension>::PrintSelf(std::ostream & os, Indent indent) const
{
