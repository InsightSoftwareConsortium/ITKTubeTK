                 ...
                 vnl_vector<T> &       HEVal)
{
  ...
  if (!prevTangent.empty())
  {
    ...
    {
      double dProd = std::fabs(dot_product(prevTangent, HEVect.get_column(i)));
    ...
    if (dot_product(prevTangent, HEVect.get_column(ImageDimension - 1)) < 0)
    {
      for (unsigned int i = 0; i < ImageDimension - 1; ++i)
      {
        HEVect.get_column(ImageDimension - 1)[i] = -1 * HEVect.get_column(ImageDimension - 1)[i];
      }
    }
  }
  ...
  if (ridge > 0)
  {
    ...
    if (ImageDimension > 2)
    {
      roundness = std::sqrt((HEVal[ImageDimension - 2] * HEVal[ImageDimension - 2]) / avgv);
    }
    else
    {
      roundness = 1;
    ...
    //     = 1 for a flat ridge
    double denom = sumv + (HEVal[ImageDimension - 1] * HEVal[ImageDimension - 1]);
    if (denom != 0)
    {
...
template <class T>
void
ComputeEigen(const vnl_matrix<T> & mat, vnl_matrix<T> & eVects, vnl_vector<T> & eVals, bool orderByAbs, bool minToMax)
{
  ...
  for (unsigned int i = 0; i < n - 1; ++i)
  {
    for (unsigned int j = i + 1; j < n; ++j)
    {
      if (mat(i, j) != mat(j, i))
      {
        std::cout << "Non-symmetric matrix passed to eign-solver." << std::endl;
        std::cout << "   " << mat(i, j) << " != " << mat(j, i) << std::endl;
        symmetric = false;
        i = n - 1;
        break;
      }
  ...
  if (orderByAbs)
  {
    for (unsigned int i = 0; i < n - 1; i++)
    {
      for (unsigned int j = i + 1; j < n; j++)
      {
        if ((std::fabs(eVals(j)) > std::fabs(eVals(i)) && !minToMax) ||
            (std::fabs(eVals(j)) < std::fabs(eVals(i)) && minToMax))
        {
          T tf = eVals(j);
          eVals(j) = eVals(i);
          eVals(i) = tf;
  ...
  else
  {
    for (unsigned int i = 0; i < n - 1; i++)
    {
      for (unsigned int j = i + 1; j < n; j++)
      {
        if ((eVals(j) > eVals(i) && !minToMax) || (eVals(j) < eVals(i) && minToMax))
        {
          T tf = eVals(j);
          eVals(j) = eVals(i);
          eVals(i) = tf;
