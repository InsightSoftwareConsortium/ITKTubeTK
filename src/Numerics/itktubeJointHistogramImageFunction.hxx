...
#ifndef itktubeJointHistogramImageFunction_hxx
#define itktubeJointHistogramImageFunction_hxx
...
JointHistogramImageFunction<TInputImage, TCoordRep>::JointHistogramImageFunction(void)
{
  m_InputMask = NULL;
  m_FeatureWidth = 40;
  m_StdevBase = 0.001;
...
template <class TInputImage, class TCoordRep>
void
JointHistogramImageFunction<TInputImage, TCoordRep>::SetHistogramSize(const unsigned int & size)
{
...
template <class TInputImage, class TCoordRep>
void
JointHistogramImageFunction<TInputImage, TCoordRep>::SetInputImage(const InputImageType * ptr)
{
...
template <class TInputImage, class TCoordRep>
void
JointHistogramImageFunction<TInputImage, TCoordRep>::SetInputMask(const typename InputImageType::Pointer mask)
{
  m_InputMask = mask;
...
template <class TInputImage, class TCoordRep>
double
JointHistogramImageFunction<TInputImage, TCoordRep>::EvaluateAtIndex(const IndexType & index) const
{
  try
  {
...
template <class TInputImage, class TCoordRep>
void
JointHistogramImageFunction<TInputImage, TCoordRep>::PrecomputeAtIndex(const IndexType & index)
{
  typename HistogramType::Pointer hist;
  hist = this->ComputeHistogramAtIndex(index, false);

  itk::ImageRegionIterator<HistogramType> iterHist(hist, hist->GetLargestPossibleRegion());
  itk::ImageRegionIterator<HistogramType> iterSum(m_SumHistogram, m_SumHistogram->GetLargestPossibleRegion());
  itk::ImageRegionIterator<HistogramType> iterSumSquare(m_SumOfSquaresHistogram,
...
template <class TInputImage, class TCoordRep>
void
JointHistogramImageFunction<TInputImage, TCoordRep>::ComputeMeanAndStandardDeviation(void) const
{
  using SmootherType = itk::DiscreteGaussianImageFilter<HistogramType, HistogramType>;
  ...
  HistIteratorType sumItr(m_SumHistogram, m_SumHistogram->GetLargestPossibleRegion());
  HistIteratorType sumOfSquaresItr(m_SumOfSquaresHistogram, m_SumOfSquaresHistogram->GetLargestPossibleRegion());
  HistIteratorType meanItr(m_MeanHistogram, m_MeanHistogram->GetLargestPossibleRegion());
  HistIteratorType stdItr(m_StandardDeviationHistogram, m_StandardDeviationHistogram->GetLargestPossibleRegion());
  if (m_NumberOfSamples != 0)
  {
    while (!meanItr.IsAtEnd())
    {
      meanItr.Set(sumItr.Get() / m_NumberOfSamples);
      stdItr.Set(std::sqrt(std::fabs(sumOfSquaresItr.Get() / m_NumberOfSamples - meanItr.Get() * meanItr.Get())));
      ++sumItr;
      ++sumOfSquaresItr;
      ++meanItr;
      ++stdItr;
...
template <class TInputImage, class TCoordRep>
void
JointHistogramImageFunction<TInputImage, TCoordRep>::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "m_SumOfSquaresHistogram = " << m_SumOfSquaresHistogram << std::endl;
  os << indent << "m_MeanHistogram = " << m_MeanHistogram << std::endl;
  os << indent << "m_StandardDeviationHistogram = " << m_StandardDeviationHistogram << std::endl;
  ...
  os << indent << "m_NumberOfComputedSamples = " << m_NumberOfComputedSamples << std::endl;
...
template <class TInputImage, class TCoordRep>
double
JointHistogramImageFunction<TInputImage, TCoordRep>::ComputeZScoreAtIndex(const IndexType & index) const
{
  typename HistogramType::Pointer hist;
...
JointHistogramImageFunction<TInputImage, TCoordRep>::ComputeHistogramAtIndex(const IndexType & index, bool blur) const
{
  m_Histogram->FillBuffer(0);
