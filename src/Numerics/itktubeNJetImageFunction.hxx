...
#ifndef itktubeNJetImageFunction_hxx
#define itktubeNJetImageFunction_hxx
...
NJetImageFunction<TInputImage>::NJetImageFunction(void)
{
  m_InputImage = 0;
  m_InputImageMask = 0;
...
template <class TInputImage>
void
NJetImageFunction<TInputImage>::SetInputImage(const InputImageType * ptr)
{
  m_InputImage = ptr;
  ...
  if (ptr != 0)
  {
    ...
    for (unsigned int i = 0; i < ImageDimension; i++)
    {
      m_InputImageSpacingSquared[i] = m_InputImageSpacing[i] * m_InputImageSpacing[i];
    }
  }
...
template <class TInputImage>
void
NJetImageFunction<TInputImage>::SetInputImageMask(const InputImageType * ptr)
{
...
template <class TInputImage>
void
NJetImageFunction<TInputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
  this->Superclass::PrintSelf(os, indent);
  os << indent << "m_UseProjection = " << m_UseProjection << std::endl;
  os << indent << "m_UseInputImageMask = " << m_UseInputImageMask << std::endl;
  ...
  os << indent << "m_InputImageMinX = " << m_InputImageMinX << std::endl;
  os << indent << "m_InputImageMaxX = " << m_InputImageMaxX << std::endl;
  os << indent << "m_InputImageSize = " << m_InputImageSize << std::endl;
  os << indent << "m_InputImageSpacing = " << m_InputImageSpacing << std::endl;
  os << indent << "m_InputImageSpacingSquared = " << m_InputImageSpacingSquared << std::endl;
  ...
  os << indent << "m_MostRecentIntensity = " << m_MostRecentIntensity << std::endl;
  os << indent << "m_MostRecentDerivative = " << m_MostRecentDerivative << std::endl;
  os << indent << "m_MostRecentHessian = " << m_MostRecentHessian << std::endl;
  os << indent << "m_MostRecentRidgeness = " << m_MostRecentRidgeness << std::endl;
}
...
template <class TInputImage>
void
NJetImageFunction<TInputImage>::ComputeStatistics(void)
{
  if (m_InputImage)
  {
    m_ValidStats = true;
    ImageRegionConstIterator<InputImageType> imageIt(m_InputImage, m_InputImage->GetLargestPossibleRegion());

    if (m_UseInputImageMask)
    {
      ImageRegionConstIterator<InputImageType> maskIt(m_InputImageMask, m_InputImageMask->GetLargestPossibleRegion());
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::GetMin(void) const
{
  return m_StatsMin;
}
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::GetMax(void) const
{
  return m_StatsMax;
}
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Evaluate(const PointType & point, double scale) const
{
  if (!m_InputImage)
  {
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Evaluate(const PointType & point, const VectorType & v1, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Evaluate(const PointType &  point,
                                         const VectorType & v1,
                                         const VectorType & v2,
                                         double             scale) const
{
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::EvaluateAtIndex(const IndexType & index, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::EvaluateAtIndex(const IndexType & index, const VectorType & v1, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::EvaluateAtIndex(const IndexType &  index,
                                                const VectorType & v1,
                                                const VectorType & v2,
                                                double             scale) const
{
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::EvaluateAtContinuousIndex(const ContinuousIndexType & cIndex, double scale) const
{
  // EVALUATE
  ...
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    xMin[i] = (int)std::floor(cIndex[i] - (scale * m_Extent / m_InputImageSpacing[i]));
    ...
    xMax[i] = (int)std::ceil(cIndex[i] + (scale * m_Extent / m_InputImageSpacing[i]));
  ...
  while (!done)
  {
    ...
    if (maskIsSet)
    {
      ...
      for (unsigned int i = 0; i < ImageDimension; i++)
      {
        physDist += (xShift[i] - cIndex[i]) * (xShift[i] - cIndex[i]) * m_InputImageSpacingSquared[i];
      }
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::EvaluateAtContinuousIndex(const ContinuousIndexType & cIndex,
                                                          const VectorType &          v1,
                                                          double                      scale) const
{
  // EVALUATE
  ...
  else
  {
    ...
    m_MostRecentIntensity = (val0 + 0.5455 * val1 + 0.5455 * val2) / (1 + 2 * 0.5455) / 2;
  }
  return m_MostRecentIntensity;
}
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Derivative(const PointType &                                     point,
                                           double                                                scale,
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::DerivativeAtIndex(const IndexType &                                     index,
                                                  double                                                scale,
                                                  typename NJetImageFunction<TInputImage>::VectorType & d) const
{
  if (!m_InputImage)
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::DerivativeAtContinuousIndex(
  ...
  typename NJetImageFunction<TInputImage>::VectorType & d) const
{
  ...
  while (!done)
  {
    ...
    if (maskIsSet)
    {
      ...
      for (unsigned int i = 0; i < ImageDimension; i++)
      {
        physDist += (cIndex[i] - xShift[i]) * (cIndex[i] - xShift[i]) * m_InputImageSpacingSquared[i];
      }
      ...
      if (physDist <= physKernelRadiusSquared)
      {
        ...
        for (unsigned int i = 0; i < ImageDimension; i++)
        {
          expValueD = -(xShift[i] - cIndex[i]) * m_InputImageSpacing[i] * expValue;
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Hessian(const PointType &                                     point,
                                        double                                                scale,
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::HessianAtIndex(const IndexType &                                     index,
                                               double                                                scale,
                                               typename NJetImageFunction<TInputImage>::MatrixType & h) const
{
  if (!m_InputImage)
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::HessianAtContinuousIndex(const ContinuousIndexType &                           cIndex,
                                                         double                                                scale,
                                                         typename NJetImageFunction<TInputImage>::MatrixType & h) const
{
  VectorType d;
                                                         ...
                                                         typename NJetImageFunction<TInputImage>::MatrixType & m) const
{
  ...
  if (m_UseProjection)
  {
    ...
    for (unsigned int i = 0; i < ImageDimension; i++)
    {
      dp += dot_product(v1.GetVnlVector(), eigSys.get_eigenvector(i)) * eigSys.get_eigenvalue(i);
    }
                                                         ...
                                                         typename NJetImageFunction<TInputImage>::MatrixType & m) const
{
  ...
  if (m_UseProjection)
  {
    ...
    for (unsigned int i = 0; i < ImageDimension; i++)
    {
      dp0 += dot_product(v1.GetVnlVector(), eigSys.get_eigenvector(i)) * eigSys.get_eigenvalue(i);
      dp1 += dot_product(v2.GetVnlVector(), eigSys.get_eigenvector(i)) * eigSys.get_eigenvalue(i);
    }
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Jet(const PointType & point, VectorType & d, MatrixType & h, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::JetAtIndex(const IndexType & index, VectorType & d, MatrixType & h, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::JetAtContinuousIndex(const ContinuousIndexType & cIndex,
                                                     VectorType &                d,
                                                     MatrixType &                h,
                                                     double                      scale) const
{
  ...
  while (!done)
  {
    ...
    if (maskIsSet)
    {
      ...
      if (physDist <= physKernelRadiusSquared)
      {
        ...
        for (unsigned int i = 0; i < ImageDimension; i++)
        {
          double distI = (xShift[i] - cIndex[i]) * m_InputImageSpacing[i];

          expValueD = -distI * expValue;
          d[i] += pixelValue * expValueD;
          dTotal[i] += std::fabs(expValueD);

          expValueD = ((distI * distI) / (scale * scale) - 1.0) * expValue;
          h[i][i] += pixelValue * expValueD;
          hTotal[i][i] += std::fabs(expValueD);

          for (unsigned int j = i + 1; j < ImageDimension; j++)
          {
            double distJ = (xShift[j] - cIndex[j]) * m_InputImageSpacing[j];
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Ridgeness(const PointType & point, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Ridgeness(const PointType & point, const VectorType & v1, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::Ridgeness(const PointType &  point,
                                          const VectorType & v1,
                                          const VectorType & v2,
                                          double             scale) const
{
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::RidgenessAtIndex(const IndexType & index, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::RidgenessAtIndex(const IndexType & index, const VectorType & v1, double scale) const
{
  if (!m_InputImage)
  {
    itkWarningMacro(<< "Input image not set.");
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::RidgenessAtIndex(const IndexType &  index,
                                                 const VectorType & v1,
                                                 const VectorType & v2,
                                                 double             scale) const
{
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::RidgenessAtContinuousIndex(const ContinuousIndexType & cIndex, double scale) const
{
  // RIDGENESS
  VectorType d;
  MatrixType h;
  ...
  ::tube::ComputeRidgeness<double>(
    h.GetVnlMatrix().as_ref(), d.GetVnlVector(), prevTangent, ridgeness, roundness, curvature, levelness, eVect, eVal);
  ...
  m_MostRecentRidgeTangent.SetVnlVector(eVect.get_column(ImageDimension - 1));
...
template <class TInputImage>
double
NJetImageFunction<TInputImage>::RidgenessAtContinuousIndex(const ContinuousIndexType & cIndex,
                                                           const VectorType &          v1,
                                                           double                      scale) const
{
  // RIDGENESS
...
NJetImageFunction<TInputImage>::ScaleSubsample(double factor)
{
  if (m_InputImage)
  {
    ...
    IteratorType imageIt(newImage, newImage->GetLargestPossibleRegion());
    ...
    if (m_UseInputImageMask)
    {
      ...
      while (!imageIt.IsAtEnd())
      {
        ...
        {
          imageIt.Set((PixelType)this->EvaluateAtContinuousIndex(ic, spacing[0] / 2));
        }
        ++imageIt;
      }
    }
