...
MetaDocument::~MetaDocument(void) { this->ClearFields(); }

void
MetaDocument::Clear(void)
{
  this->SetComment("");
...
void
MetaDocument::CopyInformation(const Self * data)
{
...
bool
MetaDocument::Read(const std::string & fileName)
{
  if (!fileName.empty())
  {
...
bool
MetaDocument::Write(const std::string & fileName)
{
  if (!fileName.empty())
  {
    this->SetFileName(fileName);
...
void
MetaDocument::ClearFields(void)
{
  for (FieldListType::iterator it = m_FieldList.begin(); it != m_FieldList.end(); ++it)
  {
    delete *it;
    *it = NULL;
  }
...
MetaDocument::ReadFields(void)
{
  ...
  FieldType * const dateModifiedField = MET_GetFieldRecord("DateLastModified", &m_FieldList);
...
void
MetaDocument::SetupReadFields(void)
{
  this->ClearFields();
...
void
MetaDocument::SetupWriteFields(void)
{
  this->ClearFields();
  ...
  {
    FieldType * const commentField = new FieldType();
    MET_InitWriteField(commentField, "Comment", MET_STRING, std::strlen(this->GetComment()), this->GetComment());
    m_FieldList.push_back(commentField);
  }
  ...
  if (std::strlen(this->GetDateModified()) > 0)
  {
    FieldType * const dateModifiedField = new FieldType();
    MET_InitWriteField(
      dateModifiedField, "DateLastModified", MET_STRING, std::strlen(this->GetDateModified()), this->GetDateModified());
    m_FieldList.push_back(dateModifiedField);
  }
  ...
  {
    FieldType * const nameField = new FieldType();
    MET_InitWriteField(nameField, "Name", MET_STRING, std::strlen(this->GetName()), this->GetName());
    m_FieldList.push_back(nameField);
  }
}

bool
MetaDocument::WriteFields(void)
{
...
void
MetaDocument::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  for (FieldListType::const_iterator it = m_FieldList.begin(); it != m_FieldList.end(); ++it)
  {
