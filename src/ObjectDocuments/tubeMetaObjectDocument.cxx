...
MetaObjectDocument ::~MetaObjectDocument(void) { this->Clear(); }
...
void
MetaObjectDocument ::SetObjectDocumentList(ObjectDocumentListType & objectDocumentList)
{
...
void
MetaObjectDocument ::AddObjectDocument(ObjectDocumentType::Pointer object)
{
...
bool
MetaObjectDocument ::ReadFields(void)
{
...
void
MetaObjectDocument ::SetupObjectReadFields(void)
{
  ...
  for (unsigned int i = 0; i < m_MaximumNumberOfTransforms; ++i)
  {
    ...
    MET_InitReadField(transformNameField, labelTransform.str().c_str(), MET_STRING, false);
...
void
MetaObjectDocument ::SetupObjectWriteFields(unsigned int index)
{
  FieldType * const typeField = new FieldType();
  MET_InitWriteField(typeField,
                     "Type",
                     MET_STRING,
  ...
  for (unsigned int i = 0; i < m_ObjectDocumentList[index]->GetNumberOfTransformNames(); ++i)
  {
    std::stringstream label;
    label << "Transform" << i;
    FieldType * const transformNameField = new FieldType();
    MET_InitWriteField(transformNameField,
                       label.str().c_str(),
                       MET_STRING,
...
void
MetaObjectDocument ::SetupReadFields(void)
{
  this->Superclass::SetupReadFields();

  FieldType * const numberOfObjectDocumentsField = new FieldType();
  MET_InitReadField(numberOfObjectDocumentsField, "NumberOfObjects", MET_INT, true);
...
void
MetaObjectDocument ::SetupWriteFields(void)
{
  this->Superclass::SetupWriteFields();

  FieldType * const numberOfObjectDocumentsField = new FieldType();
  MET_InitWriteField(numberOfObjectDocumentsField, "NumberOfObjects", MET_INT, m_NumberOfObjectDocuments);
  numberOfObjectDocumentsField->required = true;
  m_FieldList.push_back(numberOfObjectDocumentsField);
...
bool
MetaObjectDocument ::WriteFields(void)
{
...
void
MetaObjectDocument ::PrintSelf(std::ostream & os, Indent indent) const
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "MaximumNumberOfTransforms: " << m_MaximumNumberOfTransforms << std::endl;
  os << indent << "NumberOfObjectDocuments:   " << m_NumberOfObjectDocuments << std::endl;
  os << indent << "ObjectDocumentList:" << std::endl;

  for (ObjectDocumentListType::const_iterator it = m_ObjectDocumentList.begin(); it != m_ObjectDocumentList.end(); ++it)
  {
