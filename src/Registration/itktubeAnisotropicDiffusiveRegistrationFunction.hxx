...
#ifndef itktubeAnisotropicDiffusiveRegistrationFunction_hxx
#define itktubeAnisotropicDiffusiveRegistrationFunction_hxx
...
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::PrintSelf(std::ostream & os,
                                                                                                  Indent indent) const
{
  Superclass::PrintSelf(os, indent);

  os << indent << "Time step: " << m_TimeStep << std::endl;
  os << indent << "Compute regularization term: " << (m_ComputeRegularizationTerm ? "on" : "off") << std::endl;
  os << indent << "Compute intensity distance term: " << (m_ComputeIntensityDistanceTerm ? "on" : "off") << std::endl;
  os << indent << "Regularization weighting: " << m_RegularizationWeighting << std::endl;
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void *
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::GetGlobalDataPointer(void) const
{
  ...
  {
    ans->m_RegularizationGlobalDataStruct = m_RegularizationFunction->GetGlobalDataPointer();
  }
  if (this->GetComputeIntensityDistanceTerm())
  {
    ans->m_IntensityDistanceGlobalDataStruct = m_IntensityDistanceFunction->GetGlobalDataPointer();
  }
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ReleaseGlobalDataPointer(
  void * GlobalData) const
{
  ...
  if (this->GetComputeIntensityDistanceTerm())
  {
    m_IntensityDistanceFunction->ReleaseGlobalDataPointer(gd->m_IntensityDistanceGlobalDataStruct);
  }
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::InitializeIteration(void)
{
  ...
  if (this->GetComputeIntensityDistanceTerm())
  {
    ...
    m_IntensityDistanceFunction->SetDisplacementField(this->GetDisplacementField());
 ...
 */
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::PixelType
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ComputeUpdate(
  ...
  const FloatOffsetType &                             offset)
{
  ...
  if (this->GetComputeIntensityDistanceTerm())
  {
    intensityDistanceTerm =
      m_IntensityDistanceFunction->ComputeUpdate(neighborhood, gd->m_IntensityDistanceGlobalDataStruct, offset);
  }
  ...
  if (this->GetComputeRegularizationTerm())
  {
    regularizationTerm = this->ComputeRegularizationUpdate(tensorNeighborhoods,
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::PixelType
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ComputeRegularizationUpdate(
  const DiffusionTensorNeighborhoodVectorType &       tensorNeighborhoods,
  const ScalarDerivativeImageRegionArrayVectorType &  deformationComponentFirstOrderDerivativeRegions,
  const TensorDerivativeImageRegionArrayVectorType &  deformationComponentSecondOrderDerivativeRegions,
  const TensorDerivativeImageRegionVectorType &       tensorDerivativeRegions,
  ...
  const FloatOffsetType &                             offset)
{
  ...
  for (int term = 0; term < numTerms; term++)
  {
    ...
    {
      assert(deformationComponentFirstOrderDerivativeRegions[term][i].GetImage());
      assert(deformationComponentSecondOrderDerivativeRegions[term][i].GetImage());

      // Compute div( T \grad( u ) )
      intermediateComponent =
        m_RegularizationFunction->ComputeUpdate(tensorNeighborhoods[term],
      ...
      {
        intermediateVector = intermediateComponent * multiplicationVectorRegionArrays[term][i].Get();
      }
      else
      {
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
double
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ComputeIntensityDistanceEnergy(
  const typename NeighborhoodType::IndexType index,
  const DeformationVectorType &              update)
{
  return itk::Math::sqr(m_IntensityDistanceFunction->ComputeIntensityDifference(index, update));
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
double
AnisotropicDiffusiveRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ComputeRegularizationEnergy(
  const DiffusionTensorNeighborhoodVectorType &      tensorNeighborhoods,
  const ScalarDerivativeImageRegionArrayVectorType & deformationComponentFirstOrderDerivativeRegions)
{
  ...
  for (int term = 0; term < numTerms; term++)
  {
    for (unsigned int i = 0; i < ImageDimension; i++)
    {
      DiffusionTensorType  diffusionTensor = tensorNeighborhoods[term].GetCenterPixel();
      ...
      for (unsigned int row = 0; row < ImageDimension; row++)
      {
        ...
        {
          multVector[row] += diffusionTensor(row, col) * deformationComponentFirstOrderDerivative[col];
        }
      }
  ...
  for (unsigned int i = 0; i < ImageDimension; i++)
  {
    regularizationEnergy += 0.5 * (termRegularizationEnergies[i] * termRegularizationEnergies[i]);
  }
