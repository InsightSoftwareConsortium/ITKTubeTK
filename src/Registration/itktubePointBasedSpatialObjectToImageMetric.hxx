...
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::PointBasedSpatialObjectToImageMetric(void)
{
  m_SamplingRatio = 0.1;
...
template <unsigned int ObjectDimension, class TFixedImage>
void
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::Initialize(void)
{
...
template <unsigned int ObjectDimension, class TFixedImage>
unsigned int
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::GetMaximumNumberOfPoints(void)
{
  ...
  typename PointBasedSpatialObjectType::ChildrenConstListType::const_iterator pbsoIter;
  for (pbsoIter = pbsoList->begin(); pbsoIter != pbsoList->end(); ++pbsoIter)
  {
    const TubeType *                    currentTube = dynamic_cast<const TubeType *>((*pbsoIter).GetPointer());
    const SurfaceType *                 currentSurface = dynamic_cast<const SurfaceType *>((*pbsoIter).GetPointer());
    const PointBasedSpatialObjectType * currentBlob = dynamic_cast<const BlobType *>((*pbsoIter).GetPointer());
    if (currentTube != nullptr)
    {
      const typename TubeType::TubePointListType & currentPoints = currentTube->GetPoints();
    ...
    {
      const typename SurfaceType::SurfacePointListType & currentPoints = currentSurface->GetPoints();
      auto                                               pIter = currentPoints.begin();
      while (pIter != currentPoints.end())
      {
        if (this->IsValidMovingPoint(*pIter))
    ...
    {
      const typename BlobType::SpatialObjectPointListType & currentPoints = currentBlob->GetPoints();
      auto                                                  pIter = currentPoints.begin();
      while (pIter != currentPoints.end())
      {
        if (this->IsValidMovingPoint(*pIter))
  ...
  return m_MaximumNumberOfPoints + m_MaximumNumberOfTubePoints + m_MaximumNumberOfSurfacePoints;
}


template <unsigned int ObjectDimension, class TFixedImage>
void
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::ComputeCenterOfRotation(void)
{
    ...
    if (currentTube != nullptr)
    {
      ...
      while (pIter != currentPoints.end())
      {
        if (this->IsValidMovingPoint(*pIter))
        {
          ...
          {
            this->m_CenterOfRotation[ii] += pIter->GetPositionInWorldSpace()[ii];
          }
          ++pointCount;
        }
        ++pIter;
...
template <unsigned int ObjectDimension, class TFixedImage>
void
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::ComputeSubsampledPoints(void)
{
  m_SubsampledBlobPoints.clear();
  m_SubsampledBlobPointsWeights.clear();
  m_SubsampledTubePoints.clear();
  ...
  if (oName.find("Tube") != std::string::npos || oName.find("Surface") != std::string::npos ||
      oName.find("Blob") != std::string::npos || oName.find("Point") != std::string::npos)
  {
  ...
  while (pbsoIter != pbsoList->end())
  {
    ...
    if (currentTube != nullptr)
    {
      ...
      for (unsigned int count = 0; pointIter != currentPoints.end() && count < endCount; ++count)
      {
        ++pointIter;
      }
...
template <unsigned int ObjectDimension, class TFixedImage>
void
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::ComputeSubsampledPointsWeights(void)
{
  m_SubsampledTubePointsWeights.clear();
  ...
  while (tubePointIter != m_SubsampledTubePoints.end())
  {
    ...
    if (m_TubePriorityRadius != 0 && m_TubePriorityRadius > m_TubeSamplingRadiusMin)
    {
      ...
      {
        double weight = (radius - m_TubeSamplingRadiusMin) / (m_TubePriorityRadius - m_TubeSamplingRadiusMin);
    ...
    else
    {
      ...
      {
        double weight = (radius - m_TubeSamplingRadiusMin) / (m_TubeSamplingRadiusMax - m_TubeSamplingRadiusMin);
        m_SubsampledTubePointsWeights.push_back(weight);
      }
      else
      {
...
template <unsigned int ObjectDimension, class TFixedImage>
typename PointBasedSpatialObjectToImageMetric<ObjectDimension,
                                              TFixedImage>::MovingSpatialObjectType::ChildrenConstListType *
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::GetPointBasedChildren(
  const MovingSpatialObjectType *                           parentSO,
...
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::GetValue(const ParametersType & parameters) const
{
  ...
  typename NJetImageFunction<FixedImageType>::Pointer imFunc = NJetImageFunction<FixedImageType>::New();
  imFunc->SetInputImage(this->m_FixedImage);

  VnlVectorType n1T, n2T;

  typename TubePointWeightListType::const_iterator pointWeightIter = m_SubsampledTubePointsWeights.begin();
  typename TubePointListType::const_iterator       pointIter = m_SubsampledTubePoints.begin();
  while (pointIter != m_SubsampledTubePoints.end())
  {
    ...
    if (this->IsValidFixedPoint(fixedPoint))
    {
      ...
      FixedPointType fixedRadiusPoint = transformCopy->TransformPoint(radiusPoint);
...
template <unsigned int ObjectDimension, class TFixedImage>
void
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::GetDerivative(const ParametersType & parameters,
...
template <unsigned int ObjectDimension, class TFixedImage>
void
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::GetValueAndDerivative(
  ...
  DerivativeType &       derivative) const
{
  ...
  if (weightSum > 0)
  {
    ...
    while (fixedPointsIter != fixedPoints.end())
    {
      ...
      this->m_Transform->ComputeJacobianWithRespectToParameters(fixedPoint, jacobian);
...
template <unsigned int ObjectDimension, class TFixedImage>
bool
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::IsValidMovingPoint(const TubePointType & pnt) const
{
  ...
  if (this->m_MovingSpatialObjectMaskObject.IsNotNull() && this->m_UseMovingSpatialObjectMaskObject)
  {
    if (!this->m_MovingSpatialObjectMaskObject->IsInsideInWorldSpace(pnt.GetPositionInWorldSpace()))
    {
      return false;
    }
  }
...
template <unsigned int ObjectDimension, class TFixedImage>
bool
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::IsValidMovingPoint(
  const SurfacePointType & pnt) const
{
  if (this->m_MovingSpatialObjectMaskObject.IsNotNull() && this->m_UseMovingSpatialObjectMaskObject)
  {
    if (!this->m_MovingSpatialObjectMaskObject->IsInsideInWorldSpace(pnt.GetPositionInWorldSpace()))
...
template <unsigned int ObjectDimension, class TFixedImage>
bool
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::IsValidMovingPoint(const BlobPointType & pnt) const
{
  if (this->m_MovingSpatialObjectMaskObject.IsNotNull() && this->m_UseMovingSpatialObjectMaskObject)
  {
    if (!this->m_MovingSpatialObjectMaskObject->IsInsideInWorldSpace(pnt.GetPositionInWorldSpace()))
...
template <unsigned int ObjectDimension, class TFixedImage>
bool
PointBasedSpatialObjectToImageMetric<ObjectDimension, TFixedImage>::IsValidFixedPoint(const FixedPointType & pnt) const
{
  ...
  if (this->m_FixedImageMaskObject.IsNotNull() && this->m_UseFixedImageMaskObject)
  {
