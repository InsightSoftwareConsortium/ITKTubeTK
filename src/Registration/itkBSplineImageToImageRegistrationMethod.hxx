...
class BSplineImageRegistrationViewer : public Command
{
public:
...
BSplineImageToImageRegistrationMethod<TImage>::BSplineImageToImageRegistrationMethod(void)
{
...
template <class TImage>
void
BSplineImageToImageRegistrationMethod<TImage>::GenerateData(void)
{
  ...
  const unsigned int numberOfParameters = this->GetTypedTransform()->GetNumberOfParameters();
  ...
  {
    std::cout << "ERROR: numberOfParameters != InitialTransformParameters.size()" << std::endl
              << "   Using identity trasnform." << std::endl;
    std::cout << numberOfParameters << " != " << this->GetInitialTransformParameters().GetSize() << std::endl;
...
template <class TImage>
void
BSplineImageToImageRegistrationMethod<TImage>::Optimize(MetricType * metric, InterpolatorType * interpolator)
{
...
template <class TImage>
void
BSplineImageToImageRegistrationMethod<TImage>::GradientOptimize(MetricType * metric, InterpolatorType * interpolator)
{
  ...
  */
  ...
  reg->GetTransform()->SetParametersByValue(this->GetInitialTransformParameters());
  ...
  // if( this->GetReportProgress() )
  {
    ...
    {
      p[pdi] = this->GetMovingImage()->GetLargestPossibleRegion().GetSize()[pdi] / 2.0;
    }
  ...
  try
  {
    std::cout << "  InitialParams = " << this->GetInitialTransformParameters() << std::endl;
    std::cout << "   reg->Update()" << std::endl;
    reg->Update();
    std::cout << "  FinalParams = " << reg->GetLastTransformParameters() << std::endl;
  }
  catch (itk::ExceptionObject & excep)
  {
    std::cout << "Exception caught during gradient registration of BSpline." << excep << std::endl;
...
template <class TImage>
void
BSplineImageToImageRegistrationMethod<TImage>::MultiResolutionOptimize(MetricType *       itkNotUsed(metric),
                                                                       InterpolatorType * itkNotUsed(interpolator))
{
  ...
  typedef RecursiveMultiResolutionPyramidImageFilter<ImageType, ImageType> PyramidType;
  ...
  unsigned int levelNumberOfControlPoints = this->GetNumberOfControlPoints();
  double       levelScale = 1;
  ...
  typename PyramidType::ScheduleType fixedSchedule = fixedPyramid->GetSchedule();
  typename PyramidType::ScheduleType movingSchedule = movingPyramid->GetSchedule();
  ...
  for (level = 1; level < numberOfLevelsUsing; level++)
  {
    for (unsigned int i = 0; i < ImageDimension; i++)
    {
      fixedSchedule[level][i] = (int)(fixedSchedule[level - 1][i] / controlPointFactor);
      if (fixedSchedule[level][i] < 1)
      {
        fixedSchedule[level][i] = 1;
      }
      movingSchedule[level][i] = (int)(movingSchedule[level - 1][i] / controlPointFactor);
  ...
  for (level = 0; level < numberOfLevelsUsing; level++)
  {
    // if( this->GetReportProgress() )
    {
      ...
      std::cout << "   Number of control points = " << levelNumberOfControlPoints << std::endl;
      std::cout << "   Fixed image = " << fixedPyramid->GetOutput(level)->GetLargestPossibleRegion().GetSize()
                << std::endl;
      std::cout << "   Moving image = " << movingPyramid->GetOutput(level)->GetLargestPossibleRegion().GetSize()
                << std::endl;
      std::cout << "   Parameters.size = " << levelParameters.size() << std::endl;
    }

    /**/
    /* Get the fixed and moving images for this pyramid level */
    /**/
    typename ImageType::ConstPointer fixedImage = fixedPyramid->GetOutput(level);
    typename ImageType::ConstPointer movingImage = movingPyramid->GetOutput(level);
    ...
    */
    ...
    // if( this->GetReportProgress() )
    {
      std::cout << "   Deformation magnitude = " << levelDeformationMagnitude << std::endl;
      std::cout << "   Number of samples = " << levelNumberOfSamples << std::endl;
    }
    ...
    reg->SetFixedImageSamplesIntensityThreshold(this->GetFixedImageSamplesIntensityThreshold());
    reg->SetUseFixedImageSamplesIntensityThreshold(this->GetUseFixedImageSamplesIntensityThreshold());
    reg->SetMaxIterations((unsigned int)(this->GetMaxIterations() * levelFactor));
    ...
    try
    {
      std::cout << "   reg->GetInitialTransformParameters() =" << reg->GetInitialTransformParameters() << std::endl;
      std::cout << "   reg->Update()" << std::endl;
      reg->Update();
    }
    catch (itk::ExceptionObject & excep)
    {
      std::cout << "Exception caught during level registration." << excep << std::endl;
      std::cout << "Current Matrix Transform = " << std::endl;
      reg->GetTransform()->Print(std::cout, 2);
    }
    catch (...)
    {
      std::cout << "Uncaught exception during helper class registration." << std::endl;
    }

    if (level < numberOfLevelsUsing - 1)
    {
      std::cout << "post levelParameters = " << reg->GetLastTransformParameters() << std::endl;
      levelNumberOfControlPoints = (unsigned int)(levelNumberOfControlPoints * controlPointFactor);
      if (levelNumberOfControlPoints > this->GetNumberOfControlPoints() || level == numberOfLevelsUsing - 2)
      {
      ...
      if (levelNumberOfControlPoints != reg->GetNumberOfControlPoints())
      {
        ...
        reg->ResampleControlGrid(levelNumberOfControlPoints, levelParameters);
      }
      else
      {
        levelParameters = reg->GetLastTransformParameters();
      }
      std::cout << "post resample levelParameters = " << levelParameters << std::endl;
    }
    else
    {
      /**/
...
BSplineImageToImageRegistrationMethod<TImage>::GetTypedTransform(void)
{
...
BSplineImageToImageRegistrationMethod<TImage>::GetTypedTransform(void) const
{
...
BSplineImageToImageRegistrationMethod<TImage>::GetBSplineTransform(void) const
{
...
template <class TImage>
void
BSplineImageToImageRegistrationMethod<TImage>::ResampleControlGrid(int              numberOfControlPoints,
                                                                   ParametersType & parameters)
{
  ...
  typedef BSplineResampleImageFunction<ParametersImageType, double>     ResamplerFunctionType;
  ...
  for (unsigned int k = 0; k < ImageDimension; k++)
  {
    ...
    typename ResamplerFunctionType::Pointer resamplerFunction = ResamplerFunctionType::New();
    ...
    */
    ...
    upsampler->SetInput(this->GetTypedTransform()->GetCoefficientImages()[k]);
    upsampler->SetInterpolator(resamplerFunction);
    upsampler->SetTransform(identity);
    upsampler->SetSize(tmpTrans->GetCoefficientImages()[k]->GetLargestPossibleRegion().GetSize());
    upsampler->SetOutputSpacing(tmpTrans->GetCoefficientImages()[k]->GetSpacing());
    upsampler->SetOutputOrigin(tmpTrans->GetCoefficientImages()[k]->GetOrigin());
    upsampler->SetOutputDirection(tmpTrans->GetCoefficientImages()[k]->GetDirection());
    try
    {
      upsampler->Update();
    }
    ...
    */
    ...
    Iterator                                         it(newCoefficients, newCoefficients->GetLargestPossibleRegion());
    while (!it.IsAtEnd())
    {
...
template <class TImage>
void
BSplineImageToImageRegistrationMethod<TImage>::PrintSelf(std::ostream & os, Indent indent) const
{
