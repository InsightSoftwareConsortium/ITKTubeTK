...
#ifndef itktubeDiffusiveRegistrationFilter_hxx
#define itktubeDiffusiveRegistrationFilter_hxx
...
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::DiffusiveRegistrationFilter(void)
{
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::PrintSelf(std::ostream & os,
                                                                                     Indent         indent) const
{
  ...
  {
    os << indent << "Stopping criterion mask: " << m_StoppingCriterionMask << std::endl;
  }
  if (m_HighResolutionStoppingCriterionMask)
  {
    os << indent << "High resolution stopping criterion mask: " << m_HighResolutionStoppingCriterionMask << std::endl;
  }
  os << "Stopping criterion evaluation period: " << m_StoppingCriterionEvaluationPeriod << std::endl;
  os << "Stopping criterion maximum total energy change: " << m_StoppingCriterionMaxTotalEnergyChange << std::endl;
}


template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::CreateRegistrationFunction(void)
{
  typename RegistrationFunctionType::Pointer registrationFunction = RegistrationFunctionType::New();
  registrationFunction->SetComputeRegularizationTerm(true);
  registrationFunction->SetComputeIntensityDistanceTerm(true);
  this->SetDifferenceFunction(static_cast<FiniteDifferenceFunctionType *>(registrationFunction.GetPointer()));
}


template <class TFixedImage, class TMovingImage, class TDeformationField>
typename DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::RegistrationFunctionType *
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::GetRegistrationFunctionPointer(void) const
{
  RegistrationFunctionType * df = dynamic_cast<RegistrationFunctionType *>(this->GetDifferenceFunction().GetPointer());
  return df;
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::AllocateUpdateBuffer(void)
{
  // The update buffer looks just like the output and holds the voxel changes
  typename OutputImageType::Pointer output = this->GetOutput();
  assert(output);
  DiffusiveRegistrationFilterUtils::AllocateSpaceForImage(m_UpdateBuffer, output);
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::Initialize(void)
{
  ...
  // [0,1]
  if (!DiffusiveRegistrationFilterUtils::IsIntensityRangeBetween0And1(this->GetFixedImage()))
  {
    itkWarningMacro(<< "Fixed image intensity should be [0, 1].");
  }
  if (!DiffusiveRegistrationFilterUtils::IsIntensityRangeBetween0And1(this->GetMovingImage()))
  {
  ...
  if (!DiffusiveRegistrationFilterUtils::CompareImageAttributes(this->GetDisplacementField(), this->GetFixedImage()))
  {
  ...
  if (m_StoppingCriterionMask)
  {
    ...
    if (!DiffusiveRegistrationFilterUtils::CompareImageAttributes(m_StoppingCriterionMask.GetPointer(),
                                                                  output.GetPointer()))
    {
      DiffusiveRegistrationFilterUtils::ResampleImageNearestNeighbor(
        m_HighResolutionStoppingCriterionMask, output, m_StoppingCriterionMask);
      assert(DiffusiveRegistrationFilterUtils::CompareImageAttributes(m_StoppingCriterionMask.GetPointer(),
                                                                      output.GetPointer()));
    }
  }
  ...
  {
    df->SetRegularizationWeighting(m_RegularizationWeightings[m_CurrentLevel - 1]);
  }
  else
  {
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::AllocateImageMembers(void)
{
  ...
  for (int i = 0; i < numTerms; i++)
  {
    if (this->GetComputeRegularizationTerm())
    {
      ...
      DiffusiveRegistrationFilterUtils::AllocateSpaceForImage(diffusionTensorPointer, output);
      tensorDerivativePointer = TensorDerivativeImageType::New();
      DiffusiveRegistrationFilterUtils::AllocateSpaceForImage(tensorDerivativePointer, output);
    }
  ...
  for (int i = 0; i < numTerms; i++)
  {
    ...
    if ((int)m_DeformationComponentFirstOrderDerivativeArrays.size() < numTerms)
    {
      m_DeformationComponentFirstOrderDerivativeArrays.push_back(deformationComponentFirstArray);
    }
    else
    {
      m_DeformationComponentFirstOrderDerivativeArrays[i] = deformationComponentFirstArray;
    }
    if ((int)m_DeformationComponentSecondOrderDerivativeArrays.size() < numTerms)
    {
      m_DeformationComponentSecondOrderDerivativeArrays.push_back(deformationComponentSecondArray);
    }
    else
    {
      m_DeformationComponentSecondOrderDerivativeArrays[i] = deformationComponentSecondArray;
    }
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::
  InitializeDeformationComponentAndDerivativeImages(void)
{
  ...
  {
    m_DeformationComponentFirstOrderDerivativeArrays[GAUSSIAN][i] = ScalarDerivativeImageType::New();
    DiffusiveRegistrationFilterUtils::AllocateSpaceForImage(
      m_DeformationComponentFirstOrderDerivativeArrays[GAUSSIAN][i], output);

    m_DeformationComponentSecondOrderDerivativeArrays[GAUSSIAN][i] = TensorDerivativeImageType::New();
    DiffusiveRegistrationFilterUtils::AllocateSpaceForImage(
      m_DeformationComponentSecondOrderDerivativeArrays[GAUSSIAN][i], output);
  }
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ComputeDiffusionTensorImages(void)
{
  assert(this->GetComputeRegularizationTerm());
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ComputeDiffusionTensorDerivativeImages(void)
{
  assert(this->GetComputeRegularizationTerm());
  assert(this->GetOutput());
  ...
  for (int i = 0; i < this->GetNumberOfTerms(); i++)
  {
    this->ComputeDiffusionTensorDerivativeImageHelper(m_DiffusionTensorImages[i], i, spacing, radius);
  }
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ComputeDiffusionTensorDerivativeImageHelper(
  ...
  const typename OutputImageType::SizeType & radius)
{
  ...
  TensorDerivativeImagePointer tensorDerivativeImage = m_DiffusionTensorDerivativeImages[term];
  assert(tensorDerivativeImage);

  // Get the FiniteDifferenceFunction to use in calculations.
  const RegistrationFunctionType * df = this->GetRegistrationFunctionPointer();
  assert(df);
  typename RegularizationFunctionType::ConstPointer reg = df->GetRegularizationFunctionPointer();
  assert(reg);

  // Setup the structs for the face calculations, the face iterators, and the
  // iterators over the current face
  FaceStruct<DiffusionTensorImagePointer> tensorStruct(tensorImage, tensorImage->GetLargestPossibleRegion(), radius);
  DiffusionTensorNeighborhoodType         tensorNeighborhood;

  FaceStruct<TensorDerivativeImagePointer> tensorDerivativeStruct(
    tensorDerivativeImage, tensorDerivativeImage->GetLargestPossibleRegion(), radius);
  TensorDerivativeImageRegionType tensorDerivativeRegion;

  for (tensorStruct.GoToBegin(), tensorDerivativeStruct.GoToBegin(); !tensorDerivativeStruct.IsAtEnd();
       tensorStruct.Increment(), tensorDerivativeStruct.Increment())
  {
    // Set the neighborhood iterators to the current face
    tensorStruct.SetIteratorToCurrentFace(tensorNeighborhood, tensorImage, radius);
    tensorDerivativeStruct.SetIteratorToCurrentFace(tensorDerivativeRegion, tensorDerivativeImage);

    // Iterate through the neighborhood for this face and compute derivatives
    for (tensorNeighborhood.GoToBegin(), tensorDerivativeRegion.GoToBegin(); !tensorNeighborhood.IsAtEnd();
         ++tensorNeighborhood, ++tensorDerivativeRegion)
    {
      reg->ComputeDiffusionTensorFirstOrderPartialDerivatives(tensorNeighborhood, tensorDerivativeRegion, spacing);
    }
  }
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::UpdateDeformationComponentImages(
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ComputeDeformationComponentDerivativeImages(
  void)
{
  assert(this->GetComputeRegularizationTerm());
  assert(this->GetOutput());
  ...
  for (int i = 0; i < this->GetNumberOfTerms(); i++)
  {
    DiffusiveRegistrationFilterUtils::ExtractXYZComponentsFromDeformationField(this->GetDeformationComponentImage(i),
                                                                               deformationComponentImageArray);

    for (unsigned int j = 0; j < ImageDimension; j++)
    {
      this->ComputeDeformationComponentDerivativeImageHelper(deformationComponentImageArray[j], i, j, spacing, radius);
    }
  }
}
                                                   ...
                                                   typename OutputImageType::SizeType &     radius)
{
  ...
  this->GetMultiThreader()->SetSingleMethod(this->ComputeDeformationComponentDerivativeImageHelperThreaderCallback,
                                            &str);
  ...
  m_DeformationComponentSecondOrderDerivativeArrays[term][dimension]->Modified();
}
...
{
  int threadId = ((MultiThreaderBase::WorkUnitInfo *)(arg))->WorkUnitID;
  int threadCount = ((MultiThreaderBase::WorkUnitInfo *)(arg))->NumberOfWorkUnits;

  ComputeDeformationComponentDerivativeImageHelperThreadStruct * str =
    (ComputeDeformationComponentDerivativeImageHelperThreadStruct *)(((MultiThreaderBase::WorkUnitInfo *)(arg))
                                                                       ->UserData);

  // Execute the actual method with appropriate output region
  // first find out how many pieces extent can be split into.
  // Using the SplitRequestedRegion method from itk::ImageSource.
  ThreadDeformationVectorComponentImageRegionType splitDeformationVectorComponentRegion;
  int total = str->Filter->SplitRequestedRegion(threadId, threadCount, splitDeformationVectorComponentRegion);

  ThreadScalarDerivativeImageRegionType splitScalarDerivativeRegion;
  str->Filter->SplitRequestedRegion(threadId, threadCount, splitScalarDerivativeRegion);

  ThreadTensorDerivativeImageRegionType splitTensorDerivativeRegion;
  str->Filter->SplitRequestedRegion(threadId, threadCount, splitTensorDerivativeRegion);

  if (threadId < total)
  {
    str->Filter->ThreadedComputeDeformationComponentDerivativeImageHelper(str->DeformationComponentImage,
...
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::
  ThreadedComputeDeformationComponentDerivativeImageHelper(
    ...
    const ThreadDeformationVectorComponentImageRegionType & deformationVectorComponentRegionToProcess,
    ...
    const typename OutputImageType::SizeType &              radius) const
{
  ...
  FaceStruct<DeformationVectorComponentImagePointer> deformationComponentStruct(
    deformationComponentImage, deformationVectorComponentRegionToProcess, radius);
  ...
  for (deformationComponentStruct.GoToBegin(), firstOrderStruct.GoToBegin(), secondOrderStruct.GoToBegin();
       !deformationComponentStruct.IsAtEnd();
       deformationComponentStruct.Increment(), firstOrderStruct.Increment(), secondOrderStruct.Increment())
  {
    // Set the neighborhood iterators to the current face
    deformationComponentStruct.SetIteratorToCurrentFace(
      deformationComponentNeighborhood, deformationComponentImage, radius);
    firstOrderStruct.SetIteratorToCurrentFace(firstOrderRegion, firstOrderDerivativeImage);
    secondOrderStruct.SetIteratorToCurrentFace(secondOrderRegion, secondOrderDerivativeImage);

    // Iterate through the neighborhood for this face and compute
    // derivatives
    for (deformationComponentNeighborhood.GoToBegin(), firstOrderRegion.GoToBegin(), secondOrderRegion.GoToBegin();
         !deformationComponentNeighborhood.IsAtEnd();
         ++deformationComponentNeighborhood, ++firstOrderRegion, ++secondOrderRegion)
    {
      reg->ComputeIntensityFirstAndSecondOrderPartialDerivatives(
        deformationComponentNeighborhood, firstOrderRegion, secondOrderRegion, spacing);
    }
  }
}
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::InitializeIteration(void)
{
  assert(this->GetOutput());
 ...
 */
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::TimeStepType
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::CalculateChange(void)
{
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::TimeStepType
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ThreadedCalculateChange(
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::TimeStepType
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::CalculateChangeGradient(void)
{
  // Set up for multithreaded processing.
  ...
  str.UpdateMetricsIntermediate = new UpdateMetricsIntermediateStruct[this->GetNumberOfWorkUnits()];
  ...
  this->GetMultiThreader()->SetNumberOfWorkUnits(this->GetNumberOfWorkUnits());
  this->GetMultiThreader()->SetSingleMethod(this->CalculateChangeGradientThreaderCallback, &str);
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
ITK_THREAD_RETURN_FUNCTION_CALL_CONVENTION
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::CalculateChangeGradientThreaderCallback(
  void * arg)
{
  int threadId = ((MultiThreaderBase::WorkUnitInfo *)(arg))->WorkUnitID;
  int threadCount = ((MultiThreaderBase::WorkUnitInfo *)(arg))->NumberOfWorkUnits;
  ...
  CalculateChangeGradientThreadStruct * str =
    (CalculateChangeGradientThreadStruct *)(((MultiThreaderBase::WorkUnitInfo *)(arg))->UserData);

  // Execute the actual method with appropriate output region
  // first find out how many pieces extent can be split into.
  // Using the SplitRequestedRegion method from itk::ImageSource.
  ThreadRegionType splitRegion;
  int              total = str->Filter->SplitRequestedRegion(threadId, threadCount, splitRegion);

  ThreadDiffusionTensorImageRegionType splitTensorRegion;
  str->Filter->SplitRequestedRegion(threadId, threadCount, splitTensorRegion);
  ...
  str->Filter->SplitRequestedRegion(threadId, threadCount, splitStoppingCriterionMaskImageRegion);

  if (threadId < total)
  {
    str->TimeStepList[threadId] = str->Filter->ThreadedCalculateChangeGradient(splitRegion,
                                                                               splitTensorRegion,
                                                                               splitTensorDerivativeRegion,
                                                                               splitScalarDerivativeRegion,
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::TimeStepType
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ThreadedCalculateChangeGradient(
  ...
  int)
{
  ...
  FaceStruct<OutputImagePointer> outputStruct(output, regionToProcess, radius);
  NeighborhoodType               outputNeighborhood;

  ImageRegionIterator<UpdateBufferType> updateIt;

  FaceStruct<DiffusionTensorImagePointer> tensorStruct(m_DiffusionTensorImages, tensorRegionToProcess, radius);
  DiffusionTensorNeighborhoodVectorType   tensorNeighborhoods;

  FaceStruct<ScalarDerivativeImagePointer> deformationComponentFirstOrderStruct(
    m_DeformationComponentFirstOrderDerivativeArrays, scalarDerivativeRegionToProcess, radius);
  ScalarDerivativeImageRegionArrayVectorType deformationComponentFirstOrderRegionArrays;

  FaceStruct<TensorDerivativeImagePointer> deformationComponentSecondOrderStruct(
    m_DeformationComponentSecondOrderDerivativeArrays, tensorDerivativeRegionToProcess, radius);
  TensorDerivativeImageRegionArrayVectorType deformationComponentSecondOrderRegionArrays;

  FaceStruct<TensorDerivativeImagePointer> tensorDerivativeStruct(
    m_DiffusionTensorDerivativeImages, tensorDerivativeRegionToProcess, radius);
  ...
  while (!outputStruct.IsAtEnd())
  {
    ...
    if (computeRegularization)
    {
      tensorStruct.SetIteratorToCurrentFace(tensorNeighborhoods, m_DiffusionTensorImages, radius);
      deformationComponentFirstOrderStruct.SetIteratorToCurrentFace(deformationComponentFirstOrderRegionArrays,
                                                                    m_DeformationComponentFirstOrderDerivativeArrays);
      deformationComponentSecondOrderStruct.SetIteratorToCurrentFace(deformationComponentSecondOrderRegionArrays,
                                                                     m_DeformationComponentSecondOrderDerivativeArrays);
      tensorDerivativeStruct.SetIteratorToCurrentFace(tensorDerivativeRegions, m_DiffusionTensorDerivativeImages);
      multiplicationVectorStruct.SetIteratorToCurrentFace(multiplicationVectorRegionArrays,
                                                          m_MultiplicationVectorImageArrays);
    }
    if (haveStoppingCriterionMask)
    {
      stoppingCriterionMaskStruct.SetIteratorToCurrentFace(stoppingCriterionMaskRegion, m_StoppingCriterionMask);
    }
    ...
    while (!outputNeighborhood.IsAtEnd())
    {
      ...
      // Compute updates
      updateTerm = df->ComputeUpdate(outputNeighborhood,
                                     tensorNeighborhoods,
      ...
      if (haveStoppingCriterionMask)
      {
        includeInStoppingCriterion = (stoppingCriterionMaskRegion.Value() == 0.0);
      }
      ...
      if (includeInStoppingCriterion)
      {
        ...
        for (unsigned int i = 0; i < ImageDimension; i++)
        {
          ...
          squaredIntensityDistanceUpdateMagnitude += vnl_math::sqr(intensityDistanceTerm[i]);
          squaredRegularizationUpdateMagnitude += vnl_math::sqr(regularizationTerm[i]);
        }
        localUpdateMetricsIntermediate.NumberOfPixelsProcessed++;
        localUpdateMetricsIntermediate.SumOfSquaredTotalUpdateMagnitude += squaredTotalUpdateMagnitude;
        ...
        localUpdateMetricsIntermediate.SumOfTotalUpdateMagnitude += std::sqrt(squaredTotalUpdateMagnitude);
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::CalculateEnergies(
  EnergiesStruct &  energies,
  OutputImageType * outputField)
{
  ...
  // Multithread the execution
  this->GetMultiThreader()->SetNumberOfWorkUnits(this->GetNumberOfWorkUnits());
  this->GetMultiThreader()->SetSingleMethod(this->CalculateEnergiesThreaderCallback, &str);
  this->GetMultiThreader()->SingleMethodExecute();
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
ITK_THREAD_RETURN_FUNCTION_CALL_CONVENTION
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::CalculateEnergiesThreaderCallback(void * arg)
{
  int threadId = ((MultiThreaderBase::WorkUnitInfo *)(arg))->WorkUnitID;
  int threadCount = ((MultiThreaderBase::WorkUnitInfo *)(arg))->NumberOfWorkUnits;

  CalculateEnergiesThreadStruct * str =
    (CalculateEnergiesThreadStruct *)(((MultiThreaderBase::WorkUnitInfo *)(arg))->UserData);
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ThreadedCalculateEnergies(
      ...
      if (includeInStoppingCriterion)
      {
        ...
        if (computeIntensityDistance)
        {
          localIntensityDistanceEnergy +=
            df->ComputeIntensityDistanceEnergy(outputNeighborhood.GetIndex(), outputNeighborhood.GetCenterPixel());
        }

        // Calculate regularization energy
        if (computeRegularization)
        {
          localRegularizationEnergy +=
            df->ComputeRegularizationEnergy(tensorNeighborhoods, deformationComponentFirstOrderRegionArrays);
        }
      }
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::UpdateUpdateStatistics(TimeStepType stepSize)
{
  // Compute the true sumOfSquared and sumOf metrics, considering the
  // actual stepSize
  double scalingForSumOfSquaredUpdateMagnitude = vnl_math::sqr(stepSize);
  m_UpdateMetrics.IntermediateStruct.SumOfSquaredTotalUpdateMagnitude *= scalingForSumOfSquaredUpdateMagnitude;
  m_UpdateMetrics.IntermediateStruct.SumOfSquaredIntensityDistanceUpdateMagnitude *=
    scalingForSumOfSquaredUpdateMagnitude;
  m_UpdateMetrics.IntermediateStruct.SumOfSquaredRegularizationUpdateMagnitude *= scalingForSumOfSquaredUpdateMagnitude;
  double scalingForSumOfUpdateMagnitude = stepSize;
  m_UpdateMetrics.IntermediateStruct.SumOfTotalUpdateMagnitude *= scalingForSumOfUpdateMagnitude;
  m_UpdateMetrics.IntermediateStruct.SumOfIntensityDistanceUpdateMagnitude *= scalingForSumOfUpdateMagnitude;
  m_UpdateMetrics.IntermediateStruct.SumOfRegularizationUpdateMagnitude *= scalingForSumOfUpdateMagnitude;

  // Compute the RMS and mean metrics
  double numPixels = (double)m_UpdateMetrics.IntermediateStruct.NumberOfPixelsProcessed;
  if (numPixels == 0)
  {
    m_UpdateMetrics.zero();
  }
  ...
  {
    m_UpdateMetrics.RMSTotalUpdateMagnitude =
      std::sqrt(m_UpdateMetrics.IntermediateStruct.SumOfSquaredTotalUpdateMagnitude / numPixels);
    m_UpdateMetrics.RMSIntensityDistanceUpdateMagnitude =
      std::sqrt(m_UpdateMetrics.IntermediateStruct.SumOfSquaredIntensityDistanceUpdateMagnitude / numPixels);
    m_UpdateMetrics.RMSRegularizationUpdateMagnitude =
      std::sqrt(m_UpdateMetrics.IntermediateStruct.SumOfSquaredRegularizationUpdateMagnitude / numPixels);
    m_UpdateMetrics.MeanTotalUpdateMagnitude = m_UpdateMetrics.IntermediateStruct.SumOfTotalUpdateMagnitude / numPixels;
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ApplyUpdate(const TimeStepType & dt)
{
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ApplyUpdate(TimeStepType       dt,
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
ITK_THREAD_RETURN_FUNCTION_CALL_CONVENTION
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ApplyUpdateThreaderCallback(void * arg)
{
  const ThreadIdType threadId = ((MultiThreaderBase::WorkUnitInfo *)(arg))->WorkUnitID;
  const ThreadIdType threadCount = ((MultiThreaderBase::WorkUnitInfo *)(arg))->NumberOfWorkUnits;

  DenseFDThreadStruct * str = (DenseFDThreadStruct *)(((MultiThreaderBase::WorkUnitInfo *)(arg))->UserData);
  ...
  if (threadId < total)
  {
    str->Filter->ThreadedApplyUpdate(str->OutputImage, str->TimeStep, splitRegion, threadId);
  }
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ThreadedApplyUpdate(const TimeStepType &,
                                                                                               const ThreadRegionType &,
                                                                                               ThreadIdType)
{
  // This function should never be called!
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::ThreadedApplyUpdate(
  OutputImagePointer &     outputImage,
  TimeStepType             dt,
  const ThreadRegionType & regionToProcess,
  ThreadIdType)
{
  ...
  for (updateIt.GoToBegin(), outputIt1.GoToBegin(), outputIt2.GoToBegin(); !updateIt.IsAtEnd();
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
DiffusiveRegistrationFilter<TFixedImage, TMovingImage, TDeformationField>::PostProcessIteration(TimeStepType stepSize)
{
  ...
  if (elapsedIterations == 0)
  {
    ...
    std::cout << "Iteration" << delimiter << "Time Step" << delimiter << "Total Time" << sectionDelimiter
              << "RMS Change" << sectionDelimiter << "RMS Change Change" << sectionDelimiter << "Mean Change "
              << sectionDelimiter << "Mean Change Change" << sectionDelimiter << "Energy" << sectionDelimiter
              << "Energy Change" << sectionDelimiter << "Stopping Criterion" << std::endl;
  }
  std::cout.setf(std::ios::fixed, std::ios::floatfield);
  std::cout.precision(6);
  std::cout << elapsedIterations << delimiter << stepSize << delimiter << totalTime << sectionDelimiter
            ...
            << m_Energies.TotalEnergy << delimiter << m_Energies.IntensityDistanceEnergy << delimiter
            << m_Energies.RegularizationEnergy << delimiter

            << ",,, " << energiesChange.TotalEnergy << " ,,, " << delimiter << energiesChange.IntensityDistanceEnergy
            << delimiter << energiesChange.RegularizationEnergy << sectionDelimiter
  ...
  {
    std::cout << "Total energy is increasing, indicating numeric instability. " << energiesChange.TotalEnergy << ".  "
  ...
  if (elapsedIterations != 0 && ((elapsedIterations + 1) % m_StoppingCriterionEvaluationPeriod) == 0)
  {
    if (totalEnergyChangeInEvaluationPeriod > m_StoppingCriterionMaxTotalEnergyChange)
    {
      std::cout << "Stopping criterion satisfied. " << totalEnergyChangeInEvaluationPeriod << ".  "
                << "Registration halting.";
      this->StopRegistration();
    }
