...
void
DiffusiveRegistrationFilterUtils ::AllocateSpaceForImage(TUnallocatedImagePointer &   image,
...
bool
DiffusiveRegistrationFilterUtils ::CompareImageAttributes(const TCheckedImage *  image,
                                                          const TTemplateImage * templateImage)
{
  assert(image);
  assert(templateImage);

  return image->GetOrigin() == templateImage->GetOrigin() && image->GetSpacing() == templateImage->GetSpacing() &&
         image->GetDirection() == templateImage->GetDirection() &&
         image->GetLargestPossibleRegion() == templateImage->GetLargestPossibleRegion() &&
         image->GetLargestPossibleRegion().GetIndex() == templateImage->GetLargestPossibleRegion().GetIndex() &&
         image->GetLargestPossibleRegion().GetSize() == templateImage->GetLargestPossibleRegion().GetSize();
}
...
void
DiffusiveRegistrationFilterUtils ::ResampleImageNearestNeighbor(const TResampleImagePointer & highResolutionImage,
                                                                const TemplateImagePointer &  templateImage,
                                                                TResampleImagePointer &       resampledImage)
{
  ...
  DiffusiveRegistrationFilterUtils::AllocateSpaceForImage(resampledImage, templateImage);

  // Do NN interpolation
  typedef itk::ImageRegionIteratorWithIndex<ResampleImageType> ResampleImageRegionType;
  ResampleImageRegionType                                      resampledImageIt =
    ResampleImageRegionType(resampledImage, resampledImage->GetLargestPossibleRegion());
  ...
  for (resampledImageIt.GoToBegin(); !resampledImageIt.IsAtEnd(); ++resampledImageIt)
  {
    resampledImage->TransformIndexToPhysicalPoint(resampledImageIt.GetIndex(), physicalPoint);
    highResolutionImage->TransformPhysicalPointToIndex(physicalPoint, highResolutionIndex);
...
template <class TResampleImagePointer, class TemplateImagePointer>
void
DiffusiveRegistrationFilterUtils ::ResampleImageLinear(const TResampleImagePointer & highResolutionImage,
                                                       const TemplateImagePointer &  templateImage,
                                                       TResampleImagePointer &       resampledImage)
{
  // Do linear interpolation
  typedef itk::ResampleImageFilter<typename TResampleImagePointer::ObjectType,
...
void
DiffusiveRegistrationFilterUtils ::VectorResampleImageLinear(const TVectorResampleImagePointer & highResolutionImage,
                                                             const TemplateImagePointer &        templateImage,
                                                             TVectorResampleImagePointer &       resampledImage,
                                                             bool                                normalize)
{
  // Do linear interpolation
  typedef itk::ResampleImageFilter<typename TVectorResampleImagePointer::ObjectType,
  ...
  resampler->SetOutputStartIndex(templateImage->GetLargestPossibleRegion().GetIndex());
...
void
DiffusiveRegistrationFilterUtils ::NormalizeVectorField(TVectorImagePointer & image)
{
  typedef ImageRegionIterator<typename TVectorImagePointer::ObjectType> DeformationVectorImageRegionType;
  DeformationVectorImageRegionType vectorIt(image, image->GetLargestPossibleRegion());
...
bool
DiffusiveRegistrationFilterUtils ::IsIntensityRangeBetween0And1(TImage * image)
{
