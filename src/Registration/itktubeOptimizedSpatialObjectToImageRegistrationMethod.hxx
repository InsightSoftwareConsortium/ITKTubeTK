...
#ifndef itktubeOptimizedSpatialObjectToImageRegistrationMethod_hxx
#define itktubeOptimizedSpatialObjectToImageRegistrationMethod_hxx
...
class SpatialObjectToImageRegistrationViewer : public Command
{
public:
...
OptimizedSpatialObjectToImageRegistrationMethod<ObjectDimension,
                                                TImage>::OptimizedSpatialObjectToImageRegistrationMethod(void)
{
...
template <unsigned int ObjectDimension, class TImage>
void
OptimizedSpatialObjectToImageRegistrationMethod<ObjectDimension, TImage>::GenerateData(void)
{
  ...
  typename SpatialObjectType::ConstPointer movingSpatialObject = this->GetMovingSpatialObject();
  ...
  metric->SetUseMovingSpatialObjectMaskObject(this->GetUseMovingSpatialObjectMaskObject());
  if (this->GetUseMovingSpatialObjectMaskObject())
  {
    metric->SetMovingSpatialObjectMaskObject(this->GetMovingSpatialObjectMaskObject());
  }
...
template <unsigned int ObjectDimension, class TImage>
void
OptimizedSpatialObjectToImageRegistrationMethod<ObjectDimension, TImage>::Optimize(MetricType * metric)
{
  if (this->GetMaxIterations() > 0)
  {
    if (m_UseEvolutionaryOptimization)
    {
      ...
      {
        std::cout << "Exception caught during evolutionary registration." << std::endl;
        std::cout << "Continuing using best values..." << std::endl;
        std::cout << "  Pos = " << evoOpt->GetCurrentPosition() << std::endl << std::endl;
      }

      if (evoOpt->GetCurrentPosition().size() == this->GetTransform()->GetNumberOfParameters())
      {
        bool valid = true;
    ...
    {
      std::cout << "Exception caught during gradient registration." << std::endl;
      std::cout << "Continuing using best values..." << std::endl;
      std::cout << "  Pos = " << gradOpt->GetCurrentPosition() << std::endl << std::endl;
    }

    if (gradOpt->GetCurrentPosition().size() == this->GetTransform()->GetNumberOfParameters())
    {
      bool valid = true;
...
template <unsigned int ObjectDimension, class TImage>
void
OptimizedSpatialObjectToImageRegistrationMethod<ObjectDimension, TImage>::PrintSelf(std::ostream & os,
                                                                                    Indent         indent) const
{
