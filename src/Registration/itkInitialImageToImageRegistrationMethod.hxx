...
#ifndef InitialImageToImageRegistrationMethod_hxx
#define InitialImageToImageRegistrationMethod_hxx
...
InitialImageToImageRegistrationMethod<TImage>::InitialImageToImageRegistrationMethod(void)
{
...
template <class TImage>
void
InitialImageToImageRegistrationMethod<TImage>::GenerateData(void)
{
  ...
  if (this->m_UseLandmarks)
  {
    ...
    if (TImage::ImageDimension == 3)
    {
      ...
      using LandmarkTransformCalculatorType =
        AnisotropicSimilarityLandmarkBasedTransformInitializer<LandmarkTransformType, TImage, TImage>;
  ...
  if (this->m_ComputeCenterOfRotationOnly)
  {
    ...
    this->GetMovingImage()->TransformIndexToPhysicalPoint(movingCenterIndex, movingCenterPoint);
  ...
  else if (this->m_NumberOfMoments == 0)
  {
    ...
    if (!this->GetUseRegionOfInterest())
    {
      ...
      this->GetFixedImage()->TransformIndexToPhysicalPoint(fixedCenterIndex, fixedCenterPoint);
    }
    else
    {
      for (unsigned int i = 0; i < ImageDimension; i++)
      {
        fixedCenterPoint[i] = (this->GetRegionOfInterestPoint1()[i] + this->GetRegionOfInterestPoint2()[i]) / 2;
      }
    }
  ...
  else
  {
    ...
    if (this->GetUseRegionOfInterest())
    {
      momCalc->SetRegionOfInterest(this->GetRegionOfInterestPoint1(), this->GetRegionOfInterestPoint2());
    }
    ...
    typename MomentsCalculatorType::AffineTransformType::Pointer movingImageAxesTransform;
    movingImageAxesTransform = momCalc->GetPrincipalAxesToPhysicalAxesTransform();
...
InitialImageToImageRegistrationMethod<TImage>::GetTypedTransform(void)
{
...
InitialImageToImageRegistrationMethod<TImage>::GetTypedTransform(void) const
{
...
InitialImageToImageRegistrationMethod<TImage>::GetAffineTransform(void) const
{
...
template <class TImage>
void
InitialImageToImageRegistrationMethod<TImage>::SetFixedLandmarks(const LandmarkPointContainer & fixedLandmarks)
{
...
template <class TImage>
void
InitialImageToImageRegistrationMethod<TImage>::SetMovingLandmarks(const LandmarkPointContainer & movingLandmarks)
{
...
template <class TImage>
void
InitialImageToImageRegistrationMethod<TImage>::PrintSelf(std::ostream & os, Indent indent) const
{
