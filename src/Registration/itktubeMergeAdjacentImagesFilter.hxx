...
MergeAdjacentImagesFilter<TImage>::MergeAdjacentImagesFilter(void)
{
  m_Background = 0;
  m_MaskZero = false;
  m_MaxIterations = 300;
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::SetInput1(const TImage * image)
{
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::SetInput2(const TImage * image)
{
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::SetPadding(const PaddingType & padding)
{
  m_Padding = padding;
}
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::PrintSelf(std::ostream & os, Indent indent) const
{
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::LoadInitialTransform(const std::string & filename)
{
  ...
  while (transformIt != transforms->end())
  {
    ...
    {
      typename TransformType::Pointer      affine_read = static_cast<TransformType *>((*transformIt).GetPointer());
      typename TransformType::ConstPointer affine = affine_read.GetPointer();
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::SaveOutputTransform(const std::string & filename)
{
...
template <class TImage>
void
MergeAdjacentImagesFilter<TImage>::Update()
{
  ...
  typename ImageType::SizeType size1 = m_Input1->GetLargestPossibleRegion().GetSize();
  ...
  typename ImageType::SizeType size2 = m_Input2->GetLargestPossibleRegion().GetSize();
  ...
  itk::ImageRegionConstIteratorWithIndex<ImageType> iter(m_Input1, m_Input1->GetLargestPossibleRegion());
  while (!iter.IsAtEnd())
  {
  ...
  typedef typename itk::ImageToImageRegistrationHelper<ImageType> RegFilterType;
  ...
  tmpImage = regOp->ResampleImage(
    RegFilterType::OptimizedRegistrationMethodType::LINEAR_INTERPOLATION, m_Input2, NULL, NULL, m_Background);
  ...
  {
    itk::ImageRegionConstIteratorWithIndex<ImageType> iter2(tmpImage, tmpImage->GetLargestPossibleRegion());

    itk::ImageRegionIteratorWithIndex<ImageType> iterOut(m_Output, m_Output->GetLargestPossibleRegion());
  ...
  else
  {
    ...
    itk::ImageRegionConstIteratorWithIndex<ImageType> iterTmp(tmpImage, tmpImage->GetLargestPossibleRegion());

    itk::ImageRegionIteratorWithIndex<ImageType> iter2(input2Reg, input2Reg->GetLargestPossibleRegion());

    itk::ImageRegionIteratorWithIndex<ImageType> iterOut(m_Output, m_Output->GetLargestPossibleRegion());

    itk::ImageRegionIteratorWithIndex<FloatImageType> iterOutMap(outputMap, outputMap->GetLargestPossibleRegion());
    ...
    {
      typedef typename itk::GeneralizedDistanceTransformImageFilter<FloatImageType, FloatImageType, FloatImageType>
                                      MapFilterType;
      typename MapFilterType::Pointer mapDistFilter = MapFilterType::New();

      typedef itk::BinaryThresholdImageFilter<FloatImageType, FloatImageType> Indicator;
    ...
    {
      typedef typename itk::DanielssonDistanceMapImageFilter<FloatImageType, FloatImageType, FloatImageType>
                                      MapFilterType;
      typename MapFilterType::Pointer mapDistFilter = MapFilterType::New();
    ...
    itk::ImageRegionConstIteratorWithIndex<FloatImageType> iterOutVor(outputVoronoiMap,
                                                                      outputVoronoiMap->GetLargestPossibleRegion());
    itk::ImageRegionIteratorWithIndex<FloatImageType> iterVorMap(vorImageMap, vorImageMap->GetLargestPossibleRegion());
    ...
    if (m_UseFastBlending)
    {
      typedef typename itk::GeneralizedDistanceTransformImageFilter<FloatImageType, FloatImageType> MapFilterType;
    ...
    else
    {
      ...
      typedef typename itk::SignedDanielssonDistanceMapImageFilter<FloatImageType, FloatImageType> SignedMapFilterType;
      typename SignedMapFilterType::Pointer mapVorFilter = SignedMapFilterType::New();
    ...
    iter2.GoToBegin();
    iterOut.GoToBegin();
    itk::ImageRegionIteratorWithIndex<FloatImageType> iterOutDistMap(outputDistMap,
                                                                     outputDistMap->GetLargestPossibleRegion());

    itk::ImageRegionIteratorWithIndex<FloatImageType> iterVorDistMap(vorImageDistMap,
                                                                     vorImageDistMap->GetLargestPossibleRegion());
