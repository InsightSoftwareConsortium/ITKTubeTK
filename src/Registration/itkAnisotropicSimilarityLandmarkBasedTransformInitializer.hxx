...
template <class TTransform, class TFixedImage, class TMovingImage>
void
AnisotropicSimilarityLandmarkBasedTransformInitializer<TTransform, TFixedImage, TMovingImage>::InitializeTransform()
{
  // Sanity check
  if (!m_Transform)
  {
  ...
  AnisotropicSimilarity3DTransformType * testPtr =
    dynamic_cast<AnisotropicSimilarity3DTransformType *>(this->m_Transform.GetPointer());
  if (testPtr)
  {
    transformType = AnisotropicSimilarity3Dtransform;
  }
  else if (dynamic_cast<VersorRigid3DTransformType *>(this->m_Transform.GetPointer()))
  {
  ...
  switch (transformType)
  {
    case AnisotropicSimilarity3Dtransform:
    {
      ...
      {
        itkExceptionMacro("In AnisotropicSimilarity3DTransform Fixed dimension must be 3");
        return;
      }
      if (MovingImageType::ImageDimension != 3)
      {
        itkExceptionMacro("In AnisotropicSimilarity3DTransform Moving dimension must be 3");
        return;
      }
      ...
      AnisotropicSimilarity3DTransformType * transform =
        dynamic_cast<AnisotropicSimilarity3DTransformType *>(this->m_Transform.GetPointer());
      ...
      while (fixedItr != m_FixedLandmarks.end())
      {
        fixedScale += (((*fixedItr)[0] - fixedCentroid[0]) * ((*fixedItr)[0] - fixedCentroid[0]));
        fixedScale += (((*fixedItr)[1] - fixedCentroid[1]) * ((*fixedItr)[1] - fixedCentroid[1]));
        fixedScale += (((*fixedItr)[2] - fixedCentroid[2]) * ((*fixedItr)[2] - fixedCentroid[2]));
        ++fixedItr;
      }
      ...
      while (movingItr != m_MovingLandmarks.end())
      {
        movingScale += (((*movingItr)[0] - movingCentroid[0]) * ((*movingItr)[0] - movingCentroid[0]));
        movingScale += (((*movingItr)[1] - movingCentroid[1]) * ((*movingItr)[1] - movingCentroid[1]));
        movingScale += (((*movingItr)[2] - movingCentroid[2]) * ((*movingItr)[2] - movingCentroid[2]));
        ++movingItr;
      }
      ...
      if (numberOfLandmarks >= ImageDimension)
      {
        ...
        for (int loop = 0; loop < 3; loop++)
        {
          ...
          while (movingItr != m_MovingLandmarks.end())
          {
            ...
            {
              fixedCentered[i] = ((*fixedItr)[i] - fixedCentroid[i]) * scale[i];
          ...
          using SymmetricEigenAnalysisType =
            itk::SymmetricEigenAnalysis<itk::Matrix<double, 4, 4>, vnl_vector<double>, vnl_matrix<double>>;
          ...
          while (fixedItr != m_FixedLandmarks.end())
          {
            ...
            {
              double tf;
              tf = ((fixedCentered[i] - movingCentroid[i]) * (fixedCentered[i] - movingCentroid[i]));
              if ((fixedCentered[i] - movingCentroid[i]) * ((*movingItr)[i] - movingCentroid[i]) < 0)
              {
                fixedAniSign[i] -= tf;
              }
              else
          ...
          while (movingItr != m_MovingLandmarks.end())
          {
            for (int i = 0; i < 3; i++)
            {
              movingAniScale[i] += (((*movingItr)[i] - movingCentroid[i]) * ((*movingItr)[i] - movingCentroid[i]));
            }
            ++movingItr;
          }
          ...
          for (int i = 0; i < 3; i++)
          {
            if (movingAniScale[i] < 0.005 * scaleMagnitude || fabs(fixedAniScale[i]) < 0.005 * scaleMagnitude)
            {
              newScale[i] = 1;
            }
            else
    ...
    case VersorRigid3Dtransform:
    {
      ...
      if (FixedImageType::ImageDimension != 3)
      {
        itkExceptionMacro("In VersorRigid3DTransform Fixed dimension must be 3");
        return;
      }
      if (MovingImageType::ImageDimension != 3)
      {
        itkExceptionMacro("In VersorRigid3DTransform Moving dimension must be 3");
        return;
      }
      ...
      VersorRigid3DTransformType * transform =
        dynamic_cast<VersorRigid3DTransformType *>(this->m_Transform.GetPointer());
    ...
    case Rigid2Dtransform:
    {
      ...
      {
        itkExceptionMacro("In Rigid2DTransform Fixed image dimension must be 2");
        return;
      }
      if (MovingImageType::ImageDimension != 2)
      {
        itkExceptionMacro("In Rigid2DTransform Moving image dimension must be 2");
        return;
      }

      Rigid2DTransformType * transform = dynamic_cast<Rigid2DTransformType *>(this->m_Transform.GetPointer());
    ...
    case Else:
    {
      itkWarningMacro(<< "Landmark initialization using the specified input transform "
                      << "not implemented");
    ...
    default:
    {
      itkWarningMacro(<< "Landmark initialization using the specified "
...
template <class TTransform, class TFixedImage, class TMovingImage>
void
AnisotropicSimilarityLandmarkBasedTransformInitializer<TTransform, TFixedImage, TMovingImage>::PrintSelf(
  std::ostream & os,
  Indent         indent) const
{
...
#endif /* __itkAnisotropicSimilarityLandmarkBasedTransformInitializer_hxx */
