...
MeanSquareRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::MeanSquareRegistrationFunction(void)
{
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
MeanSquareRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::PrintSelf(std::ostream & os,
                                                                                        Indent         indent) const
{
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
void
MeanSquareRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::InitializeIteration(void)
{
 ...
 */
template <class TFixedImage, class TMovingImage, class TDeformationField>
typename MeanSquareRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::PixelType
MeanSquareRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ComputeUpdate(
  ...
  const FloatOffsetType &  itkNotUsed(offset))
{
  ...
  {
    fixedGradientSquaredMagnitude += vnl_math::sqr(fixedGradient[j]) * m_FixedImageSpacing[j];
  }
  ...
  PixelType update;
  if (std::fabs(speedValue) < m_IntensityDifferenceThreshold || denominator < m_DenominatorThreshold)
  {
    update.Fill(0.0);
    return update;
  }
  ...
  for (unsigned int j = 0; j < ImageDimension; j++)
  {
    update[j] = speedValue * fixedGradient[j] / denominator * this->m_GradientStep;
    if (normalizemetric)
    {
...
template <class TFixedImage, class TMovingImage, class TDeformationField>
double
MeanSquareRegistrationFunction<TFixedImage, TMovingImage, TDeformationField>::ComputeIntensityDifference(
