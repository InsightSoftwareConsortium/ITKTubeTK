...
#ifndef itktubeDiffusiveRegistrationFilterUtils_h
#define itktubeDiffusiveRegistrationFilterUtils_h
...
public:
  ...
  static void
  AllocateSpaceForImage(TUnallocatedImagePointer & image, const TTemplateImagePointer & templateImage);

  /** Helper function to check whether the attributes of an image match a
   * template */
  template <class TCheckedImage, class TTemplateImage>
  static bool
  CompareImageAttributes(const TCheckedImage * image, const TTemplateImage * templateImage);
  ...
  static void
  ResampleImageNearestNeighbor(const TResampleImagePointer & highResolutionImage,
  ...
  static void
  VectorResampleImageLinear(const TVectorResampleImagePointer & highResolutionImage,
                            const TTemplateImagePointer &       templateImage,
  ...
  static void
  ExtractXYZComponentsFromDeformationField(const TDeformationField *         deformationField,
...
struct FaceStruct
{
  using FaceCalculatorType = NeighborhoodAlgorithm::ImageBoundaryFacesCalculator<typename TImage::ObjectType>;
  using FaceListType = typename FaceCalculatorType::FaceListType;
  using FaceListIteratorType = typename FaceListType::iterator;

  FaceStruct(void) { numberOfTerms = 0; }
  ...
  template <unsigned int VLength>
  FaceStruct(const std::vector<itk::FixedArray<TImage, VLength>> & images,
             typename TImage::ObjectType::RegionType               region,
             typename TImage::ObjectType::SizeType                 radius)
  {
    numberOfTerms = 0;
  ...
  void
  SetIteratorToCurrentFace(TIterator & iterator, const TImage & image, typename TImage::ObjectType::SizeType radius)
  {
    if (image.GetPointer())
    {
  ...
  template <class TIterator>
  void
  SetIteratorToCurrentFace(TIterator & iterator, const TImage & image)
  {
    if (image.GetPointer())
    {
  ...
  template <class TIterator>
  void
  SetIteratorToCurrentFace(std::vector<TIterator> &              iterators,
                           ...
                           typename TImage::ObjectType::SizeType radius)
  {
    if ((int)iterators.size() != numberOfTerms)
    {
      for (int i = 0; i < numberOfTerms; i++)
      {
        if (images[i].GetPointer())
        {
          iterators.push_back(TIterator(radius, images[i], *faceListIts[i]));
        }
        else
        {
  ...
  template <class TIterator>
  void
  SetIteratorToCurrentFace(std::vector<TIterator> & iterators, const std::vector<TImage> & images)
  {
    if ((int)iterators.size() != numberOfTerms)
    {
      for (int i = 0; i < numberOfTerms; i++)
  ...
  void
  SetIteratorToCurrentFace(std::vector<itk::FixedArray<TIterator, VLength>> &    iterators,
