...
MetaNJetLDA ::MetaNJetLDA(const char * _headerName)
{
  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA()" << std::endl;
...
MetaNJetLDA ::MetaNJetLDA(const MetaNJetLDA & metaNJetLDA)
  : MetaLDA()
{
  if (META_DEBUG)
  {
...
MetaNJetLDA ::MetaNJetLDA(const NJetScalesType & zeroScales,
                          ...
                          const ValueListType &  outputWhitenStdDevs)
{
  ...
  this->Clear();
  this->InitializeEssential(zeroScales,
                            firstScales,
                            secondScales,
                            ridgeScales,
                            numberOfPCABasis,
...
MetaNJetLDA ::~MetaNJetLDA(void) { this->M_Destroy(); }

void
MetaNJetLDA ::PrintInfo(void) const
{
  MetaLDA::PrintInfo();

  std::cout << "ZeroScales = " << m_ZeroScales.size() << std::endl;
  std::cout << "FirstScales = " << m_FirstScales.size() << std::endl;
  std::cout << "SecondScales = " << m_SecondScales.size() << std::endl;
  std::cout << "RidgeScales = " << m_RidgeScales.size() << std::endl;

  std::cout << "ZeroScalesTmp = " << m_ZeroScalesTmp.size() << std::endl;
  std::cout << "FirstScalesTmp = " << m_FirstScalesTmp.size() << std::endl;
  std::cout << "SecondScalesTmp = " << m_SecondScalesTmp.size() << std::endl;
  std::cout << "RidgeScalesTmp = " << m_RidgeScalesTmp.size() << std::endl;
}

void
MetaNJetLDA ::CopyInfo(const MetaNJetLDA & lda)
{
...
bool
MetaNJetLDA ::InitializeEssential(const NJetScalesType & zeroScales,
                                  const NJetScalesType & firstScales,
                                  const NJetScalesType & secondScales,
                                  const NJetScalesType & ridgeScales,
                                  unsigned int           numberOfPCABasis,
                                  ...
                                  const ValueListType &  outputWhitenStdDevs)
{
  ...
  MetaLDA::InitializeEssential(numberOfPCABasis,
                               numberOfLDABasis,
                               ldaValues,
                               ldaMatrix,
                               inputWhitenMeans,
...
void
MetaNJetLDA ::SetZeroScales(const NJetScalesType & zeroScales)
{
  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: SetZeroScales" << std::endl;
    std::cout << " Size = " << zeroScales.size() << std::endl;
    for (unsigned int i = 0; i < zeroScales.size(); i++)
    {
      std::cout << " Scale " << i << " = " << zeroScales[i] << std::endl;
    }
  }
...
const MetaNJetLDA::NJetScalesType &
MetaNJetLDA ::GetZeroScales(void) const
{
  if (META_DEBUG)
  {
...
void
MetaNJetLDA ::SetFirstScales(const NJetScalesType & firstScales)
{
  if (META_DEBUG)
  {
...
const MetaNJetLDA::NJetScalesType &
MetaNJetLDA ::GetFirstScales(void) const
{
  if (META_DEBUG)
  {
...
void
MetaNJetLDA ::SetSecondScales(const NJetScalesType & secondScales)
{
  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: SetSecondScales" << std::endl;
  }
...
const MetaNJetLDA::NJetScalesType &
MetaNJetLDA ::GetSecondScales(void) const
{
  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: GetSecondScales" << std::endl;
  }
...
void
MetaNJetLDA ::SetRidgeScales(const NJetScalesType & ridgeScales)
{
  if (META_DEBUG)
  {
...
const MetaNJetLDA::NJetScalesType &
MetaNJetLDA ::GetRidgeScales(void) const
{
  if (META_DEBUG)
  {
...
bool
MetaNJetLDA ::CanRead(const char * headerName) const
{
  ...
  const bool result = !std::strncmp(MET_ReadForm(inputStream).c_str(), "NJetLDA", 7);
...
bool
MetaNJetLDA ::Read(const char * headerName)
{
  ...
  {
    std::cout << "MetaNJetLDA: Read: Cannot open file _" << m_FileName << "_" << std::endl;
    delete tmpStream;
    return false;
  }
...
bool
MetaNJetLDA ::CanReadStream(METAIO_STREAM::ifstream * stream) const
{
...
bool
MetaNJetLDA ::ReadStream(METAIO_STREAM::ifstream * stream)
{
  if (META_DEBUG)
  {
  ...
  if (m_ReadStream)
  {
    std::cout << "MetaNJetLDA: ReadStream: Are two files open?" << std::endl;
    delete m_ReadStream;
  }
  ...
  if (!this->M_Read())
  {
    std::cout << "MetaNJetLDA: Read: Cannot parse file." << std::endl;
    m_ReadStream = NULL;
    return false;
  }
  ...
  this->InitializeEssential(m_ZeroScales,
                            m_FirstScales,
                            m_SecondScales,
                            m_RidgeScales,
...
bool
MetaNJetLDA ::Write(const char * headerName)
{
  if (headerName != NULL && std::strlen(headerName) > 1)
  {
    this->FileName(headerName);
...
bool
MetaNJetLDA ::WriteStream(METAIO_STREAM::ofstream * stream)
{
  if (m_WriteStream != NULL)
  {
    std::cout << "MetaNJetLDA: WriteStream: Are two files open?" << std::endl;
...
void
MetaNJetLDA ::M_SetupReadFields(void)
{
  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: M_SetupReadFields" << std::endl;
  }
  ...
  if (META_DEBUG)
  {
    std::cout << " nZeroScalesRecNum = " << nScalesRecNum << std::endl;
  }
  ...
  mF = new MET_FieldRecordType();
  MET_InitReadField(mF, "SecondScales", MET_FLOAT_ARRAY, false, nScalesRecNum);
  m_Fields.push_back(mF);
...
void
MetaNJetLDA ::M_SetupWriteFields(void)
{
  ...
  if (!m_ZeroScales.empty())
  {
    ...
    mF = new MET_FieldRecordType();
    MET_InitWriteField(mF, "ZeroScales", MET_FLOAT_ARRAY, nZeroScales, m_ZeroScalesTmp.data_block());
    m_Fields.push_back(mF);
  }
  ...
  if (!m_FirstScales.empty())
  {
    ...
    mF = new MET_FieldRecordType();
    MET_InitWriteField(mF, "FirstScales", MET_FLOAT_ARRAY, nFirstScales, m_FirstScalesTmp.data_block());
    m_Fields.push_back(mF);
  }
  ...
  if (!m_SecondScales.empty())
  {
    ...
    mF = new MET_FieldRecordType();
    MET_InitWriteField(mF, "SecondScales", MET_FLOAT_ARRAY, nSecondScales, m_SecondScalesTmp.data_block());
    m_Fields.push_back(mF);
  }
  ...
  if (!m_RidgeScales.empty())
  {
    ...
    mF = new MET_FieldRecordType();
    MET_InitWriteField(mF, "RidgeScales", MET_FLOAT_ARRAY, nRidgeScales, m_RidgeScalesTmp.data_block());
    m_Fields.push_back(mF);
  }
}

bool
MetaNJetLDA ::M_Read(void)
{
  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: M_Read: Loading header." << std::endl;
  }

  if (!MetaLDA::M_Read())
  {
    std::cout << "MetaNJetLDA: M_Read: Error parsing file." << std::endl;
    return false;
  }

  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: M_Read: Parsing header." << std::endl;
  }

  if (META_DEBUG)
  {
    std::cout << "MetaNJetLDA: M_Read: num fields = " << m_Fields.size() << std::endl;
  ...
  if (mF && mF->defined)
  {
    ...
    if (META_DEBUG)
    {
      std::cout << "MetaNJetLDA: M_Read: ZeroScales" << std::endl;
