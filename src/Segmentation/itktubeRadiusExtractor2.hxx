...
class LocalMedialnessSplineValueFunction : public ::tube::UserFunction<int, double>
{
public:
  LocalMedialnessSplineValueFunction(RadiusExtractor2<TInputImage> * _radiusExtractor)
  {
    m_Value = 0;
  ...
  Value(const int & x)
  {
    m_Value = m_RadiusExtractor->GetKernelMedialness(x * this->GetVariableMultiplier());
...
/** Constructor */
template <class TInputImage>
RadiusExtractor2<TInputImage>::RadiusExtractor2(void)
{
  m_InputImage = NULL;
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::SetInputImage(typename InputImageType::Pointer inputImage)
{
  ...
  if (m_InputImage)
  {
    ...
    if (this->GetDebug())
    {
      ::tube::DebugMessage("RadiusExtractor2: SetInputImage: Minimum = " + std::to_string(m_DataMin));
      ::tube::DebugMessage("RadiusExtractor2: SetInputImage: Maximum = " + std::to_string(m_DataMax));
    }
  }
}
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::GetPointVectorMeasures(std::vector<TubePointType> & points,
                                                      double                       pntR,
                                                      double &                     mness,
                                                      double &                     bness,
                                                      bool                         doBNess)
...
template <class TInputImage>
bool
RadiusExtractor2<TInputImage>::GetPointVectorOptimalRadius(std::vector<TubePointType> & points,
                                                           double &                     r0,
                                                           double                       rMin,
                                                           double                       rMax,
                                                           double                       rStep,
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::SetNumKernelPoints(unsigned int _numPoints)
{
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::GenerateKernel(void)
{
  IndexType minXI;
  IndexType maxXI;
  ...
  {
    bufferI[i] = static_cast<unsigned int>(this->GetKernelExtent() * this->GetRadiusMax() / m_Spacing);
  }

  typename std::vector<TubePointType>::iterator pntIter;
  pntIter = m_KernelTube->GetPoints().begin();
  IndexType kernelPointI;
  if (!m_InputImage->TransformPhysicalPointToIndex(pntIter->GetPositionInObjectSpace(), kernelPointI))
  {
  ...
  std::fill(m_KernelTangentDistances.begin(), m_KernelTangentDistances.end(), 0);
  ...
  while (!done)
  {
    if (m_InputImage->GetLargestPossibleRegion().IsInside(xI) && m_InputImage->GetPixel(xI) > thresh)
    {
      m_KernelValues[count] = (m_InputImage->GetPixel(xI) - m_DataMin) / (m_DataMax - m_DataMin);
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::SetKernelTubePoints(const std::vector<TubePointType> & tubePoints)
{
...
template <class TInputImage>
double
RadiusExtractor2<TInputImage>::GetKernelMedialness(double r)
{
  ...
  if (this->GetDebug())
  {
    ...
    ::tube::InformationMessage("   Dist = " + std::to_string(distMin) + " - " + std::to_string(distMax));
    ::tube::InformationMessage("   Area = " + std::to_string(areaPos) + " - " + std::to_string(areaNeg));
  }
  ...
  {
    if (((*iterTanDist) < distMax || (*iterTanDist) < 1) && (*iterDist) >= distMin && (*iterDist) <= distMax)
    {
  ...
  if (this->GetDebug())
  {
    std::cout << "   Count = " << histoPosCount << " - " << histoNegCount << std::endl;
    std::cout << "   Val = " << pVal << " - " << nVal << " = " << pVal - nVal << std::endl;
  }
...
template <class TInputImage>
double
RadiusExtractor2<TInputImage>::GetKernelBranchness(double r)
{
  ...
  while (iterDist != m_KernelDistances.end())
  {
    if ((*iterTanDist) < r && (*iterDist) >= distMin && (*iterDist) <= distMax)
    {
      bin = (*iterValue) * histoBins;
      if (bin < 0)
      {
...
template <class TInputImage>
bool
RadiusExtractor2<TInputImage>::UpdateKernelOptimalRadius(void)
{
  LocalMedialnessSplineValueFunction<TInputImage> * myFunc = new LocalMedialnessSplineValueFunction<TInputImage>(this);
  ::tube::UserFunction<int, double> *               myUserFunc = myFunc;

  ::tube::GoldenMeanOptimizer1D * opt = new ::tube::GoldenMeanOptimizer1D();
  ...
  ::tube::SplineApproximation1D * spline = new ::tube::SplineApproximation1D(myUserFunc, opt);
...
template <class TInputImage>
bool
RadiusExtractor2<TInputImage>::ExtractRadii(TubeType * tube, bool verbose)
{
  ...
  for (int p = static_cast<int>(pntCount); p < static_cast<int>(tube->GetPoints().size()); p += this->GetKernelStep())
  {
    ...
    if (verbose)
    {
      std::cout << p << " : r = " << rStart << " -> " << tube->GetPoint(p)->GetRadiusInObjectSpace() << std::endl;
    }
  }

  rStart = rStart0;
  for (int p = static_cast<int>(pntCount) - this->GetKernelPointStep(); p >= 0; p -= this->GetKernelStep())
  {
    this->SetRadiusStart(rStart);
    this->GenerateKernelTubePoints(p, tube);
    this->GenerateKernel();
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::GenerateKernelTubePoints(unsigned int tubePointNum, TubeType * tube)
{
  ...
  for (int p = startP; p <= endP; p += m_KernelPointStep)
  {
    if (p < 0)
    {
      typename TubeType::PointType p1 = tube->GetPoints()[0].GetPositionInObjectSpace();
      typename TubeType::PointType p2 =
        tube->GetPoints()[m_NumKernelPoints / 2 * m_KernelPointStep].GetPositionInObjectSpace();
      for (unsigned int i = 0; i < ImageDimension; ++i)
      {
    ...
    {
      typename TubeType::PointType p1 = tube->GetPoints()[tubeSize - 1].GetPositionInObjectSpace();
      typename TubeType::PointType p2 =
        tube->GetPoints()[tubeSize - 1 - m_NumKernelPoints / 2 * m_KernelPointStep].GetPositionInObjectSpace();
      for (unsigned int i = 0; i < ImageDimension; ++i)
      {
        p2[i] = p1[i] - ((p2[i] - p1[i]) * ((p - (tubeSize - 1)) / m_KernelPointStep));
      }
      m_KernelTube->GetPoints()[count].SetPositionInObjectSpace(p2);
    }
    else
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::RecordOptimaAtTubePoints(unsigned int tubePointNum, TubeType * tube)
{
  ...
  for (int p = startP; p <= endP; ++p)
  {
    if (p < static_cast<int>(tubePointNum))
    {
      ...
      {
        d = static_cast<double>(tubePointNum - p) / (tubePointNum - startP);
      }
    ...
    else
    {
      ...
      {
        d = static_cast<double>(p - tubePointNum) / (endP - tubePointNum);
      }
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "KernelDistances = " << m_KernelDistances.size() << std::endl;
  os << indent << "KernelTangentDistances = " << m_KernelTangentDistances.size() << std::endl;
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::SetIdleCallBack(bool (*idleCallBack)())
{
...
template <class TInputImage>
void
RadiusExtractor2<TInputImage>::SetStatusCallBack(void (*statusCallBack)(const char *, const char *, int))
{
