...
SegmentTubeUsingMinimalPathFilter<Dimension, TInputPixel>::SegmentTubeUsingMinimalPathFilter(void)
{
  m_SpeedImage = NULL;
  m_RadiusImage = NULL;
...
template <unsigned int Dimension, class TInputPixel>
void
SegmentTubeUsingMinimalPathFilter<Dimension, TInputPixel>::SetIntermediatePoints(std::vector<PointType> pathPoints)
{
...
template <unsigned int Dimension, class TInputPixel>
void
SegmentTubeUsingMinimalPathFilter<Dimension, TInputPixel>::Update(void)
{
  using PathType = itk::PolyLineParametricPath<Dimension>;
  using PathFilterType = itk::SpeedFunctionToPathFilter<InputImageType, PathType>;
...
template <unsigned int Dimension, class TInputPixel>
bool
SegmentTubeUsingMinimalPathFilter<Dimension, TInputPixel>::IsPointTooNear(
  const InputSpatialObjectType * sourceTubeGroup,
  ...
  PointType &                    nearestPoint)
{
  ...
  for (typename InputSpatialObjectType::ChildrenListType::iterator tubeList_it = sourceTubeList->begin();
       tubeList_it != sourceTubeList->end();
       ++tubeList_it)
  {
    //**** Source Tube **** :
    ...
    typename TubeType::TransformType * pTubeObjectPhysTransform = pCurSourceTube->GetObjectToWorldTransform();
         ...
         ++pointList_it)
    {
      ...
      typename TubePointType::PointType curSourcePos =
        pTubeObjectPhysTransform->TransformPoint(curSourcePoint.GetPositionInObjectSpace());
...
template <unsigned int Dimension, class TInputPixel>
void
SegmentTubeUsingMinimalPathFilter<Dimension, TInputPixel>::PrintSelf(std::ostream & os, Indent indent) const
{
