...
PDFSegmenterParzen<TImage, TLabelMap>::PDFSegmenterParzen(void)
{
...
PDFSegmenterParzen<TImage, TLabelMap>::GetClassPDFImage(unsigned int classNum) const
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::SetClassPDFImage(unsigned int classNum, PDFImageType * classPDF)
{
...
PDFSegmenterParzen<TImage, TLabelMap>::GetNumberOfBinsPerFeature(void) const
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::SetNumberOfBinsPerFeature(const VectorUIntType & nBins)
{
...
PDFSegmenterParzen<TImage, TLabelMap>::GetBinMin(void) const
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::SetBinMin(const VectorDoubleType & binMin)
{
...
template <class TImage, class TLabelMap>
const typename PDFSegmenterParzen<TImage, TLabelMap>::VectorDoubleType &
PDFSegmenterParzen<TImage, TLabelMap>::GetBinSize(void) const
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::SetBinSize(const VectorDoubleType & scale)
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::GeneratePDFs(void)
{
  ...
  {
    typename ListSampleType::const_iterator inClassListIt(this->m_InClassList[c].begin());
    typename ListSampleType::const_iterator inClassListItEnd(this->m_InClassList[c].end());
  ...
  for (unsigned int i = 0; i < numFeatures; i++)
  {
    ...
    m_HistogramBinSize[i] = (histogramBinMax[i] - m_HistogramBinMin[i]) / (double)(m_HistogramNumberOfBin[i]);
  }
  ...
  if (true) // creating a local context to limit memory footprint
  {
    ...
    for (unsigned int c = 0; c < numClasses; c++)
    {
      ...
      while (inClassListIt != inClassListItEnd)
      {
        ...
        {
          double binV = (*inClassListIt)[i];
          int    binN = static_cast<int>((binV - m_HistogramBinMin[i]) / m_HistogramBinSize[i]);
          if (binN < 0)
          {
            binN = 0;
          }
          else if (static_cast<unsigned int>(binN) >= m_HistogramNumberOfBin[i])
          {
    ...
    for (unsigned int c = 0; c < numClasses; c++)
    {
      for (unsigned int i = 0; i < numFeatures; i++)
      {
        ...
        clipMax[c][i] = m_HistogramNumberOfBin[i] * m_HistogramBinSize[i] + m_HistogramBinMin[i];
      }
    }
        ...
        for (unsigned int i = 0; i < numFeatures; i++)
        {
          ...
          for (unsigned int b = 0; b < m_HistogramNumberOfBin[i]; ++b)
          {
            ...
            if (count >= tailReject)
            {
              if (b > 0)
              {
                clipMin[c][i] = (b - 1) * m_HistogramBinSize[i] + m_HistogramBinMin[i];
              }
              if (count - prevCount != 0)
              {
                clipMin[c][i] += m_HistogramBinSize[i] * ((tailReject - prevCount) / (count - prevCount));
              }
              break;
            }
          }
          ...
          for (int b = (int)m_HistogramNumberOfBin[i] - 1; b >= 0; --b)
          {
            ...
            if (count >= tailReject)
            {
              clipMax[c][i] = (b + 1) * m_HistogramBinSize[i] + m_HistogramBinMin[i];
              if (count - prevCount != 0)
              {
                clipMax[c][i] -= m_HistogramBinSize[i] * ((tailReject - prevCount) / (count - prevCount));
              }
              break;
            }
          }
  ...
  for (unsigned int i = numFeatures; i < PARZEN_MAX_NUMBER_OF_FEATURES; ++i)
  {
    spacing[i] = 1;
    origin[i] = 0;
    size[i] = 1;
  ...
  for (unsigned int c = 0; c < numClasses; c++)
  {
    ...
    while (inClassListIt != inClassListItEnd)
    {
      ...
      m_InClassHistogram[c]->SetPixel(indxHistogram, m_InClassHistogram[c]->GetPixel(indxHistogram) + 1);
      ++inClassListIt;
    }
  }
  ...
  if (true)
  {
    typedef itk::DiscreteGaussianImageFilter<HistogramImageType, HistogramImageType> HistogramBlurGenType;

    for (unsigned int c = 0; c < numClasses; c++)
    {
      if (m_HistogramSmoothingStandardDeviation > 0)
      {
        typename HistogramBlurGenType::Pointer blurFilter = HistogramBlurGenType::New();
        ...
        typedef itk::ThresholdImageFilter<HistogramImageType> ThresholdFilterType;
      ...
      typedef itk::NormalizeToConstantImageFilter<HistogramImageType, HistogramImageType> NormalizeImageFilterType;
      typename NormalizeImageFilterType::Pointer normFilter = NormalizeImageFilterType::New();
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::GenerateLabeledFeatureSpace(void)
{
  unsigned int numFeatures = this->m_FeatureVectorGenerator->GetNumberOfFeatures();
  ...
  itk::ImageRegionIterator<LabeledFeatureSpaceType> fsIter(m_LabeledFeatureSpace, region);

  unsigned int numClasses = this->m_ObjectIdList.size();

  typedef itk::ImageRegionIterator<HistogramImageType> PDFIteratorType;
  std::vector<PDFIteratorType *>                       pdfIter(numClasses);
  for (unsigned int i = 0; i < numClasses; i++)
  {
    pdfIter[i] = new PDFIteratorType(m_InClassHistogram[i], m_InClassHistogram[i]->GetLargestPossibleRegion());
    pdfIter[i]->GoToBegin();
  }
...
PDFSegmenterParzen<TImage, TLabelMap>::GetLabeledFeatureSpace(void) const
{
...
PDFSegmenterParzen<TImage, TLabelMap>::GetProbabilityVector(const FeatureVectorType & fv) const
{
  unsigned int                           numFeatures = this->m_FeatureVectorGenerator->GetNumberOfFeatures();
  ...
  for (unsigned int i = 0; i < numFeatures; i++)
  {
    int binN = static_cast<int>((fv[i] - m_HistogramBinMin[i]) / m_HistogramBinSize[i]);
    if (binN < 0)
    {
      binN = 0;
    }
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::SetLabeledFeatureSpace(LabeledFeatureSpaceType * labeledFeatureSpace)
{
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::Update(void)
{
  this->GenerateSample();
  this->GeneratePDFs();
...
template <class TImage, class TLabelMap>
void
PDFSegmenterParzen<TImage, TLabelMap>::PrintSelf(std::ostream & os, Indent indent) const
{
  Superclass::PrintSelf(os, indent);

  os << indent << "Histogram Smoothing Standard Deviation = " << m_HistogramSmoothingStandardDeviation << std::endl;
  os << indent << "InClassHistogram size = " << m_InClassHistogram.size() << std::endl;
  if (m_HistogramBinMin.size() > 0)
  {
    os << indent << "HistogramBinMin = " << m_HistogramBinMin[0] << std::endl;
    os << indent << "HistogramBinSize = " << m_HistogramBinSize[0] << std::endl;
    os << indent << "HistogramNumberOfBin[0] = " << m_HistogramNumberOfBin[0] << std::endl;
  }
  else
  {
  ...
  os << indent << "Outlier reject portion = " << m_OutlierRejectPortion << std::endl;

  if (m_LabeledFeatureSpace.IsNotNull())
  {
    os << indent << "LabeledFeatureSpace = " << m_LabeledFeatureSpace << std::endl;
  }
  else
  {
