...
public:
  ...
  typedef typename RidgeFeatureGeneratorType::FeatureImageType  FeatureImageType;
  ...
  virtual void
  SetInput(const InputImageType * img) override;
  virtual void
  SetInput(unsigned int id, const InputImageType * img) override;
  ...
  typename PDFSegmenterType::Pointer
  GetPDFSegmenter(void);
  void
  SetPDFSegmenter(PDFSegmenterType * pdfSegmenter);
  ...
  GetBasisImage(unsigned int num = 0) const;
