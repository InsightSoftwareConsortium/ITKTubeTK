...
public:
  ...
  operator()(itk::wasm::Pipeline & pipeline)
  {
    ...
    pipeline.add_option("-k,--match-mask", matchMask, "Match crop to the axis-aligned box that fits the mask.")
    ...
    std::vector<int> max;
    pipeline.add_option(
      "-M,--max", max, "Maximum coordinate (use instead of size). Another corner of the hyper-rectangle.");

    std::vector<int> size;
    pipeline.add_option(
      "-s,--size",
      size,
      "Size of ROI (use instead of maximum). Distance to the adjacent corner of the hyper-rectangle.");

    std::vector<int> center;
    pipeline.add_option(
      "-c,--center", center, "Center of ROI (use instead of minimum/maximum). Center of the hyper-rectangle.");
    ...
    if (size.size() > 0 || max.size() > 0 || min.size() > 0 || matchVolume.Get() || matchMask.Get())
    {
      ...
      {
        tube::ErrorMessage("You must specify either --center or --min options.  Not both.");
        return EXIT_FAILURE;
      }
