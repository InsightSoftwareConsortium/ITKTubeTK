...
int itktubePDFSegmenterRandomForestTest( int argc, char * argv[] )
{
  if( argc != 9 )
    {
    ...
    std::cout << argv[0]
      ...
      << " outputProbImg1 outputLabelMap" << std::endl;
    return EXIT_FAILURE;
    }
  ...
  typedef itk::tube::PDFSegmenterRandomForest< ImageType,
  ...
  filter->SetTrainingDataStride( 100 );
  filter->SetNumberOfDecisionTrees( 10 );
  filter->SetBalanceClassSampleSize( true );
  ...
  probWriter1->SetFileName( argv[7] );
  ...
  labelmapWriter->SetFileName( argv[8] );
