/*=========================================================================
  ...
  Program:   itkUNC
  Module:    $RCSfile: itkImageToTubeRigidRegistration.txx,v $
  Language:  C++
    ...
    for(float y = -10; y<=10; y+=1.0)
      {
        ...
        fprintf(fic,"%f %f %f %f %f %f %f %f %f %f %f %f %f\n",params[0],params[1],params[2],params[3],params[4],params[5],
          derivatives[0],derivatives[1],derivatives[2],derivatives[3],derivatives[4],derivatives[5],0);//this->GetMetric()->GetValue(params));
      }
    }

  std::cout << "Total time to run MonteCarlo simulation: " <<   ((float)clock()-(float)c0)/(float)CLOCKS_PER_SEC << " s" << std::endl;
...
*/
  ...
  std::cout << "Total Time = " << (clock()-c0)/(double)CLOCKS_PER_SEC << std::endl;
...
::SparseRegistration(ParametersType & parameters)
{
  ...
  static_cast<itk::ImageToTubeRigidMetric<TFixedImage,TMovingTube>*>(this->GetMetric())->SparseRegistration(parameters);
}
