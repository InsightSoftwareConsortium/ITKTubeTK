 ...
 *
 *=========================================================================*/
#ifndef itkTubeScaleSkewVersor3DTransform_hxx
#define itkTubeScaleSkewVersor3DTransform_hxx

#include "itkTubeScaleSkewVersor3DTransform.h"
#include "itkMath.h"
...
TubeScaleSkewVersor3DTransform<TParametersValueType>
::TubeScaleSkewVersor3DTransform() :
...
template<typename TParametersValueType>
TubeScaleSkewVersor3DTransform<TParametersValueType>
::TubeScaleSkewVersor3DTransform(unsigned int parametersDimension) :
  Superclass(parametersDimension)
{
  m_Scale.Fill(1.0);
  m_Skew.Fill(0.0);
...
// Constructor with arguments
template<typename TParametersValueType>
TubeScaleSkewVersor3DTransform<TParametersValueType>::TubeScaleSkewVersor3DTransform(const MatrixType & matrix,
...
::SetMatrix(const MatrixType & matrix, const TParametersValueType tolerance)
{
  vnl_svd<double> decomposition(
    matrix.GetVnlMatrix(),
    -1E-6);
  vnl_diag_matrix<vnl_svd<double>::singval_t> Winverse(
    decomposition.Winverse() );
  ...
  // Set Scale
  m_Scale[0] = matrix[0][0] - ortho[0][0] + 1.0;
  m_Scale[1] = matrix[1][1] - ortho[1][1] + 1.0;
  m_Scale[2] = matrix[2][2] - ortho[2][2] + 1.0;

  m_Skew[0] = matrix[0][1] - ortho[0][1];
  m_Skew[1] = matrix[0][2] - ortho[0][2];
  m_Skew[2] = matrix[1][0] - ortho[1][0];
  m_Skew[3] = matrix[1][2] - ortho[1][2];
  m_Skew[4] = matrix[2][0] - ortho[2][0];
  m_Skew[5] = matrix[2][1] - ortho[2][1];
...
template<typename TParametersValueType>
const typename TubeScaleSkewVersor3DTransform<TParametersValueType>::ParametersType
& TubeScaleSkewVersor3DTransform<TParametersValueType>
...
template<typename TParametersValueType>
void
TubeScaleSkewVersor3DTransform<TParametersValueType>::PrintSelf(std::ostream & os, Indent indent) const
{
