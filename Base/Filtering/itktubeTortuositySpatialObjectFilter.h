...
public:
  ...
  /** The kind of metrics that can be ran on the input spatial object.
    ...
    *  **** VESSEL-WISE METRICS ****
    *
    *  - Average radius metric: average of the radius at each point of the vessel
    *
    ...
    *
    *  - Inflection count 1 metric: This inflection count method is computed by
    * thresholding the curvature scalar function along the vessel, and counting the
    * number of times the curve goes over the threshold. For this 1st method, the
    * threshold is set to lambda/sigma. sigma is the smoothing scale (standard
    * deviation for gaussian smoothing), and lambda is an arbitrary value, set
    * to 1.5, because it works. See Curvature scalar metric below.
    *
    *  - Inflection count 2 metric: This inflection countis computed by the same
    * method as Inflection count 1 metric, except that the threshold value is the
    * Percentile 95 metric. See Percentile 95 metric below.
    *
    *  - Chord length metric: Length of the line between the first and last point
    *
    *  - Path length metric: Sum of distances between each point
    *
    *  - Percentile 95 metric: The 95th percentile of the histogram of the curvature
    * scalars along the vessel. It is the value for which 95% of the points have
    * curvature scalars below it. See the Curvature scalar metric below.
    *
    ...
    * curvature vector. See the Curvature vector metric below. This metric is used to
    * calculate a lot of other metrics.
    *
    *  - Curvature vector metric: Defined at each point as the cross product between
    * the first and the second derivative of the spatial line. It is a 3-components
    * vector (x, y, z). Mostly used for debugging
    *
    ...
    * The histogram of the curvature scalar along the vessel is computed with the number
    * of bins, minimum and maximum passed in parameters. The population in each bin is
    * considered as a metric. This is intended to be fed to a machine learning algorithm.
    *
    *
    *
    *   **** METRIC BITMASKS ****
    ...
    *
    *
    * NOTE: For the metrics (not the bitmasks), the numbers are given by date of
    * addition, but the names are sorted by metric type, then alphabetically.
    *
    */
  enum MeasureType
    {
  ...
  /** Set/Get the sensibility of the filter. Spacing values are usually around 0.1
    * On some vessels, it can be lower, and it causes the CURVATURE_SCALAR_METRIC
    * to go abnormally high. We don't normalize by spacing when it is lower than
    * EpsilonForSpacing. Default value (1e-2) should be acceptable for most cases.
    */
  itkSetClampMacro( EpsilonForSpacing, double, 0, std::numeric_limits<double>::max())
  ...
  /** Set/Get the sensibility of the filter. This is used in internal
    ...
    */
  itkSetClampMacro( EpsilonForZero, double, 0, std::numeric_limits<double>::max())
  itkGetConstMacro( EpsilonForZero, double )

  /** Set/Get the maximum of the range of values the histogram must be computed on.
    * Used for CURVATURE_HISTOGRAM_METRICS. Default value is 1.
    */
  itkSetClampMacro( HistogramMax, double, 0, std::numeric_limits<double>::max() )
  itkGetConstMacro( HistogramMax, double )

  /** Set/Get the minimum of the range of values the histogram must be computed on.
    * Used for CURVATURE_HISTOGRAM_METRICS. Default value is 0.
    */
  itkSetClampMacro( HistogramMin, double, 0, std::numeric_limits<double>::max() )
  itkGetConstMacro( HistogramMin, double )

  /** Set/Get the arbitrary parameter lambda used to compute Inflection count 1 metric
    * By default, it is set to 1.5, because it works well. These methods are mainly
    * for experimentation. See INFLECTION_COUNT_1_METRIC
    */
  ...
  /** Set/Get the smoothing method to be applied to the vessel before computing
    * the metrics. Default method is SMOOTH_TUBE_USING_INDEX_GAUSSIAN.
    */
  itkSetMacro( SmoothingMethod, ::tube::SmoothTubeFunctionEnum  )
  itkGetConstMacro( SmoothingMethod, ::tube::SmoothTubeFunctionEnum  )

  /** Set/Get the smoothing scale to be applied to the vessel before computing
    * the metrics.
    * This value has different meanings when using different smoothing methods.
