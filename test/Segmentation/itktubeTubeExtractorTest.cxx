...
int
itktubeTubeExtractorTest(int argc, char * argv[])
{
  if (argc != 3)
  {
    std::cout << "itktubeTubeExtractorTest <inputImage> <vessel.tre>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (int mcRun = 0; mcRun < 2; mcRun++)
  {
    ...
    if (tubeOp->FindLocalTubeInObjectSpace(pntX1) != TubeOpType::RidgeExtractorType::SUCCESS)
    {
    ...
    if (diff > 2)
    {
      std::cout << "Local tube test failed.  Local tube too far." << std::endl;
    ...
    std::cout << "***** Attempting duplicate extraction - should fail *****" << std::endl;
    ...
    if (xTube2.IsNotNull())
    {
      ...
      std::cout << "  Tube size = " << xTube2->GetPoints().size() << std::endl;
      ++failures;
    ...
    std::cout << "***** Attempting extract after delete *****" << std::endl;
  ...
  itk::ImageRegionIterator<TubeOpType::TubeMaskImageType> maskIt(mask, mask->GetLargestPossibleRegion());
