...
int
itktubeRadiusExtractor2Test(int argc, char * argv[])
{
  if (argc != 3)
  {
    std::cout << "itktubeRadiusExtractor2Test <inputImage> <vessel.tre>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (unsigned int mcRun = 0; mcRun < numMCRuns; mcRun++)
  {
    ...
    unsigned int      rndPointNum = rndGen->GetUniformVariate(0, 1) * numPoints * 0.8 + numPoints * 0.1;
    ...
    double rStep = 0.25;
    double rTol = 0.125;
    if (!radiusOp->GetPointVectorOptimalRadius(tubePoints, r1, rMin, rMax, rStep, rTol))
    {
