...
int
itktubeRidgeExtractorTest(int argc, char * argv[])
{
  if (argc != 3)
  {
    std::cout << "itktubeRidgeExtractorTest <inputImage> <outputImage>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "RidgenessStart threshold = " << minRidgenessStart << std::endl;
  ...
  std::cout << "CurvatureStart threshold = " << minCurvatureStart << std::endl;
  ...
  std::cout << "RoundnessStart threshold = " << minRoundnessStart << std::endl;
  ...
  while (!itOut.IsAtEnd())
  {
    ...
    switch (taskHash)
    {
      ...
      case 1:
      {
        itOut.Set(ridgeOp->Ridgeness(pntOut, intensity, roundness, curvature, levelness));
        break;
      }
      case 2:
      {
        ridgeOp->Ridgeness(pntOut, intensity, roundness, curvature, levelness);
        itOut.Set(roundness);
        break;
      }
      case 3:
      ...
      case 5:
      {
        if ((int)(contIndx[1]) == (int)(size[1] / 2))
        {
          ...
          RidgeOpType::FailureCodeEnum ridgeResult = ridgeOp->LocalRidge(pntOut);
          if (ridgeResult == RidgeOpType::SUCCESS)
          {
            ...
            if (isInside)
            {
              ...
              std::cout << "      ridgeness = " << ridgeOp->GetCurrentRidgeness() << std::endl;
              std::cout << "      roundness = " << ridgeOp->GetCurrentRoundness() << std::endl;
              std::cout << "      curvature = " << ridgeOp->GetCurrentCurvature() << std::endl;
              std::cout << "      levelness = " << ridgeOp->GetCurrentLevelness() << std::endl;
            }
          }
          else
          {
