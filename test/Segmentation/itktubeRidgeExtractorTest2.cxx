...
int
itktubeRidgeExtractorTest2(int argc, char * argv[])
{
  if (argc != 3)
  {
    std::cout << "itktubeRidgeExtractorTest <inputImage> <vessel.tre>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (int mcRun = 0; mcRun < 2; mcRun++)
  {
    ...
    {
      --mcRun;
      std::cout << "Warning: Tube point outside of image, repicking..." << std::endl;
    ...
    if (ridgeOp->LocalRidge(xRidgePnt) != RidgeOpType::SUCCESS)
    {
      ...
      std::ignore = im->TransformPhysicalPointToContinuousIndex(xRidgePnt, xRidgeContI);
      std::cout << "*** FAILURE: Local ridge test failed.  No ridge found." << std::endl;
      ...
      ridgeness = ridgeOp->Ridgeness(xRidgePnt, intensity, roundness, curvature, levelness);
    ...
    {
      std::cout << "*** FAILURE: Local ridge test failed.  Local ridge too far." << std::endl;
      std::cout << "   Source = " << xContI << std::endl;
      std::cout << "   Result = " << xRidgeContI << std::endl;
      ++failures;
      continue;
    ...
    if (xTube.IsNull())
    {
      std::cout << "*** FAILURE: Ridge extraction failed...repicking" << std::endl;
      ++failures;
      continue;
    }
  ...
  for (unsigned int code = 0; code < ridgeOp->GetNumberOfFailureCodes(); ++code)
  {
    std::cout << "   " << ridgeOp->GetFailureCodeName(RidgeOpType::FailureCodeEnum(code)) << " : "
              << ridgeOp->GetFailureCodeCount(RidgeOpType::FailureCodeEnum(code)) << std::endl;
  }
