...
int
itktubeTubeXIOTest(int argc, char * argv[])
{
  if (argc != 6)
  {
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    if (ioMethod->GetDimensions()[i] != dim)
    {
      std::cerr << "Error, dimensions are not the same." << std::endl
                << " Expected: " << ioMethod->GetDimensions()[i] << " got: " << dim << std::endl;
      return EXIT_FAILURE;
    }
  }

  IOMethodType::TubeGroupType::Pointer tubeGroup = ioMethod->GetTubeGroup();
