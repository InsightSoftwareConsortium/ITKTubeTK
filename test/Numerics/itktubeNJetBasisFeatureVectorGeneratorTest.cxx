...
int
itktubeNJetBasisFeatureVectorGeneratorTest(int argc, char * argv[])
{
  if (argc != 7)
  {
  ...
  for (unsigned int i = 0; i < basisFilter->GetNumberOfFeatures(); ++i)
  {
    ...
    {
      std::cerr << "Unhandled exception caught during basis write:" << fname << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  for (unsigned int i = 0; i < filter->GetNumberOfFeatures(); ++i)
  {
    ...
    {
      std::cerr << "Unhandled exception caught during write:" << fname << std::endl;
      return EXIT_FAILURE;
    }
  }
