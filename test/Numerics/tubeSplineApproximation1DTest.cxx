...
int
tubeSplineApproximation1DTest(int argc, char * argv[])
{
  if (argc != 2)
  {
  ...
  itk::ImageRegionIteratorWithIndex<ImageType> itIm(im, im->GetLargestPossibleRegion());
  ...
  while (!itIm.IsAtEnd())
  {
    ...
    x = pnt[0] + pnt[1] * 0.2;
    if (itIm.GetIndex()[0] == itIm.GetIndex()[1] && itIm.GetIndex()[0] == 0)
    {
      std::cout << "Slice index = " << (itIm.GetIndex()[2] - index0[2]) % 7 << std::endl;
    }
  ...
  for (unsigned int c = 0; c < 100; c++)
  {
    ...
    else
    {
      ...
      {
        std::cout << "Spline.Extreme() solution not ideal: x=" << x << " != ideal=-1.4749" << std::endl;
        ...
        {
          std::cout << "    tx0 = " << tx << " v = " << spline.Value(tx) << std::endl;
        }
        for (double tx = x - 1; tx < x + 1; tx += 0.1)
        {
          std::cout << "    tx = " << tx << " v = " << spline.Value(tx) << std::endl;
        }
        returnStatus = EXIT_FAILURE;
        err = true;
      }
      ...
      {
        std::cout << "Spline.Extreme() output not ideal: val=" << xVal << " != ideal=1.0" << std::endl;
        std::cout << "   Optimization started at x = " << x0 << std::endl;
        std::cout << "   Iteration = " << c << " of 100." << std::endl;
        returnStatus = EXIT_FAILURE;
        err = true;
