...
int
tubeGoldenMeanOptimizer1DTest(int argc, char * argv[])
{
  if (argc < 9)
  {
  ...
  if (opt->GetXMin() != xMin)
  {
    std::cout << "xMin should be " << xMin << " and not " << opt->GetXMin() << std::endl;
  ...
  if (opt->GetXMax() != xMax)
  {
    std::cout << "xMax should be " << xMax << " and not " << opt->GetXMax() << std::endl;
    returnStatus = EXIT_FAILURE;
  }
  ...
  {
    std::cout << "xStep should be " << 0.1 * factor << " and not " << opt->GetXStep() << std::endl;
    returnStatus = EXIT_FAILURE;
  }
  ...
  {
    std::cout << "tolerance should be " << 0.000001 * factor << " and not " << opt->GetTolerance() << std::endl;
    returnStatus = EXIT_FAILURE;
  }
  ...
  {
    std::cout << "maxIterations should be 300 and not " << opt->GetMaxIterations() << std::endl;
    returnStatus = EXIT_FAILURE;
  }
  ...
  {
    std::cout << "Optimization not within tolerance!  x=" << x << " diff=" << std::fabs(idealX - x) << std::endl;
    returnStatus = EXIT_FAILURE;
  }

  if (std::fabs(idealV - xVal) > epsilon)
  {
    std::cout << "Optimization not within tolerance!  xVal=" << xVal << std::endl;
    returnStatus = EXIT_FAILURE;
  }
