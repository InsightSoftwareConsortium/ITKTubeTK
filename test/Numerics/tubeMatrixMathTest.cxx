...
Test(void)
{
  ...
  for (unsigned int count = 0; count < 1000; count++)
  {
    ...
    if (VDimension == 3)
    {
      ...
      {
        std::cout << count << " : ";
        std::cout << "FAILURE: ComputeOrthogonalVector: DotProduct = " << v1 << " .* " << v2 << " = "
                  << dot_product(v1, v2) << std::endl;
        returnStatus = EXIT_FAILURE;
      }

      vnl_vector<float> v3 = tube::ComputeCrossVector(v1, v2);
      if (std::fabs(dot_product(v1, v3)) > epsilon || std::fabs(dot_product(v2, v3)) > epsilon)
      {
        std::cout << count << " : ";
        std::cout << "FAILURE: ComputeCrossVector: DotProduct = " << dot_product(v1, v3) << " and "
    ...
    if (std::fabs(tube::ComputeEuclideanDistanceVector(v1, v4) - 0.5) > epsilon)
    {
      std::cout << count << " : ";
      std::cout << "FAILURE: ComputeLineStep = " << v1 << " + 0.5 * " << v2 << " != " << v4 << std::endl;
      std::cout << "FAILURE: ComputeEuclidenDistanceVector = " << tube::ComputeEuclideanDistanceVector(v1, v4)
                << std::endl;
      returnStatus = EXIT_FAILURE;
    }
    ...
    for (unsigned int d = 0; d < VDimension; d++)
    {
      ...
      if (std::fabs(v1.magnitude() - std::fabs(eVals[d])) > epsilon)
      {
        ...
        std::cout << "FAILURE: ComputeEigen : "
                  << " v1 * M1 = " << v1 << " and v1 norm = " << v1.magnitude() << " != " << eVals[d] << std::endl;
        returnStatus = EXIT_FAILURE;
      }
    }
  }
...
int
tubeMatrixMathTest(int itkNotUsed(argc), char * itkNotUsed(argv)[])
{
  if (Test<2>() == EXIT_FAILURE || Test<3>() == EXIT_FAILURE || Test<4>() == EXIT_FAILURE)
  {
    return EXIT_FAILURE;
  }
