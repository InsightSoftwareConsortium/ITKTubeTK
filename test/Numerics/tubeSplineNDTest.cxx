...
int
tubeSplineNDTest(int argc, char * argv[])
{
  if (argc != 2)
  {
  ...
  for (unsigned int c = 0; c < 100; c++)
  {
    ...
    {
      bool err = false;
      if (std::fabs(x[0] - 3.139) > 0.001 || std::fabs(x[1] - 0.0) > 0.001)
      {
        std::cout << "Spline.Extreme failed." << std::endl;
        std::cout << "  x = ( " << x[0] << ", " << x[1] << " ) != ideal = ( 3.1139, 0.0 )" << std::endl;
        returnStatus = EXIT_FAILURE;
        err = true;
      }
      ...
      {
        std::cout << "Success: x = " << x << " : xVal = " << xVal << std::endl;
      }
    }
  }
  for (unsigned int c = 0; c < 100; c++)
  {
    ...
    else
    {
      ...
      if (std::fabs(x[0] - 3.139) > 0.1 || std::fabs(x[1] - 0.0) > 1.1)
      {
        std::cout << "Spline.ExtremeConjGrad failed." << std::endl;
        std::cout << "  x = ( " << x[0] << ", " << x[1] << " ) != ideal = ( 3.1139, 0.0 )" << std::endl;
        returnStatus = EXIT_FAILURE;
