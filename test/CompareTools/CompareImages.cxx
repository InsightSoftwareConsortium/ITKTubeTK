...
int
RegressionTestImage(const char *, const char *, int, bool, double, int, int, double);

int
CompareImages(int argc, char * argv[])
{
  if (argc < 5)
  {
    std::cerr << "Usage:" << std::endl;
    std::cerr << "-t testImage" << std::endl;
    std::cerr << "-b baselineImage1[,baselineImage2[,baselineImage3[, ...]" << std::endl;
    std::cerr << "    If more than one baselineImage, test will pass if any" << std::endl;
  ...
  // Option for setting the number of pixel that can be tolerated to
  // have different intensities.
  command.SetOption("toleranceCoordinates", "c", false, "Acceptable differences in image origin/spacing");
  command.AddOptionField("toleranceCoordinates", "value", MetaCommand::FLOAT, true);
  ...
  command.SetOption("baselineImages", "b", true, "Baseline images filename");
  command.AddOptionField("baselineImages", "filenames", MetaCommand::STRING, true);
  ...
  {
    toleranceCoordinates = command.GetValueAsFloat("toleranceCoordinates", "value");
  }
