...
int
RegressionTestFile(const char * testFilename,
                   const char * baselineFilename,
                   bool         reportErrors,
                   bool         createDifferenceFile,
                   double       valueTolerance,
                   int          numberOfDifferenceTolerance);

int
CompareTextFiles(int argc, char * argv[])
{
  ...
  command.SetOption("toleranceValue", "d", false, "Acceptable differences in numeric values in the files");
  command.AddOptionField("toleranceValue", "value", MetaCommand::FLOAT, true);

  // Option for setting the number of pixel that can be tolerated to
  // have different intensities.
  command.SetOption("toleranceNumberOfDifferences",
                    "n",
                    false,
                    "Number of differences that are acceptable to have intensity differences");
  command.AddOptionField("toleranceNumberOfDifferences", "value", MetaCommand::INT, true);

  // Option for setting the filename of the test image.
  command.SetOption("testFile", "t", true, "Filename of the text file to be tested against the baseline images");
  command.AddOptionField("testFile", "filename", MetaCommand::STRING, true);

  // Option for setting the filename of multiple baseline images.
  command.SetOption("baselineFiles", "B", false, "List of baseline text files <N> <image1> <image2>...<imageN>");
  command.AddOptionField("baselineFiles", "filename", MetaCommand::LIST, true);

  // Option for setting the filename of a single baseline image.
  command.SetOption("baselineFile", "b", false, "Baseline text file filename");
  command.AddOptionField("baselineFile", "filename", MetaCommand::STRING, true);
  ...
  {
    toleranceValue = command.GetValueAsFloat("toleranceValue", "value");
  }
  ...
  {
    toleranceNumberOfDifferences = command.GetValueAsInt("toleranceNumberOfDifferences", "value");
  }

  // Get the filename of the image to be tested
  if (command.GetOptionWasSet("testFile"))
  {
    testFilename = command.GetValueAsString("testFile", "filename");
  }
  ...
  if (!command.GetOptionWasSet("baselineFile") && !command.GetOptionWasSet("baselineFiles"))
  {
    std::cerr << "You must provide a -BaselineFile or -BaselineFiles option" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (command.GetOptionWasSet("baselineFile"))
  {
    ...
    baselineFilename = command.GetValueAsString("baselineFile", "filename");
  }
  ...
  try
  {
    ...
    {
      bestBaselineStatus = RegressionTestFile(
        testFilename.c_str(), baselineFilename.c_str(), false, false, toleranceValue, toleranceNumberOfDifferences);
    ...
    else
    {
      ...
      while (baselineFileItr != baselineFilenames.end())
      {
        ...
        const int currentStatus = RegressionTestFile(
          testFilename.c_str(), baselineFileItr->c_str(), false, false, toleranceValue, toleranceNumberOfDifferences);
    ...
    if (bestBaselineStatus == 0)
    {
      ...
      RegressionTestFile(
        testFilename.c_str(), bestBaselineFilename.c_str(), true, false, toleranceValue, toleranceNumberOfDifferences);
    }
    else
    {
      std::cout << "Rejecting : " << bestBaselineFilename.c_str() << std::endl;
      RegressionTestFile(
        testFilename.c_str(), bestBaselineFilename.c_str(), true, true, toleranceValue, toleranceNumberOfDifferences);
    }
  }
                   ...
                   int          numberOfDifferenceTolerance)
{
  ...
  if (createDifferenceFile)
  {
    ...
    if (!diffFout.is_open())
    {
      std::cerr << "Cannot create difference file = " << diffFilename << std::endl;
      return EXIT_FAILURE;
    }
  }
