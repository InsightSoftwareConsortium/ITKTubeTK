...
int
itktubePointsToImageTest(int argc, char * argv[])
{
  if (argc != 3)
  {
    std::cout << "itktubePointsToImageTest <vessel.tre> <outputImage>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  typedef itk::SpatialObject<Dimension>::ChildrenListType ObjectListType;
  ...
  for (unsigned int i = 0; i < numTubes; i++)
  {
    ...
    for (unsigned int j = 0; j < numPoints; j++)
    {
      ...
      std::cout << "point " << j << "; "
                << "position " << position << "radius " << radius << "normal1 " << normal1 << "normal2 " << normal2
                << std::endl;
      ++pointIt;
    }
    ++tubeIt;
  ...
  SpatialObjectToImageFilterType::Pointer tubeToImageFilter = SpatialObjectToImageFilterType::New();
